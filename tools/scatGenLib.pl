...
sub scatGen_main
{
    my ($func, $bb, $flash_href, $flash_blk_href, $rs, $nor_device, $fota_cfg, 
        $bb_path, $makefile, $need_build_bootloader, $mem_dev_h_cfg, 
        $IsFlashtoolLayoutInput, $use_dummy_scatter, $feature_config, 
        $nFactoryBinSize, $obj_sys_auto_gen) = @_;
    $feature_query_function_ptr = $func;
    $g_bb                       = $bb;
    $norflash_info_href         = $flash_href;
    $norflash_info_blk_href     = $flash_blk_href;
    ...
    my $alice    = &$func('alice_support');
    
    my $compiler = &$func('compiler');
    if($compiler ne "RVCT")
    {
        &sysUtil::sysgen_die("[1.1]Unsupported Compiler: $compiler !", __FILE__, __LINE__);
    }
    ...
    if (&sysUtil::is_SmartPhone($bb) == 1)
    {
        $output_result .= &layout9($bb);
    ...
    if (&isNAND($func))
    {
        if ($bb eq 'MT6253' or &sysUtil::is_MT6261_Family($bb))
        {
            &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination NFB on platform $bb!", __FILE__, __LINE__);
        }
          
        if (defined $zimage and $zimage eq 'TRUE')
        {
            &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : ZIMAGE + NFB!", __FILE__, __LINE__);
        }
        ...
        {
            &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : FOTA + ENFB!", __FILE__, __LINE__) if($g_nfb_or_emb eq 'ENFB');
            $output_result .= &layout6($bb);
        }
        # Add FACTORY_COMBINE layout
        if(defined $fast_boot and $fast_boot eq 'FACTORY_COMBINE')
        {
            &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : FACTORY_COMBINE + ENFB!", __FILE__, __LINE__) if($g_nfb_or_emb eq 'ENFB');
        ...
        my $is_secure = ($secure_ro_enable eq 'TRUE' or $security eq 'TRUE') ? 1 : 0;
        if($is_secure and ((&sysUtil::is_arm7($bb) == 1) and ($g_nfb_or_emb eq 'BASIC')))
        {
            &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : SECURE + BASIC on ARM7 platform!", __FILE__, __LINE__);
        }
    ...
    else
    {
        ...
        {
            &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination NOR booting on platform $bb!", __FILE__, __LINE__);
        }
        ...
        if (defined $zimage and $zimage eq 'TRUE')
        {
            if ((defined $fota and $fota eq 'FOTA_DM'))
            {
                &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : ZIMAGE + FOTA!", __FILE__, __LINE__);
            }
        }
        if ((defined $zimage and $zimage eq 'TRUE') and (defined $multibin and $multibin eq 'TRUE'))
        {
            if (&sysUtil::is_sv5($bb) != 1)
            {
                &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : ZIMAGE + MBA on non sv5 platform $bb!", __FILE__, __LINE__);
            }
        }
        if ($fota eq 'FOTA_DM')
        {
            ...
            {
                &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : FOTA + ALICE!", __FILE__, __LINE__);
            }
        }
        
        # scatter Assert - used to trigger 2-phase linking
        my $strScatterAssert = ($obj_sys_auto_gen eq 'TRUE')? "ScatterAssert(0)\n" : "";
        $output_result .= $strScatterAssert;
        
        # use builtin function and alignment
        my ($alignment_config,$preCompile);
        my $norflash_non_xip = &$func('norflash_non_xip_support');
        my $isMauiInit = &$func("dcm_compression_maui_init");
        if (defined $norflash_non_xip and $norflash_non_xip eq 'TRUE')
        {   
            $preCompile = 1;
...
__TEMPLATE
        ...
        if(defined $isMauiInit and $isMauiInit eq 'TRUE')
        {
            $preCompile = 1;
            $alignment_config .= <<"__TEMPLATE";
#define ALIGNMENT_32B 0x20
__TEMPLATE
        }
...
__TEMPLATE
        ...
        $output_result .= $alignment_config;
        ...
        elsif (($multibin eq 'TRUE') && ($zimage eq 'TRUE'))
        {
            $output_result .=  &layout12($bb, $flash_size, $rs, $boot_cert);
        }
...
sub scatGen_verno
{
    return "v6.99 -> p7.00 -> p7.01 -> p7.04 -> p7.05 -> p7.09 -> p7.14" . " + scatFrame". scatFrame::scatFrame_verno() ;
    # p7.14 by Yinli,  20140421, Reserve 12kB ram-space for SWLA when zimage using BEST_PERF policy
    # p7.09 by Yinli,  20140324, Adjust intsram distribution to reslove INTSRAM_CODE expired when TINY_SYSTEM is enabled
    # p7.05 by Yinli, 20140128, Set EXT_BL max-size to 28kB when SECURE_SUPPORT is enabled on 32_32 segment for MT6260 and MT6261 platform
    # p7.04 by Yinli, 20140117, Support MT6261 family
    # p7.01 by Yinli, 20140117, Support BL scatter file re-org
    # p7.00 by Yinli, 20140117, remove RO-CODE in MBA region
    # v6.99 by Yinli, 20131107, Add OVERLAY attribute to ZIMAGE region when DCM_COMPRESSION_MAUI_INIT is enabled
    #                           to avoid ROM access ZIMAGE region before it is initialized
    # v6.98 by Yinli,  20131101, Set EXT_BL max-size to 20kB when SECURE_SUPPORT is enabled on 32_32 segment for MT6260 platform
    # v6.97 by Yinli,  20131031, Set EXT_BL max-size to 20kB when ENABLE_SECURITY_ON_SLIM_SEG is enabled on 32_32 segment
    # v6.96 by Yinli,  20131031, Support DCM new region MAUIINIT
    # v6.95 - reserved for add Add CustDataAccountRes.obj in custpack region (Bree\TRUNK\Bree)
    # v6.94 by Yinli,  20130923, A verno-number is reserved for the new trunk branch 11C
    # p6.93 - reserved for modify LANG_PACK and CUSTPACK input section list content, patch back from Bree to Niki (NIKITA/TRUNK/NIKI60)
    # p6.92 by Yinli,  20130816, Support BT Box, ROM will shadow to RAM when NORFLASH_NON_XIP is enabled
    # p6.91 by Carina, 20130508, ROOTCERT and CBR region are generated if one of the SECURE_SUPPORT and SECURE_RO_ENABLE is TRUE
    #                            and ROOTCERT has size if SECURE_SUPPORT = TRUE
    # p6.90 by Carina, 20130503, [scatGen.pl] use five more blocks size as SDS spare size on MT6260 NAND project
    # p6.89 by Carina, 20130416, Support DCM new region VCH
    # p6.88 by Carina, 20130408, Support ENFB+SECURE layout
    # p6.87 by Carina, 20130508, Get the CBr total size from the real flash block size
    # p6.86 by Carina, 20130315, To put third_rom_int.obj in primary MAUI for removing from sys_sec.lib to sys_int.lib
    # p6.85 by Carina, 20130306, Put color_custom*.obj in primary MAUI for PQ support
    # p6.84 by Carina, 20130305, Modify DCMP/MBA+VIVA dummy scatter to prevent from wrong fail result
    # p6.81 by Carina, 20130227, Correct the base address of VIVA and JUMP_TABLE in ZIMAGE+MBA layout
    # v6.80 by Carina, 20130220, [scatGen.pl] modify the script to adapt for the build environment of customer
    # v6.79 by Carina, 20130207, Remove DCMP load region max size
    # v6.78 by mei, 20130205, Support DCMP got from csv and support FORCE 2 phase linking
    # v6.77 by mei, 20130204, Refactory DCMP part and fix bugs
    # v6.76 - reserved for ignoring small segment for BL reserved size while secure support (11BW1224_RESTRICT_SP)
    # v6.75 - reserved for generating MBA and VIVA signature regions for verifying all images (11BW1224_RESTRICT_SP)
    # v6.74 - reserved for adding new objects in LANG_PACK_ROM_CONTENT for customer's request (11BW1224_RESTRICT_SP)
    # v6.73 by Carina, 20130129, Support ALICE on NAND
    # v6.72 by Carina, 20130128, Modify the pool size of codec according to the new region MATVDRIVER
    # v6.71 - reserved for modifying the script to adapt for the build environment of customer (11BW1132SP2)
    # v6.70 by Carina, 20130123, Modify MM/IME layout (seperate CAMCAL and CAMMED to another non-overlay group if INPUT_METHOD is none)
    # v6.69 - reserved for supporting BL special layout if CARD_DOWNLOAD support (11BW1132SP2)
    # v6.68 by Carina, 20130122, Support MM/IME DCM pool size adjustment by flavor (modify the input parameter of query_MM_IME_Grouplength())
    # v6.67 by Carina, 20130121, Support MM/IME DCM pool size adjustment by flavor
    # v6.66 by Carina, 20130121, Move lcm related code to primary MAUI
    # v6.65 by Carina, 20130117, Enlarge VEENC/VEDEC pool size if KINGMOVIE_FILE_FORMAT_SUPPORT = TRUE
    # v6.64 by Carina, 20130114, [scatGen.pl] modify custom_img_config.h to fix the MBA potential bug
    # v6.63 by Carina, 20130111, Support MM/IME DCM modification and new layout
    # v6.62 by Carina, 20130201, Add three objects in LANG_PACK_ROM_CONTENT region and new pragma to support string compression
    # v6.61 by Carina, 20130110, BL shrink support 24_32 segment
    # v6.60 - reserved for removing BL_FEATURE_SUPPORT (11BW1132SP)
    # v6.59 - reserved for Enlarge _HIDDEN_SECURE_RO MaxSize from 4K to 8K ()
    # v6.58 by Carina, 20121221, Support MT6260 NAND booting
    # v6.57 by Carina, 20121220, Support new DCM regions for MMI slim approach
    # v6.56 by Carina, 20121217, [scatGenBL.pl] Modify the base address of BL_INFO_NOR
    # v6.55 - reserved for supporting 3G memory shrink layout on MT6577 (11AMDW1236SP)
    # v6.54 - reserved for modifying scatGen to support JRDSH MiniQ3G(MT6276+MT3336)
    # v6.53 - reserved for modifying the order of Demand Paging regions to fix a bug (11AW1112SP1\11AW1112MP\11AW1144MP)
    # v6.52 by Carina, 20121116, Move Dec2Hex from sysGenUtility to CommonUtility
    # v6.51 by Carina, 20121113, Refine DCM subroutines
    # v6.50 by Carina, 20121106, Support MT6260 NOR booting
    # v6.49 by Carina, 20121031, [scatGen.pl]Move is_NOR() from sysGenUtility to FileInfoParser to support general query
    # v6.48 by Carina, 20121018, The wildcard is generated by objListGen if ALICE or ZIMAGE is enabled to support ZIMAGE_CONTENT_POLICY
    # v6.47 by Carina, 20121009, [scatGen.pl] generate dummy scatter pattern in dummy scatter file
    # v6.46 by Carina, 20121106, Support shrinking CBR block size of MT6250D/MT6250M (11BW1132SP)
    # v6.45 by Carina, 20120920, Isolate THIRD_ROM execution regions
    # v6.44 - reserved for supporting BL_FEATURE_SUPPORT (11BW1132SP)
    # v6.43 by Carina, 20120906, Support ENFB(Third_ROM)
    # v6.42 by Carina, 20120831, Phase-out system_drive_on_nand feature option
    # v6.41 by Carina, 20120828, layout1() refinement and add layoutMBA_NAND() & layoutSECURE_NAND
    # v6.40 by Carina, 20120717, Move DSPTXRX_query_length to sysgenUtility.pm
    # v6.39 by Carina, 20120710, Support EWS via tools::pack_dep_gen
    # v6.38 by Carina, 20120629, Modify pragma (_fs_filetable) to (_fs_filetable_zi) to reduce ROM usage
    # v6.37 by Carina, 20120625, Fix the bug of GetBottomRegion() and GetBottomLayout() for MBA+ZIMAGE
    # v6.36 by Carina, 20120619, Add _nvram_callback_tbl into EXTSRAM_NVRAM_LTABLE
    # v6.35 by Carina, 20120614, Enlarge EXTSRAM_DSP_RX size in MT6250 GSM project
    # v6.34 by Carina, 20120612, Error message refinement
    # v6.33 by Carina, 20120608, [scatGenBL.pl]add new region "REGION_INIT_SUB" in Ext Bootloader¡¦s scatter file for 50 BL shrinking
    # v6.32 by Carina, 20120605, [scatGen.pl]modify the base address of SDS to grow from the bottom of flash
    # v6.31 by Carina, 20120604, Modify DCM pool size from 64KB to 68KB when only MPEG4 decoder enabled
    ...
    # v6.26 by Carina, 20120529, Moveout gdi_g2d.lib from demand paging region due to boot-up fail
    # v6.25 by Carina, 20120529, Support DCM audio new feature options to decide whether audio related part is put into DCM pool
    # v6.24 by Carina, 20120529, Support MBA with compression (MBA+ZIMAGE)
    # v6.23 by Carina, 20120524, Modify ZImage input section in ZIMAGE.csv to solve sanity fail(boot-up fail)
    ...
    # v6.17 by Carina, 20120516, Add CAT pragma to support CAT new feature
    # v6.16 by Carina, 20120516, Modify ZIMAGE execution name to ZIMAGE_ER to support ScatterAssert function
    # v6.15 by mei, 20120515, Rename #define ALIGN to SetAddress
    # v6.14 by Carina, 20120514, [scatGen.pl] Modify custom_secure_config_h_file_body() to autogen default values for SDS
    # v6.13 by mei, 20120513, Isolate ZImage input section
    # v6.12 - reserved for always generating INTSRAM_DATA_PREINIT region
    ...
    # v5.60 by mei, 20120112, Not to use Project Limit if not INTER_MTK
...
sub NOR_Get_BASE_and_MaxSize
{
    ...
    if($RegionInfo_ref->{$strKey}[0] == BLOCK_ALIGNED)
    {
        ...
        {
            $nblock_size = &memConfig::Get_flash_block_size($nBottomAddress - 1,$norflash_info_href->{'AVAILABLE_SIZE'});
...
sub GetBottomRegion
{
    ...
    my $alice = &$feature_query_function_ptr('alice_support') if ($mba eq 'TRUE' and $zimage ne 'TRUE');
    my $dcmp = &$feature_query_function_ptr('dcm_compression_support') if ($mba eq 'TRUE' and $zimage ne 'TRUE');
...
sub GetBottomLayout
{
    ...
    {
        $viva_Layout = &layoutVIVA($RegionInfo_ref->{'VIVA'}->[0], $RegionInfo_ref->{'VIVA'}->[1]);
    }
    ...
    # --- --- ---
    # ALICE region
    my $alice_Layout = &layoutALICE($bb,"+0x0") if(defined $alice and $alice eq 'TRUE');
    # --- --- ---
    # MBA region
    my $ev_lang_pack;
    my $ev_custpack_rom;
    ...
    {
        $ev_lang_pack         = &layout2_lang_ev($RegionInfo_ref->{LANG_PACK_ROM}->[0], $bb, "+0x00");
        $ev_custpack_rom      = &layout2_cust_ev($bb, "+0x00");
...
# Total layout
    ...
    $str_LAYOUT .= $viva_Layout if(((defined $dcmp and $dcmp eq 'TRUE') or (defined $alice and $alice eq 'TRUE')));	
...
sub layout15 #DCMP_ONLY
{
    ...
    &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_DCMP_MAX_COMPRESSED_SIZE need to be defined when DCM_COMPRESION_SUPPORT is turned on!", __FILE__, __LINE__)
    ...
    {
        &sysUtil::sysgen_die("[1.2]FlashSize(fs-offset) 
        isn't big enough to put in DCM Compression($nDCMCompressedSize)", __FILE__, __LINE__);
    }
    ...
    # --- --- ---
    #VIVA Layout
    my $lv_viva = &layoutVIVA("+0x400", &CommonUtil::Dec2Hex($viva_header_size));    
    ...
    if(defined $alice and $alice eq 'TRUE')
    {
        &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_ALICE_MAX_COMPRESSED_SIZE need to be defined when ALICE_SUPPORT is turned on!", __FILE__, __LINE__)
        ...
        {
            &sysUtil::sysgen_die("[1.2]$bb_folder\\custom_FeatureConfig.h: CONFIG_ALICE_MAX_COMPRESSED_SIZE cannot be configured larger than available ROM size! Please configure CONFIG_ALICE_MAX_COMPRESSED_SIZE within ".&CommonUtil::Dec2Hex($available_rom)."!", __FILE__, __LINE__);
        }      	  
        $lv_alice = &layoutALICE($bb,"+0x0");	
...
sub layout14
{
    ...
    $boot_cert_ev_base = (&FileInfo::is("alice_support", "TRUE") and !&isNAND($feature_query_function_ptr))?
...
sub layoutALICE
{
    ...
    my $ro_code;
    my $attr_str = (&sysUtil::is_MT6261_Family($bb))? "1024" : "128" ;
    if($Is_Flashtool_Layout == 1) #If not flash tool, still need the following layout.
    {
        return (undef);
    }
    
    # the wildcard is generated if dummy scatter is enabled
    if (!&FileInfo::is("zimage_support", "TRUE") and !&FileInfo::is("boot_zimage_support", "TRUE") and ($dummy_scatter eq 'TRUE') and !&isNAND($feature_query_function_ptr))
    {
        $ro_code = "        *                             (+RO-CODE)";
    }
    
    my $template = <<"__TEMPLATE";
ALICE $base_addr ALIGN $attr_str
{
    ALICE +0x0 OVERLAY
    {
        *                             (ALICE_FIRST, +First)
...
sub layoutVIVA
{
    my ($base_addr, $size) = @_;
    my $viva_gfh;
    my $viva_nand_comment = ";the base address of VIVA region will wrap to 0x0" if(&isNAND($feature_query_function_ptr));
    $base_addr =  "+0x0" if (&FileInfo::is("multiple_binary_files", "TRUE") and ($dummy_scatter eq 'TRUE') and &FileInfo::is_NOR());
    if(&sysUtil::is_sv5($g_bb) == 1)
    {
        $viva_gfh = <<"__VIVAGFH";
...
sub layout13
{
    ...
    my $isSecure = &FileInfo::is("secure_ro_enable", "TRUE") or &FileInfo::is("secure_support", "TRUE");
    my $available_size = $norflash_info_href->{'AVAILABLE_SIZE'};
    my ($HEADER_BLOCK_MIN_SIZE, $BL_MIN_SIZE, $EXT_BL_MIN_SIZE, $EXT_BL_BAK_MIN_SIZE);
    ...
    print "[layout13]bl_base=$bl_base_str,bl_size=$bl_size_str\n";
    ...
    my $root_cert_size = (&FileInfo::is("secure_support", "TRUE") and !($package_seg eq '16_3_SEG'))? (4 * 1024) : 0;
    my $root_cert_size_str = sprintf("0x%08x", $root_cert_size);
    print "[layout13]root_cert_base=$root_cert_base_str,root_cert_size=$root_cert_size_str\n"; 
    
    ### EXT_BOOTLOADER is behind ROOT_CERT and should be block-aligned if EXT_BL_UPDATE_SUPPORT is turned on
    my $ext_bl_base = (defined $ext_bl_update_support and $ext_bl_update_support eq 'TRUE') ? (&memConfig::Get_next_block_aligned_addr($root_cert_base,$root_cert_size,$available_size)) 
                                                                                            : ($root_cert_base + $root_cert_size);  # EXT_BL base without remap
    my $ext_bl_base_str = sprintf("0x%08x", $ext_bl_base + &config_query_flash_base_address($bb));
    my $ext_bl_size = (defined $ext_bl_update_support and $ext_bl_update_support eq 'TRUE') ? (&memConfig::Get_next_block_aligned_addr($ext_bl_base,$EXT_BL_MIN_SIZE,$available_size) - $ext_bl_base) 
                                                                                            : ($EXT_BL_MIN_SIZE);
    my $ext_bl_size_str = sprintf("0x%08x", $ext_bl_size);
    print "[layout13]ext_bl_base=$ext_bl_base_str,ext_bl_size=$ext_bl_size_str\n";
    ...
    {
        $ext_bl_bak_base = (&memConfig::Get_next_block_aligned_addr($ext_bl_base,$ext_bl_size,$available_size));# EXT_BL base without remap
        $ext_bl_bak_size = (&memConfig::Get_next_block_aligned_addr($ext_bl_bak_base, $EXT_BL_BAK_MIN_SIZE,$available_size) - $ext_bl_bak_base);
    }
    ...
    print "[layout13]ext_bl_bak_base=$ext_bl_bak_base_str,ext_bl_bak_size=$ext_bl_bak_size_str\n";
    ...
    my $cbr_base = $ext_bl_bak_size==0 ? &memConfig::Get_next_block_aligned_addr($ext_bl_base,$ext_bl_size,$available_size)
                                       : &memConfig::Get_next_block_aligned_addr($ext_bl_bak_base, $ext_bl_bak_size,$available_size);  # CBR base without remap
    my $nBaseAddress = $cbr_base;
    my $nblock_size = 0;
    my $cbr_size = 0;
    foreach (1..$cbr_blk)
    {
        $nblock_size = &memConfig::Get_flash_block_size($nBaseAddress,$available_size);
    ...
    print "[layout13]cbr_base=$cbr_base_str,cbr_size=$cbr_size_str\n";
...
__LAYOUT
    ...
    or (($bb eq 'MT6250' or $bb eq 'MT6260' or &sysUtil::is_MT6261_Family($bb)) and $isSmallpackage and !$isSecure))
    {
...
sub GetDummyEnd
{
    ...
    if(defined &Query_EXTSRAM_PAGETABLE_SupportBB($bb))
    {
        ...
        my $nRemainder = ($rs - $nOffset) % (16*1024);
        $nOffset += $nRemainder; 
...
sub layout12
{
    ...
    my $viva_header_size = &sysUtil::query_viva_size($bb);
    &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_ZIMAGE_MAX_COMPRESSED_SIZE need to be defined when ZIMAGE_SUPPORT is turned on!", __FILE__, __LINE__)
      if (!defined($feature_config_href->{'CONFIG_ZIMAGE_MAX_COMPRESSED_SIZE'}));
    my $comp_zimage = hex($feature_config_href->{'CONFIG_ZIMAGE_MAX_COMPRESSED_SIZE'});
    my $mba = &$feature_query_function_ptr('multiple_binary_files');
    # --- --- ---
    my $norflash_non_xip = &$feature_query_function_ptr('norflash_non_xip_support');
    my $isMauiInit = &$feature_query_function_ptr("dcm_compression_maui_init");
    my $fbase;
    my $bl_size;
    if($norflash_non_xip eq 'TRUE')
    {
        $fbase = sprintf("0x%08x", &ev_extsram_base($bb));
    }
    else
    {
        $fbase   = sprintf("0x%08x", &lv_rom_fbase($bb));
    }

    my $bl_size = &config_query_rom_offset();#query bootloader size
    my $viva_offset = (defined $mba and $mba eq 'TRUE') ? 0 : 0x400; #VIVA +0x400
    my $available_rom = $fs - &config_query_rom_offset() - $viva_offset - $viva_header_size;
    &WarnMinus(($available_rom - $comp_zimage), __FILE__, __LINE__, "$bb_folder\\custom_FeatureConfig.h: CONFIG_ZIMAGE_MAX_COMPRESSED_SIZE cannot be configured larger than available ROM size! Please configure CONFIG_ZIMAGE_MAX_COMPRESSED_SIZE within ".&CommonUtil::Dec2Hex($available_rom)."!");
    $available_rom -= $comp_zimage;
    # if secure support
    my $secure = &$feature_query_function_ptr('secure_support');
    ### Get bottom regions
    my $bottom_region_size = 0;
    ...
    {  
        ...
        &WarnMinus(($available_rom - $bottom_region_size), __FILE__, __LINE__, "FlashSize(fs-0x400-comp_size: ".&CommonUtil::Dec2Hex($available_rom).") isn't big enough to put in BOTTOM regions(".&CommonUtil::Dec2Hex($bottom_region_size).")");
        $available_rom -= $bottom_region_size;
    }
    ...
    {
        &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_BOOT_ZIMAGE_MAX_COMPRESSED_SIZE need to be defined when BOOT_ZIAMGE_SUPPORT is turned on!", __FILE__, __LINE__)
    ...
    ,  __FILE__, __LINE__,"flash can't put in VIVA regions:
    ZIMAGE region($comp_zimage)/DCM compressed region($nDCMCompressedSize)/ALICE region($comp_alice)/BOOT_ZIMAGE regions($comp_boot_zimage)");
    $available_rom -= $total_viva_regions;
    # VIVA size + compressed size of ZIMAGE + BOOT_ZIMAGE + DCMP + ALICE 
    my $viva_total_size = $viva_offset + $viva_header_size + $comp_zimage + $total_viva_regions;
 
    ### VIVA layout
    my $viva_base = ($norflash_non_xip ne 'TRUE')? "+0x400" : '+'.&CommonUtil::Dec2Hex($bl_size+0x10000400); #viva at 1Bank
    #print"viva_base=$viva_base\n";
    $viva_base = sprintf("0x%08x", &config_query_flash_base_address($bb) + $fs - $viva_total_size - $bottom_region_size) if(defined $mba and $mba eq 'TRUE');
    my $lv_viva = &layoutVIVA($viva_base, &CommonUtil::Dec2Hex($viva_header_size));
    ...
    my $decomp_total_size = $DCMP_TotalSize;
    #my $dcmp_ram_size = &CommonUtil::Dec2Hex($DCMP_TotalSize);

    ### ALICE layout
    my $lv_alice;
    $lv_alice = &layoutALICE($bb,"+0x0") if(defined $alice and $alice eq 'TRUE');
    ### MBA layout
    my $lv_mba;
    my $lv_jump_table;
    my $mba_decomp;
    if(defined $mba and $mba eq 'TRUE')
    {
        ($lv_mba, $mba_decomp) = &layoutMBA_COMPRESSION($bb, $cached_dummy_end_start_address_val);
        $decomp_total_size += $mba_decomp;
        ### JUMP TABLE
        my $jump_table_size = 0x280;
        my $ev_jump_table   = &layout2_jump_ev($bb);
        my $jsize   = &CommonUtil::Dec2Hex($jump_table_size);
        my $jbase   = ($dummy_scatter eq 'TRUE') ? "+0x0"
                                                 : sprintf("0x%08x", hex($viva_base) - $jump_table_size);
        $available_rom -= $jump_table_size;
        $lv_jump_table = <<"__LAYOUT";
...
__LAYOUT
    ...
    my $bottom_decomp_size = &CommonUtil::Dec2Hex($decomp_total_size);
    # Let cached_dummy_end get original start address(which depends on $DCMP_TotalSize & $mba_decomp)
    my $flength = &CommonUtil::Dec2Hex($available_rom);
    my $ev_rom_default = &layout0_rom_ev($bb, $fbase, $rs, $flength, $bottom_decomp_size);
    chomp $ev_rom_default;
    ...
    my $zimage_ev_base = (defined $boot_zimage and $boot_zimage eq 'TRUE')? "ImageLimit(BOOT_ZIMAGE_ER)" : "ImageLimit($previous_region)";
    my $zimage_ev_attribute = ((defined $boot_zimage and $boot_zimage eq 'TRUE') 
                               or (defined $mba and $mba eq 'TRUE')
                               or (defined $isMauiInit and $isMauiInit eq 'TRUE')
                               ) ? "OVERLAY" : "";
...
__ANY_RO
    ...
    # --- --- ---
    my $for_CAT;
    my $cat_in_zimage = &$feature_query_function_ptr('cat_in_zimage');
    $for_CAT = "        * (CAT_RODATA_IMG, CAT_RODATA_ADO, CAT_RODATA_STR)" if(defined $cat_in_zimage and $cat_in_zimage eq 'TRUE');
    # --- --- --- 
    my $layout_zimage;
...
ZIMAGE +0x0
{
    ZIMAGE_ER $zimage_ev_base $zimage_ev_attribute
    {
$for_CAT
$ev_rom_zimage
    }
...
#****************************************************************************
# subroutine:  MBA with compression Layout <Class>
# return:      the body strings
#****************************************************************************
sub layoutMBA_COMPRESSION
{
    my ($bb, $cached_dummy_end_start_address) = @_; 
    &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_LANG_PACK_ROM_DECOMPRESSED_SIZE need to be defined when MULTIPLE_BINARY_FILES is turned on!", __FILE__, __LINE__)
      if ((!defined($feature_config_href->{'CONFIG_LANG_PACK_ROM_DECOMPRESSED_SIZE'})) or (!defined($feature_config_href->{'CONFIG_LANG_PACK_ROM_MAX_COMPRESSED_SIZE'})));
    &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_CUSTPACK_ROM_DECOMPRESSED_SIZE need to be defined when MULTIPLE_BINARY_FILES is turned on!", __FILE__, __LINE__)
      if (!defined($feature_config_href->{'CONFIG_CUSTPACK_ROM_DECOMPRESSED_SIZE'}));
    ### check bin
    my $mba_bin = &$feature_query_function_ptr('MBA_DEFAULT_BINARY');
    ### check if need compress
    my $lang_pack_need_comp = $feature_config_href->{'LANG_PACK_ROM_COMPRESS'};
    my $custpack_need_comp = $feature_config_href->{'CUSTPACK_ROM_COMPRESS'};
    my $langpack_comp_size = $feature_config_href->{'CONFIG_LANG_PACK_ROM_MAX_COMPRESSED_SIZE'};
    my $lang_pack_decomp_size = 0;
    my $custpack_decomp_size = 0;
    my $mba_decomp = 0;
    my $lang_pack_base = "+0x0";
    my $custpack_base = "+0x0";
    my $lang_pack_attribute;
    my $custpack_attribute;

    if((defined $custpack_need_comp and $custpack_need_comp eq 1) and (!defined $mba_bin or $mba_bin ne 'LANG_ONLY'))
    {
        $custpack_decomp_size = hex($feature_config_href->{'CONFIG_CUSTPACK_ROM_DECOMPRESSED_SIZE'});
        $custpack_base = sprintf("0x%08x", ($cached_dummy_end_start_address - $custpack_decomp_size) | 0xF0000000);
        $cached_dummy_end_start_address -= $custpack_decomp_size;
        $mba_decomp += $custpack_decomp_size;
        $custpack_attribute = "OVERLAY";
    }
    if((defined $lang_pack_need_comp and $lang_pack_need_comp eq 1) and (!defined $mba_bin or $mba_bin ne 'CUST_ONLY'))
    {
        $lang_pack_decomp_size = hex($feature_config_href->{'CONFIG_LANG_PACK_ROM_DECOMPRESSED_SIZE'});
        $lang_pack_base = sprintf("0x%08x", ($cached_dummy_end_start_address - $lang_pack_decomp_size) | 0xF0000000 );
        $cached_dummy_end_start_address -= $lang_pack_decomp_size;
        $mba_decomp += $lang_pack_decomp_size;
        $lang_pack_attribute = "OVERLAY";
    }
    
    my $ev_lang_pack    = &layout2_lang_ev("+0x00", $bb, $lang_pack_base);
    my $ev_custpack_rom = &layout2_cust_ev($bb, $custpack_base);

    # mba default binary
    my $custpack_rom_base = ((!defined $mba_bin or $mba_bin ne 'CUST_ONLY') and ($lang_pack_need_comp == 1) and $Is_Flashtool_Layout != 1)?
    "ImageBase(LANG_PACK_ROM)+$langpack_comp_size" : "+0x0";
    my $lv_lang_pack;
    my $lv_cust_pack;
    my $lv_mba;
    ...
    $lv_lang_pack = <<"__LANGPACK";
LANG_PACK_ROM +0x0 $lang_pack_attribute
{
$ev_lang_pack
}
...
__LANGPACK
    ...
    $lv_cust_pack = <<"__CUSTPACK";
CUSTPACK_ROM $custpack_rom_base $custpack_attribute
{
$ev_custpack_rom
}
...
sub layout9
{
    ...
    if(defined $secure_ro_support and $secure_ro_support eq 'TRUE')
    {
        ...
        my $nflash_secure_ro_addr = hex($fbase) + hex($flength) - $nSECURE_RO_SIZE;
...
__TEMPLATE
    ...
    my $ev_rom_default        = &layout0_rom_ev($bb, $fbase, hex($rs), $flength, "0x0");
    chomp $ev_rom_default;
...
sub layout7
{
    ...
    if (&$feature_query_function_ptr('fota_update_package_on_nand') eq 'TRUE')
    {
    ...
    my $block_sz_update_package = &memConfig::Get_flash_block_size($fota_package_storage_base);
    ...
    {
          $tmp_block_sz = &memConfig::Get_flash_block_size($tmp_addr);
          &sysUtil::sysgen_die("[3.2]$bb_folder\\custom_img_config.h: All blocks in FOTA update package should have the same size! Please check FOTA_PACKAGE_STORAGE_BASE and FOTA_PACKAGE_STORAGE_SIZE settings in custom_img_config.h!", __FILE__, __LINE__) if ($tmp_block_sz != $block_sz_update_package);
    ...
    &WarnMinus(($available_rom - (&config_query_rom_offset() + hex($fota_cfg_href->{'MAX_LENGTH_ROM'}))), __FILE__, __LINE__, "FlashSize isn't big enough to put in BOTTOM regions($bottom_region_size).");
    # --- --- ---
...
sub layout5
{
    ...
    if(defined $alice and $alice eq 'TRUE')
    {
        &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: and CONFIG_ALICE_MAX_COMPRESSED_SIZE need to be defined when ALICE_SUPPORT is turned on!", __FILE__, __LINE__)
           if (!defined($feature_config_href->{'CONFIG_ALICE_MAX_COMPRESSED_SIZE'}));
        $comp_alice = hex($feature_config_href->{'CONFIG_ALICE_MAX_COMPRESSED_SIZE'});
    }    
    # --- --- ---
    ...
    my $jump_table_size = 0x280;
    my $jbase   = ((&FileInfo::is("alice_support", "TRUE") or &FileInfo::is("dcm_compression_support", "TRUE")) and ($dummy_scatter eq 'TRUE')) 
                ? "+0x0"
                : sprintf("0x%08x", &config_query_flash_base_address($bb) + $available_rom- $jump_table_size);
    # --- --- --- 
    # Lang Pack   address setup
    my $flash_lang_pack_addr = sprintf("+0x%x", &memConfig::Get_max_flash_block_size($fs,$norflash_info_href->{'AVAILABLE_SIZE'}));
    &WarnMinus( $available_rom - &config_query_rom_offset(), __FILE__, __LINE__, "Layout5(NOR::MBA+Secure)-FlashSize is insufficient");
    ...
    my $ev_lang_pack         = &layout2_lang_ev("+0x00", $bb, "+0x00");
    my $ev_custpack_rom      = &layout2_cust_ev($bb, "+0x00");
...
sub layout4
{
    ...
    my $ext_bl_base = (defined $ext_bl_update_support and $ext_bl_update_support eq 'TRUE') ? (&memConfig::Get_next_block_aligned_addr($bl_base, $bl_size,$available_size)) 
                                                                                            : ($bl_base + $bl_size);  # EXT_BL base without remap
...
sub layout3
{
    ...
    $query_address           = $fs - 1;
    $last_block_size         = &memConfig::Get_flash_block_size($query_address,$norflash_info_href->{'AVAILABLE_SIZE'});
...
# use builtin function and alignment
    my $alignment_config;
    if (&FileInfo::is("alice_support", "TRUE") and !&FileInfo::is("multiple_binary_files", "TRUE"))
    {
        $alignment_config = <<"__TEMPLATE";
#define ALIGNMENT_4KB 0x1000
#define SetAddress(address, alignment) ((address+(alignment-1)) AND ~(alignment-1))
...
__LAYOUT
    ...
    my $lv_secure;
    $lv_secure = &layoutSECURE_NAND($rs, $temp_ref) if($security eq 'TRUE');
    ...
    # --- --- ---
    my $ev_primary;
    if ($g_nfb_or_emb eq 'BASIC')
    {
        $ev_primary = &layout1_primaryrom_basic_ev($bb, $multibin, $security);
    }
    else
    {
        $ev_primary = &layout1_primaryrom_ev($bb, $multibin, $security);
    }
    my $ev_secondary = &layout1_secondaryrom_ev($bb,
                                                ...
                                                $security,
                                                $g_nfb_or_emb);
    my $ev_third     = &layout1_thirdrom_ev($bb) if($g_nfb_or_emb eq 'ENFB');
    ...
    {
        $ev_demand_paging = &layout1_demandpaging_ev($bb, $rs, $BBtbl_EXTSRAM_Shrinking_By_DP{$bb}, $g_nfb_or_emb, $dp_lv_size_string);
    }

    ### MBA region
    my $lv_mba;
    my ($lang_pack_size_string, $custpack_size_string);
    ($lv_mba, $lang_pack_size_string, $custpack_size_string) = &layoutMBA_NAND($dp_lv_start_string, $dp_lv_size_string) if(defined $multibin and $multibin eq 'TRUE');

    # DP and MBA's region can't exceed 0xffffffff
    my $nLimit =0;
    if(&isNAND($feature_query_function_ptr) and $g_nfb_or_emb ne 'BASIC')
    {
    ...
    ### VIVA+ALICE (viva region is generated if alice support)
    my ($lv_viva, $lv_alice);
    if (&FileInfo::is("alice_support", "TRUE"))
    {    
        # VIVA
        my $viva_header_size = &sysUtil::query_viva_size($bb);
        my $previous_region = &GetRegion_before_cached_dummy_end();
        my $viva_base = &FileInfo::is("multiple_binary_files", "TRUE") ? "+0x10000000 ALIGN 4096" : "SetAddress(ImageLimit($previous_region)+0x10000000, ALIGNMENT_4KB)";
        $lv_viva = &layoutVIVA($viva_base, &CommonUtil::Dec2Hex($viva_header_size));
        # ALICE
        $lv_alice = &layoutALICE($bb,"+0x20000000");
    }
    ### Determine ROM/SECONDARY_ROM size : to sync with feature_config.h and custom_img_config.h
    ### Unsupported: MT6260(ENFB) + FOTA
    ...
    my $str2ndROM_MaxSize = ($fota eq 'FOTA_DM')? &CommonUtil::Dec2Hex($n2ndROM_MaxSize) : "";
...
__LAYOUT
    ...
    # --- --- --- 
    if ($g_nfb_or_emb eq 'ONDEMAND')
    {
        return $layout_dp_nfb;
    }
    elsif ($g_nfb_or_emb eq 'BASIC')
    {
        if ($multibin eq 'TRUE' and ((&sysUtil::is_arm9($bb) == 1) or (&sysUtil::is_arm11($bb) == 1)))
        {
            &sysUtil::sysgen_die("[1.1]$themf: Unsupported Combination BASIC NFB + MULTIPLE_BINARY_FILES for arm9 above platform!", __FILE__, __LINE__);
        }
        elsif ($fota eq 'FOTA_DM' and $fast_boot eq 'FACTORY_ONLY')
        {
            &sysUtil::sysgen_die("[1.1]$themf: Unsupported Combination BASIC NFB + FOTA for factory bin!", __FILE__, __LINE__);
        }
        else
        {
            if ((&sysUtil::is_arm9($bb) == 1) or (&sysUtil::is_arm11($bb) == 1))
    ...
    elsif ($g_nfb_or_emb eq 'ENFB')
    {
        return $layout_enfb;
    }
    else
    {
        &sysUtil::sysgen_die("[1.1]$themf: Unsupported NAND_FLASH_BOOTING setting $g_nfb_or_emb in project makefile!", __FILE__, __LINE__);
    }
}

#****************************************************************************
# subroutine:  SECURE Layout <NAND>
# return:      the body strings
#****************************************************************************
sub layoutSECURE_NAND
{
    my ($rs, $secureInfo_href) = (@_);
    ...
    $flash_custpara_size_str = &CommonUtil::Dec2Hex( $secureInfo_href->{"CUST_PARA"}[0]);
    $flash_custpara_addr = &CommonUtil::Dec2Hex( $rs - $secureInfo_href->{"CUST_PARA"}[1]);

    $flash_toolcfg_size_str = &CommonUtil::Dec2Hex( $secureInfo_href->{"FLASH_TOOLCFG"}[0]);
    $flash_toolcfg_addr = &CommonUtil::Dec2Hex( $rs - $secureInfo_href->{"FLASH_TOOLCFG"}[1]);

    $flash_secure_ro_size_str = &CommonUtil::Dec2Hex( $secureInfo_href->{"SECURE_RO"}[0]);
    $flash_secure_ro_addr = &CommonUtil::Dec2Hex( $rs - $secureInfo_href->{"SECURE_RO"}[1]);
       
    $flash_secure_mac_size_str = &CommonUtil::Dec2Hex( $secureInfo_href->{"SECURE_MAC"}[0]);
    $flash_secure_mac_addr = &CommonUtil::Dec2Hex( $rs - $secureInfo_href->{"SECURE_MAC"}[1]);

    my $ev_hidden_secure_mac = ($fota eq 'FOTA_DM')? &layout3_fota_secure_mac_ev() : &layout3_secure_mac_ev();
...
#****************************************************************************
# subroutine:  MBA Layout <NAND>
# return:      the body strings
#****************************************************************************
sub layoutMBA_NAND
{
    my($dp_lv_start_string, $dp_lv_size_string) = (@_);
    my $layout_mba;
    my $mba_bin = &$feature_query_function_ptr('MBA_DEFAULT_BINARY');
    my $lv_lang_pack;
    my $lv_cust_pack;
    ...
    ### if NFB = BASIC or ENFB the MBA regions are tightly concatenated with the last cacheable region
    if($g_nfb_or_emb eq 'BASIC' or $g_nfb_or_emb eq 'ENFB')
    {
        my $previous_region = &GetRegion_before_cached_dummy_end();
        $lang_pack_start_string = "ImageLimit($previous_region)";
        $custpack_start_string = &FileInfo::is("mba_default_binary", "CUST_ONLY") ? "ImageLimit($previous_region)" : "+0x0";
    }

    my $ev_lang_pack          = &layout2_lang_ev($lang_pack_start_string, $g_bb, "+0x00");
    my $ev_custpack_rom       = &layout2_cust_ev($g_bb, "+0x00");
...
__CUSTPACK
        ...
        if(!defined $mba_bin or (defined $mba_bin and $mba_bin eq 'CUST_AND_LANG'))
        {
            $layout_mba = $lv_lang_pack.$lv_cust_pack;
        }
        elsif(defined $mba_bin and $mba_bin eq 'LANG_ONLY')
        {
            $layout_mba = $lv_lang_pack;	
        }
        elsif(defined $mba_bin and $mba_bin eq 'CUST_ONLY')
        {
            $layout_mba = $lv_cust_pack;
        }
        return ($layout_mba, $lang_pack_size_string, $custpack_size_string);
}
#****************************************************************************
...
sub layout0
{
    ...
    {   
        ...
        $comp_alice = $feature_config_href->{'CONFIG_ALICE_MAX_COMPRESSED_SIZE'};
        my $viva_header_size = &sysUtil::query_viva_size($bb);    
        $VIVA_rom = &layoutVIVA("+0x400", &CommonUtil::Dec2Hex($viva_header_size));
        $ALICE_rom = &layoutALICE($bb,"+0x0");
...
sub config_add_rom_offset
{
    ...
    {
    	$nBlockSize += &memConfig::Get_flash_block_size($g_config_rom_offset+$nBlockSize,$norflash_info_href->{'AVAILABLE_SIZE'});
    }
...
sub layout3_rom_ev
{
    ...
    my %BBtbl_RVCT =
       (     'MT6235'  => sub { &ROM_type_0_RVCT($bb, $fbAddr_string, "TRUE") .
             ...
             'MT6250'  => sub { &ROM_type_0_RVCT($bb, $fbAddr_string, "TRUE") . 
                                ...
                                &INTSRAM_type1($bb) .
                                &INTSRAM_DCM_EV($bb); },
             'MT6260'  => sub { &ROM_type_0_RVCT($bb, $fbAddr_string, "TRUE") . 
                                &EXTSRAM_type_7($bb, &ev_extsram_base($bb), $ram_size_value, "0x0") .
                                &INTSRAM_type1($bb) .
                                &INTSRAM_DCM_EV($bb); },         
             'MT6261_family'  => sub { &ROM_type_0_RVCT($bb, $fbAddr_string, "TRUE") . 
                                &EXTSRAM_type_7($bb, &ev_extsram_base($bb), $ram_size_value, "0x0") .
                                &INTSRAM_type1($bb) .
                                &INTSRAM_DCM_EV($bb); },    
       );
    ...
    if ($compiler eq 'RVCT')
    {
        $func = (&sysUtil::is_MT6261_Family($bb))? $BBtbl_RVCT{'MT6261_family'} : $BBtbl_RVCT{$bb};
    }
    return &$func;
}
    ...
    my %BBtbl_RVCT =
       (
             ...
             'MT6250'  => sub { &ROM_type_4_RVCT($bb, $fbAddr) .
                                ...
                                &INTSRAM_type1($bb) .
                                &INTSRAM_DCM_EV($bb); },
             'MT6260'  => sub { &ROM_type_4_RVCT($bb, $fbAddr) .
                                &EXTSRAM_type_7($bb, &ev_extsram_base($bb), $ram_size_value, "0x0") .
                                &INTSRAM_type1($bb) .
                                &INTSRAM_DCM_EV($bb); },        
             'MT6261_family'  => sub { &ROM_type_4_RVCT($bb, $fbAddr) .
                                &EXTSRAM_type_7($bb, &ev_extsram_base($bb), $ram_size_value, "0x0") .
                                &INTSRAM_type1($bb) .
                                &INTSRAM_DCM_EV($bb); },   
       );
       #map {print $_, "\n";} keys %BBtbl;
    &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination SECURE+USB-Download or SECURE+FOTA on $bb!", __FILE__, __LINE__) if not exists $BBtbl_RVCT{$bb}; 
    my $func;
    my $compiler = &$feature_query_function_ptr('compiler');
    if ($compiler eq 'RVCT')
    {
    ...
    my %BBtbl =
       (
             ...
             'MT6922'  => sub { &ROM_type_1($bb, $multibin, $security) .
                                ...
                                &INTSRAM_type1($bb) . 
                                &ROM_SIGNATURE_SECTION_EV($bb, "ROM", "MAUI");},
             'MT6260'  => sub { &ROM_type_1($bb, $multibin, $security) .
                                &EXTSRAM_type_8($bb) .
                                &INTSRAM_type1($bb) . 
                                &ROM_SIGNATURE_SECTION_EV($bb, "ROM", "MAUI");},
       );
       #map {print $_, "\n";} keys %BBtbl;
    &sysUtil::sysgen_die("[1.3]Unsupported Feature Combination NFB on $bb!", __FILE__, __LINE__) if not exists $BBtbl{$bb};
    my $func = $BBtbl{$bb};
    return &$func;
}
...
{
    my ($bb, $ram_size_value, $cached_dummy_end_addr, $multibin, $security, $nfb_or_emb) = @_;
    ...
    my %BBtbl =
       (
             ...
             'MT6922'  => sub { &ROM_type_5($bb, "+0x0", $security, $nfb_or_emb) .
                                ...
                                &FLMM_EV($bb) .
                                &ROM_SIGNATURE_SECTION_EV($bb, "SECONDARY_ROM", "S_MAUI") ;},
             'MT6260'  => sub { &ROM_type_5($bb, "+0x0", $security, $nfb_or_emb) .
                                &EXTSRAM_type_9($bb, $ram_base_above_arm9, $ram_size_value, $cached_dummy_end_addr, $multibin, $security) .
                                &INTSRAM_BLINUSED_EV($bb, "0x40000000", "0x00004000") .
                                &INTSRAM_FOTATEMP_EV($bb, "0x40006000") .
                                &INTSRAM_MED_EV($bb, "0x40000000", 1, "0x00007000") .
...
sub layout1_thirdrom_ev
{
    ...
    my $ExecutionRegions = &GetExecutionRegions("THIRD_ROM");
    if (&sysUtil::is_sv5($bb) == 1)
    {
      $sv5_thirdrom_gfh = <<"__TEMPLATE";
    THIRD_ROM_GFH +0x0
    {
         *                             (THIRD_ROM_GFH, +First)
    }
...
; IG
		 *intergrafx_sw.lib       (+RO-CODE, +RO-DATA)
		 *intergrafx_hw.lib       (+RO-CODE, +RO-DATA)	
...
__TEMPLATE
    ...
    my $browser_other_temp = <<"__TEMPLATE";
    	 ...
    	 *opera10adp.lib (+RO-CODE, +RO-DATA)
     	 ...
     	 *insignia_dm.lib (+RO-CODE, +RO-DATA)
     	 *insignia_dmsdk.lib (+RO-CODE, +RO-DATA)
__TEMPLATE
...
sub layout0_rom_ev
{
    ...
    my %BBtbl_RVCT =
       ...
       'MT6250'  => sub { &ROM_type_0_RVCT($bb, $bAdr, "FALSE"). &EXTSRAM_type_7($bb, &ev_extsram_base($bb), $rSZ, $decomp_zlength_str). &INTSRAM_type1($bb). &INTSRAM_DCM_EV($bb). &ROM_SIGNATURE_SECTION_EV($bb, "ROM", "MAUI");},
       'MT6260'  => sub { &ROM_type_0_RVCT($bb, $bAdr, "FALSE"). &EXTSRAM_type_7($bb, &ev_extsram_base($bb), $rSZ, $decomp_zlength_str). &INTSRAM_type1($bb). &INTSRAM_DCM_EV($bb). &ROM_SIGNATURE_SECTION_EV($bb, "ROM", "MAUI");},
       'MT6261_family'  => sub { &ROM_type_0_RVCT($bb, $bAdr, "FALSE"). &EXTSRAM_type_7($bb, &ev_extsram_base($bb), $rSZ, $decomp_zlength_str). &INTSRAM_type1($bb). &INTSRAM_DCM_EV($bb). &ROM_SIGNATURE_SECTION_EV($bb, "ROM", "MAUI");},
       );
       #map {print $_, "\n";} keys %BBtbl;
    #&sysUtil::sysgen_die("[1.3]Unsupported Feature Combination NOR booting on $bb!", __FILE__, __LINE__) if not exists $BBtbl_RVCT{$bb}; 
    my $func;
    my $compiler = &$feature_query_function_ptr('compiler');
    if ($compiler eq 'RVCT')
    {
...
sub layout2_lang_ev
{
    my ($lang_pack_rom_base, $bb, $lang_pack_content_base) = @_;
    &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: LANG_PACK_ROM_COMPRESS and CONFIG_LANG_PACK_ROM_DECOMPRESSED_SIZE need to be defined when MULTIPLE_BINARY_FILES is turned on!", __FILE__, __LINE__)
      if (!defined($feature_config_href->{'LANG_PACK_ROM_COMPRESS'}) or !defined($feature_config_href->{'CONFIG_LANG_PACK_ROM_DECOMPRESSED_SIZE'}));
    my $zimage = &$feature_query_function_ptr('zimage_support');
    my $need_comp = ($feature_config_href->{'LANG_PACK_ROM_COMPRESS'} and (defined $zimage and $zimage eq 'TRUE'));
    #$lang_pack_content_base = "+0x0" if (!$need_comp);
    my $lang_pack_content_size =  $feature_config_href->{'CONFIG_LANG_PACK_ROM_DECOMPRESSED_SIZE'} if($need_comp);
...
$sv5_lang_gfh
    ...
    LANG_PACK_ROM_CONTENT $lang_pack_content_base $lang_pack_content_size
    {
         *  (LANGPACK_RODATA)
         CustStrRes.obj (+RO-DATA)
         CustStrDataRes_*.obj (+RO-DATA)
         ...
         CustMenuRes.obj (+RO-DATA)
         CustMenuDataRes_*.obj (+RO-DATA)
         CustFontMapExt.obj (+RO-DATA)
         CustFontResExt.obj (+RO-DATA)
         vf_*.obj (+RO-DATA)
         CustStrSearchComp.obj (+RO-DATA)
         CustStrResComp.obj (+RO-DATA)
         CustStrMapComp.obj (+RO-DATA)
    }
__TEMPLATE
    return $template;
}
...
sub layout2_cust_ev
{
    my ($bb, $custpack_content_base) = @_;
    &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CUSTPACK_ROM_COMPRESS and CONFIG_CUSTPACK_ROM_DECOMPRESSED_SIZE need to be defined when MULTIPLE_BINARY_FILES is turned on!", __FILE__, __LINE__)
      if (!defined($feature_config_href->{'CUSTPACK_ROM_COMPRESS'}) or !defined($feature_config_href->{'CONFIG_CUSTPACK_ROM_DECOMPRESSED_SIZE'}));
    my $zimage = &$feature_query_function_ptr('zimage_support');
    my $need_comp = ($feature_config_href->{'CUSTPACK_ROM_COMPRESS'} and (defined $zimage and $zimage eq 'TRUE'));
    #$custpack_content_base = "+0x0" if (!$need_comp);
    my $custpack_content_size =  $feature_config_href->{'CONFIG_CUSTPACK_ROM_DECOMPRESSED_SIZE'} if($need_comp);
...
__TEMPLATE
    ...
    if($mmi_version eq 'PLUTO_MMI' or $mmi_version eq 'COSMOS_MMI')
    {
        $Cust_Theme = <<"__TEMPLATE";
         CustVenusThemeData.obj (+RO-DATA)
__TEMPLATE
        chomp $Cust_Theme;
    }
    my $template = <<"__TEMPLATE";
$sv5_cust_gfh
    ...
    CUSTPACK_ROM_CONTENT $custpack_content_base $custpack_content_size
    {
         *  (CUSTPACK_RODATA)¡@
         ThemeRes.obj (RESOURCE_1ST_CONTENT)
         ThemeRes.obj (+RO-DATA)
         nvram_cust_pack.obj (+RO-DATA)
         CustImgResExt.obj (+RO-DATA)
         CustAdoResExt.obj (+RO-DATA)
         custpack_certs.obj (+RO-DATA)
         custpack_java_certs.obj (+RO-DATA)
$Cust_Theme
    }
__TEMPLATE
    return $template;
...
$sv5_jump_gfh
    JUMP_TABLE +0x00
    {
         custom_jump_tbl.obj (+RO-DATA, +RW)
    }
__TEMPLATE
    return $template;
}
...
sub ROM_type_4_RVCT
{
    ...
    my $rom_size_string   = sprintf("0x%08x", 0x800);
    my $rom0_begin_string = sprintf("0x%08x", hex($rom_size_string));
    # --- --- --- 
...
sub ROM_type_5
{
    ...
    if ($compiler eq 'RVCT')
    {
        ...
        if (($nfb_or_emb ne 'ONDEMAND') or (!defined $demand_paging_version) or (defined $demand_paging_version and $demand_paging_version ne 'DPV2'))
        {
            ...
            if(&FileInfo::is("alice_support", "TRUE"))
            {
                $rom2_rodata_temp = ($dummy_scatter ne 'TRUE') ? &Get_ObjListGen_List('objectROM2_RODATA.list') : &Get_ObjListGen_List('objectROM2_RODATA_dummy.list');
                $rom2_rodata_temp .= "* (CAT_RODATA_IMG, CAT_RODATA_ADO, CAT_RODATA_STR)";
            }
            else
            {
                $rom2_rodata_temp  = "* (+RO-DATA)\n";
                $rom2_rodata_temp .= "* (CAT_RODATA_IMG, CAT_RODATA_ADO, CAT_RODATA_STR)";
            }
            my @rom2_rodatalibs = split("\n", $rom2_rodata_temp);
            map { s/^\s+//; } @rom2_rodatalibs;
            $ev_string .= "    ; Add ROM2_RODATA to use 2-MAUI RO-DATA to push 2-MAUI RO-CODE away from 1-MAUI RO-CODE to prevent L6478E\n";
            $ev_string .= &format_execution_view( "ROM2_RODATA",
                                                  "+0x0 ABSOLUTE",
                                                  undef,
...
sub EXTSRAM_type_6
{
    ...
    my $mode = &$feature_query_function_ptr('MODE');
    my ($region_size_tx, $region_size_rx) = &sysUtil::DSPTXRX_query_length($bb, $mode);
    $ev_string .= &DSPTXRX( $bb, "EXTSRAM", 0x0, $ram_size, $region_size_tx, $region_size_rx );
    $ev_string .= &shouldbe_emptysection();
    return $ev_string;
}
...
sub EXTSRAM_type_7
{
    ...
    if ($norflash_non_xip eq 'TRUE')
    {
    	$ev_baseaddr = "+0x0"; #append to ROM region
    }
    else
    {
    	$ev_baseaddr = sprintf("0x%08x", ($ram_base == 0x0) ? 0x200 : $ram_base);
    }
    ...
    &sysUtil::sysgen_die("[1.1]Unsupported combination: feature phone with DSP=DUALMACDSP in NOR\n", __FILE__, __LINE__) if((!&sysUtil::is_SmartPhone($bb)) and $is_DUALMACDSP); # FCORE has aligned requirement in feature phone.
    ...
    if (defined $ev_string)
    {
        if ($norflash_non_xip eq 'TRUE')
        {
            #0x1e00 = 0x2000-0x200, length of SINGLE_BANK_CODE
            #EXTSRAM begin address should be 4k alignment requested by MMU to set region attribute
            $ev_baseaddr = $Is_Flashtool_Layout != 1 ? "SetAddress(ImageBase(SINGLE_BANK_CODE)+0x1e00,ALIGNMENT_4KB)" : "+0x0";
            my $mode = &$feature_query_function_ptr('MODE');
            $ev_baseaddr = $Is_Flashtool_Layout != 1 ? "SetAddress(ImageBase(SINGLE_BANK_CODE)+0x3e00,ALIGNMENT_4KB)" : "+0x0" if($mode eq "BASIC"); # because basic load has too many debug information
        }
        else
        {
            $ev_baseaddr = sprintf("0x%08x", $ram_base+0x2000);
            my $mode = &$feature_query_function_ptr('MODE');
            $ev_baseaddr = sprintf("0x%08x", $ram_base+0x4000) if($mode eq "BASIC"); # because basic load has too many debug information
        }
    }
    #ALICE is not enabled when enable norflash_non_xip_support
    if($alice eq 'TRUE')
    {
    ...
    if ((&sysUtil::is_arm9($bb) == 1) or (&sysUtil::is_arm11($bb) == 1))
    {
        $ev_string .= &format_execution_view( "PAGE_TABLE",
                                              (&config_query_remapping() or &sysUtil::is_SmartPhone($bb)) ? "+0xF0001000" : "+0xE0001000",
                                              undef,
                                              ["* (PAGETABLE)"]
                                            );
    ...
    my $ram_disk_size = ((&sysUtil::is_SmartPhone($bb) == 1) and ($nand_support ne 'TRUE')) ? 0x300000 : 0;
    ...
    $cached_dummy_end_start_address_val -= $cached_dummy_end_skip_for_zimage;
    $cached_dummy_end_start_address = sprintf("0x%08x", ($cached_dummy_end_start_address_val | 0xF0000000));

    #reserve memory for SWLA when ZIMAGE_CONTENT_POLICY = BEST_PERF
    my $swla_res_space = &$feature_query_function_ptr('SWLA_RESERVE_SPACE');
    my $zimage_policy = &$feature_query_function_ptr('ZIMAGE_CONTENT_POLICY');
    my ($swla_res_begin_addr,$swla_res_size) = (0,0);
    if((defined $zimage_policy and $zimage_policy eq 'BEST_PERF')
       and (defined $swla_res_space and (($swla_res_space eq 'YES') or ($swla_res_space eq 'YES_WO_ADDON'))))
    {
        $swla_res_size = 12*1024;#reserve 12kB size for minimum SWLA working space
        $swla_res_begin_addr = $cached_dummy_end_start_address_val - $swla_res_size;
        $cached_dummy_end_start_address_val -= $swla_res_size;
        $cached_dummy_end_start_address = sprintf("0x%08x", ($cached_dummy_end_start_address_val | 0xF0000000));
    }
    ...
    if((defined $zimage_policy and $zimage_policy eq 'BEST_PERF')
       and (defined $swla_res_space and (($swla_res_space eq 'YES') or ($swla_res_space eq 'YES_WO_ADDON'))))
    {
        my ($str_swla_res_size,$str_swla_res_begin_addr) = ('','');
        $str_swla_res_begin_addr = sprintf("0x%08x",($swla_res_begin_addr | 0xF0000000));
        $str_swla_res_size = sprintf("0x%08x",$swla_res_size);
        $ev_string .= &format_execution_view( "SWLA_RES_MEM",
                                          $str_swla_res_begin_addr,
                                          $str_swla_res_size,
                                          ['']
                                          );
    }
...
sub EXTSRAM_type_9
{
    ...
    &sysUtil::sysgen_die("[1.1]Unsupported combination: nonCOSMOS+VIDEO_STANDALONE_MEMORY=TRUE\n", __FILE__, __LINE__) if ($mmi_version ne 'COSMOS_MMI' and $is_video_standalone);
    ...
    {
        &sysUtil::sysgen_die("[1.1]DYNAMIC_CACHEABLE_EXTSRAM_DEFAULT_CACHEABLE_ZI_VIDEOPOOL is unsupported under non-DUALMACDSP platform", __FILE__, __LINE__) if($is_video_standalone);
        # --- --- --- --- --- --- --- --- --- --- --- --- ---
    ...
    $g_region_before_cached_dummy_end = "DYNAMIC_CACHEABLE_EXTSRAM_DEFAULT_CACHEABLE_RW";
    # --- --- --- --- ---
	my $extsram_base_value = 0;
...
__SBNORFLASHTEMPLATE
    ...
    my %BBtbl =
       (        'MT6235'  => $sbcode_tempD,
                ...
                'MT6250'  => $sbcode_tempA, 
                'MT6260'  => $sbcode_tempA,    
                'MT6261_family'  => $sbcode_tempA,   
       );
       #map {print $_, "\n";} keys %BBtbl;
    #&sysUtil::sysgen_die("[1.1]Unsupported Feature Combination ESB on $bb!", __FILE__, __LINE__) if not exists $BBtbl{$bb}; 
    # --- --- --- 
    # +First but not present will make basic and l1s load build failure
    my $mode = &$feature_query_function_ptr('MODE');
    my $sbcode_str = (&sysUtil::is_MT6261_Family($bb))? $BBtbl{'MT6261_family'} : $BBtbl{$bb} ;
    if (defined $mode)
    {
        if ($mode eq "BASIC" or $mode eq "L1S")
        {
           $sbcode_str =~ s/,\+First\)/\)/g;
        }
    }
    # --- --- --- 
    return $sbcode_str;
...
__LARGEPOOLTEMPLATE
    ...
    my %BBtbl =
       ...
       #map {print $_, "\n";} keys %BBtbl;
    &sysUtil::sysgen_die("[2.0]unsupported $bb!", __FILE__, __LINE__) if not exists $BBtbl{$bb}; 
    # --- --- --- 
    # +First but not present will make basic and l1s load build failure
    my $mode = &$feature_query_function_ptr('MODE');
    if (defined $mode)
...
; specific objects in che.lib
        md5_engine.obj                (+RO)
; for removing the following object from sys_sec.lib to sys_int.lib
        third_rom_int.obj             (+RO)
...
sub app_feature_rom2
{
    ...
    if (&FileInfo::is("alice_support", "TRUE"))
    {
        $rom2_temp .= ($dummy_scatter ne 'TRUE') ? &Get_ObjListGen_List('objectROM2.list') : &Get_ObjListGen_List('objectROM2_dummy.list');
    }
    elsif (($nfb_or_emb eq 'ONDEMAND') and (defined $demand_paging_version) and ($demand_paging_version eq 'DPV2'))
    {#exception list
...
;L1_TDD128
         *uas.lib (+RO)
         *ast_l1_ast2001.lib (+RO)
         *ast_l1_ast3001.lib (+RO)
         *fta.lib (+RO)
__TEMPLATE
...
__TEMPLATE
    ...
    else  # no ONDEMAND or ONDEMAND+DPV1
    {
    	$rom2_temp .= <<"__TEMPLATE";
         * (+RO-CODE)
__TEMPLATE
...
sub DSP_MT6256_DSPROM
{
	...
	{
		&sysUtil::sysgen_die("[1.1]DSP_MT6256 can't exist with original DSP regions", __FILE__, __LINE__); 
	}
	my $template;
	if( $bb eq "MT6256")
	{
...
sub DSPTXRX
{
    ...
    my $ev_addr_tx = sprintf("0x%08x", ($baseaddr + $ram_size - $region_size_tx - $region_size_rx));
    my $ev_addr_rx = sprintf("0x%08x", ($baseaddr + $ram_size - $region_size_rx));
...
; DSP execution regions cannot be OVERLAY; its size should be specified as 2^N, larger than 4KB, and should be 32K aligned with its start and END address
    ${prefix_name}_DSP_TX $ev_addr_tx ABSOLUTE $ev_len_tx
    {
         *.obj (EXTRAM_TXDATA)
    }
    ${prefix_name}_DSP_RX $ev_addr_rx $ev_len_rx
    {
         *.obj (EXTRAM_RXDATA)
    }
__DSPTEMPLATE
    ...
    my %BBtbl_DSPTXRX =
       ...
       #map {print $_, "\n";} keys %BBtbl;
    my $dsptxrx_str = (&sysUtil::is_MT6261_Family($bb))? $BBtbl_DSPTXRX{'MT6261_family'} 
                                                       : $BBtbl_DSPTXRX{$bb};
    if (defined $dsptxrx_str and $dsptxrx_str)
    {
        return $ev_dsp;
    }
}
...
#****************************************************************************
# subroutine:  Default Layout :: ROM :: Exec View :: INTSRAM type 3
# input:       BB
# remark:      INTSRAM only
#****************************************************************************
sub INTSRAM_type3
...
__TEMPLATE_6575
    # --- --- --- 
    my $template_6253EL = <<"__TEMPLATE_6253EL";
...
; Please do NOT remove any content above!
    ...
    INTSRAM_DUMMY_END 0x4000CC00
    {
         bootarm.obj (INTSRAM_END)
    }
__TEMPLATE_6253EL
    my $template_6250 = <<"__TEMPLATE_6250";
...
__TEMPLATE_6250
    my $template_6260 = <<"__TEMPLATE_6260";
    INTSRAM_DATA_PREINIT +0x0 0x0C00
    {
; data passed from bootloader to maui
         * (PREINIT_INTSRAM_RODATA, PREINIT_INTSRAM_RW, PREINIT_INTSRAM_ZI)
...
__TEMPLATE_6260
    my $template_6261 = <<"__TEMPLATE_6261";
...
__TEMPLATE_6261
    ...
    &sysUtil::sysgen_die("[1.3/2.0]Unsupported $bb!", __FILE__, __LINE__) if(!defined $strCODE_Base or !defined $strCODE_Size or !defined $strDATA_Base or !defined $strDATA_Size ); 
    my $template_INTERSARM_HW_BOUNDARY = undef;
    if(&sysUtil::is_CR4($bb) == 1)
    {
        my ($strHW_Base, $strHW_Size) = sysUtil::GetIntsramHWBoundary($bb);
        &sysUtil::sysgen_die("[1.3/2.0]Unsupported $bb - intsram HW boundary!", __FILE__, __LINE__) if(!defined $strHW_Base or !defined $strHW_Size); 
...
_TEMPLATE_INTSRAM
    # --- --- --- --- --- --- --- --- --- --- --- 
    my %BBtbl =
       (        'MT6516'  => sub { $template_INTERSARM ;},
                'MT6235'  => sub { $template_INTERSARM ;},
                'MT6235B' => sub { $template_INTERSARM ;},
                'MT6236'  => sub { $template_INTERSARM. $template_6236 ;},
                'MT6236B' => sub { $template_INTERSARM. $template_6236 ;},
                'MT6921'  => sub { $template_INTERSARM. $template_6236 ;},
                'MT6268'  => sub { $template_INTERSARM ;},
                'MT6253'  => sub { $template_INTERSARM. $template_6253 ;},
                'MT6276'  => sub { $template_INTERSARM. $template_6276 ;},
                'MT6253E' => sub { $template_INTERSARM. $template_6253EL ;},
                'MT6253L' => sub { $template_INTERSARM. $template_6253EL ;},
                'MT6252H' => sub { $template_INTERSARM. $template_6253EL ;},
                'MT6252'  => sub { $template_INTERSARM. $template_6253EL ;},
                'MT6250'  => sub { $template_INTERSARM. $template_6250 ;},
                'MT6260'  => sub { $template_INTERSARM. $template_6260 ;}, 
                'MT6261_family'  => sub { $template_INTERSARM. $template_6261 ;},
                'MT6573'  => sub { $template_INTERSARM. $template_6573 ;},
                'MT6575'  => sub { $template_INTERSARM. $template_6575 ;},
                'MT6256'  => sub { $template_INTERSARM ;},
                'MT6255'  => sub { $template_INTERSARM ;},
                'MT6922'  => sub { $template_INTERSARM ;},
                'TK6280'  => sub { $template_INTERSARM ;},
       );
    #map {my $func = $BBtbl{$_}; print &$func . "\n";} keys %BBtbl;
    $bb = 'MT6261_family' if(&sysUtil::is_MT6261_Family($bb));
    &sysUtil::sysgen_die("[1.3/2.0]Unsupported $bb!", __FILE__, __LINE__) if not exists $BBtbl{$bb}; 
    my $func = $BBtbl{$bb};
    return &$func;
}
...
__TEMPLATE
    ...
    if (defined $BBtbl_extra_smallcfg{$bb} and $BBtbl_extra_smallcfg{$bb})
    {
       ...
       if ($bb eq 'MT6250' || $bb eq 'MT6260' || $bb eq 'MT6261_family')
       {
...
__TEMPLATE
   # --- --- --- 
    ...
    if (defined $BBtbl_extra_smallcfg{$bb} and $BBtbl_extra_smallcfg{$bb})
    {
       if ($bb eq 'MT6252' or $bb eq 'MT6252H')
       {
           $template = $template_6252_sys . $template . $template_small_nucleus;
       }
       elsif ($bb eq 'MT6250' || $bb eq 'MT6260' || $bb eq 'MT6261_family')
       {
...
sub INTSRAM_BLINUSED_EV
{
    ...
    # --- --- --- 
    return undef if(&sysUtil::is_noSYSRAM($bb) == 1);
...
sub INTSRAM_FOTATEMP_EV
{
    ...
    if ($fota ne 'FOTA_DM' or &sysUtil::is_noSYSRAM($bb) == 1)
    {
       return;
    }
...
#    &sysUtil::sysgen_die("tools\\scatGenLib.pl: unsupported $bb!", __FILE__, __LINE__) if not exists $BBtbl{$bb}; 
    ...
    if (((&sysUtil::is_arm9($bb) == 1) or (&sysUtil::is_arm11($bb) == 1)) && (&$feature_query_function_ptr('nand_flash_booting') eq 'BASIC'))
    {
        $result .= &format_execution_view( "DYMANIC_CODE_DUMMY_END", 
                                           "+0x0 ABSOLUTE",
                                           undef,
                                           ["bootarm.obj (DUMMY_DYNAMIC_CODE)"]
                                         );
    }
    return $result;
}
...
{
    my ($strBaseAddr, $bb) = @_;
    $strBaseAddr = '+0x0' if(!defined $strBaseAddr or (&FileInfo::is("multiple_binary_files", "TRUE") and ($dummy_scatter eq 'TRUE')));
    &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_DCMP_MAX_COMPRESSED_SIZE need to be defined when DCM_COMPRESION_SUPPORT is turned on!", __FILE__, __LINE__)
       if (!defined($feature_config_href->{'CONFIG_DCMP_MAX_COMPRESSED_SIZE'}));
...
sub DCMP_code_layout
{
    ...
    {
        &sysUtil::sysgen_die("[2.0]DCM Compression should be 32B aligned!", __FILE__, __LINE__);
    }
    my $strRamAddress = &CommonUtil::Dec2Hex($nRamBottomAddress);
    my $nMMUPrefix = &sysUtil::is_mmu($bb) ? 0xf0000000 : 0;
    $nRamBottomAddress |= $nMMUPrefix;
    # --- --- --- --- combination --- --- --- --- 
    scatUtil::ParseDCMP($bb_folder);
    my $ExistCombination = scatUtil::DCMP_GetCombination();
    
    my %DCM_COMP_CODE_table; #postfix => [nBaseAddr, nMaxSize]
    my $nTotalDecompressedSize = scatUtil::DCMP_GetQueryInfo($nRamBottomAddress, $ExistCombination, \%DCM_COMP_CODE_table);
    
    # In Dummy scatter, all DCM regions are OVERLAY (with MaxSize 0x2000)
    $nTotalDecompressedSize = 0x2000 if ($dummy_scatter eq 'TRUE');
   
    ##map {print "DCMP_code_layout:$_", "\n";} keys %DCM_COMP_CODE_table;
    my $layout;
    foreach my $strPostfix (@$ExistCombination)
    {
        my ($nBaseAddr, $nMaxSize) = ($DCM_COMP_CODE_table{$strPostfix}->[0], $DCM_COMP_CODE_table{$strPostfix}->[1]);
        my $strInputSection = &DCMP_InputSection_template($strPostfix);
        if(&sysUtil::DCMP_is_MMorIME_head($strPostfix))
        {   #the layout of MM/IME DCM regions are different with general DCM regions
            $nMaxSize = scatUtil::DCMP_GetRegionSize($strPostfix);
        }
        $layout .= &DCMP_code_template($strPostfix, $nBaseAddr, $nMaxSize, $strInputSection, $nRamBottomAddress);
        my $Info_aref = scatUtil::DCMP_Get_MM_IME_subRegionInfo($strPostfix, $nBaseAddr, $nMaxSize);
        foreach my $item(@$Info_aref)
        {                                #($strPostfix, $nBaseAddr, $nMaxSize, $InputSection)
            my $strInputSection = DCMP_InputSection_template($item->[0]);
            $layout .= &DCMP_code_template($item->[0], $item->[1], $item->[2], $strInputSection, $nRamBottomAddress);
        }
    }

    #for special DCM region layout
    $layout .= &scatUtil::DCMP_MAUIINIT_Layout($bb);
...
{
    my ($strPostfix, $nBaseAddr, $nMaxSize, $InputSection, $nRamBottomAddress) = @_;
    my $strHeadSize = '0x40';
    ...
    if ($dummy_scatter eq 'TRUE')
    {
        $strBody_MaxSize = ""; 
        # In Dummy scatter, all DCM regions are OVERLAY (with MaxSize 0x2000)
        $strBaseAddr = &CommonUtil::Dec2Hex($nRamBottomAddress - 0x2000);   
    }

# Load Region 32B aligned for not to generate paddings between +0x0 and execution region's fixed base address.
    my $template = <<"__TEMPLATE";
DYNAMIC_COMP_$strPostfix +0x0 ALIGN 32
{
; 32byte aligned to support dynamically switchable cachability 
    DYNAMIC_COMP_$strPostfix $strBaseAddr ALIGN 32 OVERLAY $strHeadSize
    {
...
__TEMPLATE
    my $templateMP4DEC=<<"__TEMPLATE";
       ...
       * (DYNAMIC_CODE_MP4DECSP_ROCODE, DYNAMIC_CODE_MP4DECSP_RODATA)
__TEMPLATE
...
__TEMPLATE
    my $templateMP4ENC=<<"__TEMPLATE";
       ...
       * (DYNAMIC_CODE_MP4ENC_ROCODE, DYNAMIC_CODE_MP4ENC_RODATA)
__TEMPLATE
...
__TEMPLATE
    my $templateWAVETABLE=<<"__TEMPLATE";
       ...
       *melody_syn.a (+RO-CODE, +RO-DATA)
__TEMPLATE
    my $templateVCH=<<"__TEMPLATE";
       *BesTS.a (+RO-CODE, +RO-DATA)
       * (DYNAMIC_CODE_VCH_ROCODE, DYNAMIC_CODE_VCH_RODATA)
__TEMPLATE
    ...
    my $template;
    if(exists $DCM_COMP_CODE_INPUTSECTION_table{$strPostfix})
    {
        $template = $DCM_COMP_CODE_INPUTSECTION_table{$strPostfix};
        chomp($template);
    }
    else
    {
        $template = "         * (DYNAMIC_CODE_$strPostfix\_ROCODE, DYNAMIC_CODE_$strPostfix\_RODATA)";
    }
    return $template;
}
...
sub Get_ObjListGen_List
{
    ...
    {
        &sysUtil::sysgen_die("[3.2]$OBJ_LIST: list not exist!", __FILE__, __LINE__);
    }
    return $content;
}
...
sub GetList
{
    ...
    open (FILE, "<$strListPath") or &sysUtil::sysgen_die("[3.2]$strListPath: open list error!", __FILE__, __LINE__);
    {
        local $/;
        $content .= <FILE>;
    }
...
sub GetROM1_inputsection
{
    ...
    if(&FileInfo::is("zimage_support", "TRUE"))
    {
        ...
        my $for_CAT = undef;
        my $cat_in_zimage = &$feature_query_function_ptr('cat_in_zimage');
        $for_CAT = "         *    (CAT_RODATA_IMG, CAT_RODATA_ADO, CAT_RODATA_STR)" if(!defined $cat_in_zimage or (defined $cat_in_zimage and $cat_in_zimage ne 'TRUE'));
        # --- --- ---
       $obj_list .= <<"__TEMPLATE";
$for_CAT
...
{
    my ($RegionName) = @_;
    my $strCSVPath = &scatFrame::GetConfigInput($RegionName, "Common.csv", scatFrame::InputSections, $bb_folder);
    PrintDependency($strCSVPath);
...
#****************************************************************************
# subroutine:  GetExecutionRegions
# input:       $strLoadRegionName: which Load region Execution region belongs to 
# output:      Execution region layout (include input sections)
#****************************************************************************
sub GetExecutionRegions
{
    my ($strLoadRegionName) = @_;
    my $strContent;
    my $strCSVPath = &scatFrame::GetConfigInput(undef, "RegionList.csv", scatFrame::Regions, $bb_folder);
    PrintDependency($strCSVPath);
    my ($BasicExecutionRegionList_aref, $Index_href) = &CommonUtil::ParseCSV($strCSVPath, undef);
    my @ExecutionRegionList;
    my %InputSections;
    foreach my $item (@$BasicExecutionRegionList_aref)
    {
        my $LoadRegionName = $item->[$Index_href->{LoadRegion}];
        my $strBase = $item->[$Index_href->{Base}];
        my $strOffset = $item->[$Index_href->{Offset}];
        my $strCondition = $item->[$Index_href->{FeatureOption}];
        #Check validity of csv file
        &sysUtil::sysgen_die("[2.0]LoadRegion field shoud be defined in $strCSVPath", __FILE__, __LINE__) if($LoadRegionName eq "");
        &sysUtil::sysgen_die("[2.0]Offset and Base fields both should not be filled or empty in $strCSVPath", __FILE__, __LINE__) 
        if((($strBase ne "") and ($strOffset ne "")) or (($strBase eq "") and ($strOffset eq "")));

        #Only the execution regions which belongs to strLoadRegionName need to be generated
        next if($strLoadRegionName ne $LoadRegionName);
        next if($strCondition ne "" and (0 == &FileInfo::EvaluateFeatureOptionCondition($strCondition, $g_MakeFile_ref)));
        push @ExecutionRegionList, $item;

        my $ExecutionRegionName = $item->[$Index_href->{RegionName}];
        my $strInputSection = &GetInputSections($ExecutionRegionName);
        $InputSections{$ExecutionRegionName} =  $strInputSection;
    }
    
    if(scalar(@$BasicExecutionRegionList_aref) != 0)
    {
        $strContent = ";$strCSVPath\n";
        $strContent .= &scatFrame::GenExecutionRegions(\@ExecutionRegionList, $Index_href, \%InputSections);
    }
    return $strContent;
}
...
{
    &sysUtil::sysgen_die("[2.0]the layout of extsram has not been generated", __FILE__, __LINE__) 
...
sub WarnMinus
{
    ...
    &sysUtil::sysgen_die("[1.2]The Size($nNumber) is not correct. $error_msg", $file, $line_no) if($nNumber < 0);
}
