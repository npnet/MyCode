...
{
    my @usage = ();
    push @usage, "\nUsage 1: 'mmi_feature_check.pl <project name> <compiler> <via option> <feature filename> <switch filename> <checkCfg : TRUE, FALSE> <returnErrorOrNot> <MMI or DRV> <MMI_DEF_DEFS make file> <output path>'\n\n";
    push @usage, "Usage 2: 'mmi_feature_check.pl <project name> <compiler> <via option> <filename> <filename> <checkCfg : TRUE, FALSE> <returnErrorOrNot> <MMI or DRV> <MMI_DEF_DEFS make file> <output path>'\n\n";
    push @usage, "e.g. 'mmi_feature_check.pl DRAGONFLY_DEMO armcc --via plutommi\\mmi\\inc\\mmi_features.h plutommi\\mmi\\inc\\mmi_features.h FALSE 0'\n\n";
    die join('', @usage);
}
else
{
...
if ($logPath eq "")
{
    ...
    {
        $logPath = "$g_mcuPath\\build\\$prjName\\log";
...
sub getMacroPreprocessResult()
{
    ...
    my $checkMainFilename = "~".splitFilename($targetFilename, 0)."_check";    
...
sub processCheckDotC()
{
    ...
    my $infoFilename = "$g_mcuPath\\build\\$prjName\\log\\info.log";
    ...
    {
        my @incFiles = ();
        if (&getFileList("$g_mcuPath\\make\\$mod\\*.inc /s", \@incFiles) > 0)
        {
    ...
    {
        my @defFiles = ();
        if (&getFileList("$g_mcuPath\\make\\$mod\\*.def /s", \@defFiles) > 0)
        {
    ...
    print hFile "\@echo on\n";
    close(hFile);
    exit 1 if (system("makeCheck.bat"));
    
    #get compile error log from mmi_features.h or mmi_features_switch.h
    if ($switch =~ /MMI/ig)
    {
        &checkMMIFeatureCompileError($defTmpFilename, $incTmpFilename);
    }
...
#  none
#******************************************************************************
sub checkMMIFeatureCompileError()
{
    my $defTmpFilename;
    my $incTmpFilename;
    my $mmi_feature_compile_log;
    ($defTmpFilename, $incTmpFilename) = @_;
    $mmi_feature_compile_log = "$logPath\\"."mmi_feature_compile.log";
    
    #create temp c file for test
    open(cTest, ">mmi_feature_compile_test.c") or die "mmi_feature_compile_test.c";
    print cTest "#include \"mmi_features.h\" \n";
    close(cTest);
    ...
    if ($g_removeTempFile)
    {
        system("del mmi_feature_compile_test.obj");
        system("del mmi_feature_compile_test.c");
    }
}
