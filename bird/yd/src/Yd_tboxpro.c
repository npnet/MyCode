...
#if 1//add by zyz		
		car_can_data.date[0] = 0;
		car_can_data.date[1] = 0;
		car_can_data.date[2] = 0;
#endif		
    }
    else
    {
...
void bird_tbox_control_param_res(U8* rest_buf, U32 length)
{
    ...
    else if(buf[buf_pos]==0x05)//断开数据连接，bird_get_lixian_time分钟后重新连接
    {
    ...
    /*
	else if(buf[buf_pos]==0x06) //报警等级
	{
	
	}
	else if(buf[buf_pos]==0x07)//开启抽样检测链路
	{
...
U8 bird_soc_send_tboxrealinfo(Send_Info *sendinfo)
{
    ...
    //if(Lima_get_soc_conn_flag()) //add by zyz
    {
        if(bird_get_encryption()==ENCRYPT_AES)
