...
bool g_is_mainini_finished = FALSE;
...
applib_time_struct g_save_dt= {0};//tbox获取数据时的时间
//如果有tbox的数据即txbox_can_time不为空时，则保存tbox的数据到g_save_send和g_cur_send，并把当前的数据保存到文件中。
void Yd_savedata()
{
    U8 rtn=0;

	//g_n_ydislogin = 1;//just for debug
    if(g_n_ydislogin==3)
    {
        kal_prompt_trace(MOD_USB," Yd_savedata login3");
    }
    else
    {
        rtn=bird_soc_send_tboxrealinfo(&g_save_send);
        kal_prompt_trace(MOD_USB," Yd_savedata %d",rtn);
        if(rtn==1)
        {
            ...
            kal_prompt_trace(MOD_USB," Yd_savedata %d %d",g_save_send.send_buf[24],g_cur_send.send_buf[24]);
            //can_data_reset();
            //RJ_reset_GPS_flag();
...
void Bird_Tbox_savedata(Send_Info *_send,applib_time_struct dt)
{
    ...
    kal_prompt_trace(MOD_SOC,"Bird_Tbox_savedata %d %d %d %d %d %d",dt.nYear, dt.nMonth, dt.nDay, dt.nHour,dt.nMin,dt.nSec);
    ...
    if(tboxdata_handle >= FS_NO_ERROR)
    {
        ...
        if(buflen>=0)
        {
			...
			//kal_prompt_trace(MOD_SOC,"Bird_Tbox_savedata handle for test2 =%d %s",len,file_name_str);
            FS_Close(tboxdata_handle);

			//tboxdata_handle = FS_Open(file, FS_CREATE | FS_READ_WRITE);
			//FS_GetFileSize(tboxdata_handle,&len);
			//if(tboxdata_handle >= FS_NO_ERROR)
			//	kal_prompt_trace(MOD_SOC,"Bird_Tbox_savedata handle for test =%d %s",len,file_name_str);
			//else
			//	kal_prompt_trace(MOD_SOC,"Bird_Tbox_savedata handle for test1 =%d %s",len,file_name_str);
			//FS_Close(tboxdata_handle);
        ...
        {
            {
				...
				if(Lima_get_soc_conn_flag())
					...
					FS_Write(tboxinfo_handle, "2",strlen("2"), &len); // add by zyz 离线数据
