...
extern void Bird_soc_setreduceflag_posfile(applib_time_struct dt , bool g_is_error_ack);
...
//找 %c:\\Tboxinfo\\rec_%04d-%02d-%02d-%02d.txt 中时间最早的，
...
S32 Bird_soc_getlast_unsend(S8* unsendtime)
{
	...
	//modify by zyz
    tmp_time.nMin=(bird_get_ser_res_time()+59)/60;
    ...
    if (handle >= FS_NO_ERROR)
    {
        ...
        if(buflen>0)
        {
            for(off=0; off<buflen; off=off+lenper)
            {
				...
				//add by zyz 容错处理
				if(len!=lenper||buf[lenper-1]>'9'||buf[lenper-1]<'0')
				{
					kal_prompt_trace(MOD_SOC,"Bird_soc_getlast_unsend error1");
					FS_Close(handle);
					FS_Delete(file);
					return nresult;
				}
                ...
                {
                    kal_prompt_trace(MOD_SOC,"Bird_soc_getlast_unsend time limit");//如不到超时的时间，则继续等待服务器应答
                    FS_Close(handle);
                    return nresult;
                }
				//要在前一次发送完成一分钟以后才能继续补发信息
                if(buf[lenper-1]=='8'||buf[lenper-1]=='7'||buf[lenper-1]=='6')
                {
				...
				//modify by zyz 离线数据要不做上一次发送时间上的判断，即时发送。
                else if(buf[lenper-1]=='2')
                {
					if(1)
                    {
                        g_readpos.offset=off/lenper;
                    ...
                    {
                        kal_prompt_trace(MOD_SOC,"Bird_soc_getlast_unsend time limit3");
                    }
                    FS_Close(handle);
                    return nresult;
                }
				//未找到未发信息 add by zyz
                else if((off+lenper==buflen)&&((buf[lenper-1]=='1')||(buf[lenper-1]=='9')))
                {
                    ...
                    if((atol(yearchar)<curtime.nYear)||(atol(monthchar)<curtime.nMonth)||(atol(daychar)<curtime.nDay)
                            ||(atol(hourchar)<curtime.nHour))//最后一条小于当前时间，则删除掉这个文件
                    {
                        FS_Close(handle);
                        FS_Delete(file);
                        return nresult;
                    ...
                    {
                        kal_prompt_trace(MOD_SOC,"Bird_soc_getlast_unsend delete time limit4");
                        FS_Close(handle);
                        return nresult;
                    }
                }
	...
	{
		kal_prompt_trace(MOD_SOC,"Bird_soc_getlast_unsend error test test");
	}
    FS_Close(handle);
    return nresult;
}
...
U8 bird_busend_handle(U8* unsendtime)
{
    ...
    {
        FS_GetFileSize(handle,&buflen);
        //if(buflen>=0) modify by zyz 
        if(buflen > 0)
        {
            U32 left, right, middle;

            left = 0,
            //right = buflen/lenper-1; // fix me add by zyz
            right = buflen/lenper;
	...
	else // add by zyz
	{
        ...
        nfind.nMin=atol(minchar);
        nfind.nSec=atol(secchar);
        kal_prompt_trace(MOD_SOC,"bird_busend_handle find");

        Bird_soc_setreduceflag_posfile(nfind,FALSE);
    }		
