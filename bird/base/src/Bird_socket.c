...
static S32                           g_lima_nwt_acount_id  = -1;   // ¶ÏÍøID
static mmi_lima_soc_progress_t     g_lima_pfn_progreess  = NULL;//  ½ø¶ÈÌõÍ¨Öªº¯Êý
static S32                           g_lima_timeout_count  = -1;   // ÉèÖÃ½ÓÊÕÊý¾Ý³¬Ê±µÄcount
static BOOL                          g_lima_gb_timer_ted = FALSE;//³¬Ê±µÄtimerÊÇ·ñÒÑ¾­¿ªÆô
...
static BOOL g_lima_is_soc_close_finished = TRUE; // add by zyz
...
{
    kal_prompt_trace(MOD_SOCKET," Bird_clear_soc_conn");
    //g_n_reconn = 0;
...
Youzhong            2009-08-25        08AÌí¼Óg_lima_cbm_app_idÇø·Ö²»Í¬Ó¦ÓÃ·¢ËÍµÄÇëÇó
GuoJin                      2009-08-25             08AÌí¼Ócbm_hold_bearer£¬ÔÚsoc_colseÊ±±£³ÖÍøÂçÁ¬½Ó²»¹Ø±Õ
GuoJin                      2009-09-28             ÐÞ¸ÄÒÆ¶¯¿¨ÓÅÏÈÁªÍø
**************************************************************************************************/
...
Functions        :        ÉèÖÃÁ¬½Ó³¬Ê±µÄÊ±¼äcounter
Parameters        :    1
    input        :    count :³¬Ê±count
    output        :    NONE
    return        :    NONE
...
S8 *  bird_get_nw_plmn(void)
{
    ...
    kal_prompt_trace(MOD_SOCKET,"status=%d",status);
    kal_prompt_trace(MOD_SOCKET,"plmn=%s",nw_plmn);
...
{
    U8 i=0;
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_set_sendflag flow:%d",nflow);
    for(i=0; i<MAX_BIRD_SENDBUF_LEN; i++)
    {
        //kal_prompt_trace(MOD_SOCKET,"Bird_soc_set_sendflag statue:%d,flow %d",g_socket_send.send_info[i].send_statue,g_socket_send.send_info[i].send_flow);
        ...
        if(g_socket_send.send_info[i].send_flow==nflow)
        {
            ...
            kal_prompt_trace(MOD_SOCKET,"Bird_soc_set_sendflag index:%d",i);
            break;
        }
    }
}
...
{
    kal_prompt_trace(MOD_SOCKET, "[Bird_soc_set_revflag] %d", flag);
    if(flag)
    {
...
{
    U8 i=0;
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_set_loginflag ");
    for(i=0; i<MAX_BIRD_SENDBUF_LEN; i++)
    {
        if(g_socket_send.send_info[i].send_statue==1)
        {
        ...
        {
            g_socket_send.send_info[i].send_statue=1;
            kal_prompt_trace(MOD_SOCKET,"Bird_soc_set_loginflag index:%d",i);
            break;
        }
    }
}
...
U8 Bird_soc_get_loginflag()
{
    ...
    for(i=0; i<MAX_BIRD_SENDBUF_LEN; i++)
    {
        if(g_socket_send.send_info[i].send_type==BIRD_SOC_SEND_LOGIN)
        {
            ...
            {
                nflag = 1;
                kal_prompt_trace(MOD_SOCKET,"bird_soc_send_login entry2\n");
                break;
            }
        }
    }
...
void Bird_soc_clear_file()
{
    ...
    U32  buflen;
    U16  name[128];
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_clear_file entry");
...
void Bird_soc_save_file(Send_Info *_send,applib_time_struct dt)
{
    ...
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_save_file %d",handle);
    ...
    if (handle >= FS_NO_ERROR)
    {
        ...
        if(buflen>=0)
        {
            ...
            kal_prompt_trace(MOD_SOCKET,"Bird_soc_save_file %d %d %d",buflen,_send->buf_len,_send->send_buf[2]);
            {
                FS_Seek(handle, buflen, FS_FILE_BEGIN);
...
void Bird_soc_save_info(applib_time_struct dt)
{
    ...
    handle = FS_Open(name, FS_CREATE | FS_READ_WRITE);
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_save_info %d",handle);
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_save_info %d %d %d %d %d %d",dt.nYear, dt.nMonth, dt.nDay, dt.nHour,dt.nMin,dt.nSec);
    ...
    if (handle >= FS_NO_ERROR)
    {
        ...
        {
            {
                ...
                kal_prompt_trace(MOD_SOCKET,"Bird_soc_save_info %s %d,%d",time,buflen,len);
                {
                    FS_Close(handle);
                    return;
                }
        ...
        {
            kal_prompt_trace(MOD_SOCKET,"Bird_soc_save_info buflen<0");
        }
    }
    FS_Close(handle);
}
...
void Bird_infoformat(applib_time_struct* dt, U8 *time, U32 *length)
{
    ...
    dt->nSec=input[5];
    */
    kal_prompt_trace(MOD_SOCKET,"Bird_infoformat %d %d %d %d %d %d",dt->nYear, dt->nMonth, dt->nDay, dt->nHour,dt->nMin,dt->nSec);
...
void Bird_soc_setflag_posfile(applib_time_struct dt)
{
    ...
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_setflag_posfile %d %d %d %d %d", handle,dt.nYear,dt.nMonth,dt.nDay,dt.nHour);
    ...
    if (handle >= FS_NO_ERROR)
    {
        ...
        if(buflen> 0)
        {
            for(off=0; off<buflen; off=off+lenper)
            {
                ...
                kal_prompt_trace(MOD_SOCKET,"Bird_soc_setflag_posfile %d,%d,%s",buflen,off,buf);
                        ...
                        &&(atoi(minchar)==dt.nMin)&&(atoi(secchar)==dt.nSec))
                {
                    ...
                    kal_prompt_trace(MOD_SOCKET,"Bird_soc_setflag_posfile write %d",len);
                    //FS_Close(handle);
                    //return;
                }
...
//add by zyz
void Bird_soc_setreduceflag_posfile(applib_time_struct dt , bool g_is_error_ack)
{
    FS_HANDLE  handle;
    U32  len;
    U32 lenper;
    ...
    kal_wsprintf(name,BIRD_TBOXINFO_PATH,(S16)MMI_CARD_DRV, dt.nYear, dt.nMonth, dt.nDay, dt.nHour);
    handle = FS_Open(name, FS_READ_WRITE);
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_setreduceflag_posfile %d %d %d %d %d", handle,dt.nYear,dt.nMonth,dt.nDay,dt.nHour);
    ...
    if (handle >= FS_NO_ERROR)
    {
        ...
        if(buflen> 0)
        {
            for(off=0; off<buflen; off=off+lenper)
            {
                ...
                kal_prompt_trace(MOD_SOCKET,"Bird_soc_setreduceflag_posfile %d,%d,%s",buflen,off,buf);
                if((atoi(yearchar)==dt.nYear)&&(atoi(monthchar)==dt.nMonth)&&(atoi(daychar)==dt.nDay)&&(atoi(hourchar)==dt.nHour)
                        &&(atoi(minchar)==dt.nMin)&&(atoi(secchar)==dt.nSec))
                {
                    FS_Seek(handle, off+14, FS_FILE_BEGIN);

					if(g_is_error_ack)//ÊÕµ½µÚ1´ÎµÄ´íÎóÓ¦´ðÊ±ÐÞ¸Ä×´Ì¬¡£
					{
						if(buf[14] == '9')
							buf[14] = '8';
					}
	                ...
	                FS_Write(handle, (S8 *)&buf[14] , strlen("1"), &len);
					
                    kal_prompt_trace(MOD_SOCKET,"Bird_soc_setreduceflag_posfile write %d",buf[14]);
                    //FS_Close(handle);
                    //return;
                }
...
S8 Bird_soc_getfrom_file(U16 noff)
{
    ...
    U32 off;
    S8 i;
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_getfrom_file entry");
    ...
    if (handle >= FS_NO_ERROR)
    {
        ...
        if(buflen> 0)
        {
            /*for(i=0;i<MAX_BIRD_POSSAVE_COUNT;i++)
             {
            	...
            	memset(buf, 0, sizeof(buf));
            	FS_Read(handle, (void *)buf, lenper, &len);
            	kal_prompt_trace(MOD_SOCKET,"Bird_soc_save_file len=%d,%s",len,buf);
            	if(len==0)
            	{
            		break;
            	}
            ...
            if(off<=buflen)
            {
                ...
                memset(buf, 0, sizeof(buf));
                FS_Read(handle, (void *)buf, lenper, &len);
                kal_prompt_trace(MOD_SOCKET,"Bird_soc_getfrom_file2 len%d,%d,off=%d",buflen,len,off);
                kal_prompt_trace(MOD_SOCKET,"Bird_soc_getfrom_file2 len%s",buf);
                //Î´´¦Àí9 or 8
...
S8 Bird_soc_is_exist_unread(U16 noff)
{
    ...
    U32 off;
    S8 i;
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_is_exist_unread entry");
    lenper = MAX_BIRD_NUMBER_PERLINE;
    memset(name, 0, sizeof(name));
    kal_wsprintf((kal_wchar*) name,BIRD_POS_FILE_PATH,(S16)MMI_CARD_DRV);
    handle = FS_Open((U16 *)name, FS_CREATE | FS_READ_WRITE);
    if (handle >= FS_NO_ERROR)
    {
        ...
        if(buflen> 0)
        {
            ...
            if(off<=buflen)
            {
                ...
                memset(buf, 0, sizeof(buf));
                FS_Read(handle, (void *)buf, lenper, &len);
                kal_prompt_trace(MOD_SOCKET,"Bird_soc_is_exist_unread len%d,%d,off=%d",buflen,len,off);
                kal_prompt_trace(MOD_SOCKET,"Bird_soc_is_exist_unread len%s",buf);
                //Î´´¦Àí9 or 8
                if(buf[0]==57 || buf[0]==56)
                {
                    nresult = 9;
...
void Bird_alarm_run_return()
{
    ...
    kal_prompt_trace(MOD_SOCKET,"[Bird_alarm_run_return] times,return,index=%d,%d,%d",g_alarmrun_times,g_alarmrun_return,nindex);
...
{
    kal_prompt_trace(MOD_SOCKET,"Bird_heart_return %d",Lima_get_soc_conn_flag());
    Rj_stop_timer(BIRD_TASK_SOCKET_RECON);
    Rj_stop_timer(BIRD_TASK_SOCKET_SEND);
    //Rj_stop_timer(Bird_task_heart_Timer); removed by zyz
    //if(!Lima_get_soc_conn_flag())
    {
...
void Bird_soc_send()
{
	...
	U16 j;
    //Bird_Log2(2);
    //kal_prompt_trace(MOD_SOCKET,"Bird_soc_send entry");
    ...
    if(!Lima_get_soc_conn_flag())
    {
        //lrf 20131029add Èç¹ûÓÐÐèÒª·¢ËÍµÄÊý¾ÝÆô¶¯Á¬½Ó
    ...
    if(g_socket_send.g_n_send_state!=BIRD_SOC_NULL)
    {
    	 ...
    	 kal_prompt_trace(MOD_SOCKET,"Bird_soc_send:g_n_rev_state =%d ,g_n_rev_reconn =%d",g_socket_send.g_n_rev_state,g_n_rev_reconn);
    ...
    */
    if(g_socket_send.g_n_send_index<g_socket_send.g_n_send_indexAdd)
    	kal_prompt_trace(MOD_SOCKET,"[Bird_soc_send]g_socket_send.g_n_send_state= %d,index  = %d,indexAdd = %d",g_socket_send.g_n_send_state,g_socket_send.g_n_send_index,g_socket_send.g_n_send_indexAdd);
    if(g_socket_send.g_n_send_state!=BIRD_SOC_SENDING)
    {
        ...
        for(i=0;i<MAX_BIRD_SENDBUF_LEN;i++)
        {
        	...
        	kal_prompt_trace(MOD_SOCKET,"[Bird_soc_send] send_count = %d,index = %d",g_socket_send.send_info[i].send_count,i);
        	if(g_socket_send.send_info[i].send_count>15)
        	{
        	       kal_prompt_trace(MOD_SOCKET,"[Bird_soc_send] resent flow= %d,sendtimes=%d",g_socket_send.send_info[i].send_flow,g_socket_send.send_info[i].send_times);
        ...
        if(g_socket_send.g_n_send_state!=BIRD_SOC_SENDING)
        {
            if(g_socket_send.g_n_send_index<g_socket_send.g_n_send_indexAdd)
            {
                ...
                else if(g_socket_send.send_info[send_index].send_type==BIRD_SOC_SEND_ALARM_LOW)
                {
                    ...
                    g_alarmlow_return = 0;
                    g_alarmlow_times = 0;
                    kal_prompt_trace(MOD_SOCKET,"[Bird_soc_send] alarm_low");
                    Rj_stop_timer(BIRD_TASK_ALARM_LOW_WAIT_TIMER);
                    Rj_start_timer(BIRD_TASK_ALARM_LOW_WAIT_TIMER, 60*1000, Bird_alarm_low_return,NULL);
                }
                else if(g_socket_send.send_info[send_index].send_type==BIRD_SOC_SEND_ALARM_POWEROFF)
                {
                    ...
                    g_alarmpoweroff_times = 0;
                    kal_prompt_trace(MOD_SOCKET,"[Bird_soc_send] alarm_poweroff");
                    Rj_stop_timer(BIRD_TASK_ALARM_POWEROFF_WAIT_TIMER);
                    Rj_start_timer(BIRD_TASK_ALARM_POWEROFF_WAIT_TIMER, 60*1000, Bird_alarm_poweroff_return,NULL);
                }
                else if(g_socket_send.send_info[send_index].send_type==BIRD_SOC_SEND_ALARM_MOVE)
                {
                    ...
                    g_alarmmove_times = 0;
                    kal_prompt_trace(MOD_SOCKET,"[Bird_soc_send] alarm_move");
                    Rj_stop_timer(BIRD_TASK_ALARM_MOVE_WAIT_TIMER);
                    Rj_start_timer(BIRD_TASK_ALARM_MOVE_WAIT_TIMER, 60*1000, Bird_alarm_move_return,NULL);
                }
                else if(g_socket_send.send_info[send_index].send_type==BIRD_SOC_SEND_ALARM_ZD)
                {
                    ...
                    g_alarmrun_return = 0;
                    g_alarmrun_times = 0;
                    kal_prompt_trace(MOD_SOCKET,"[Bird_soc_send] alarm_zd");
                    Rj_stop_timer(BIRD_TASK_ALARM_RUN_WAIT_TIMER);
                    Rj_start_timer(BIRD_TASK_ALARM_RUN_WAIT_TIMER, 60*1000, Bird_alarm_run_return,NULL);
                }
    ...
    g_socket_send.g_n_send_state=BIRD_SOC_SENDING;
    kal_prompt_trace(MOD_SOCKET,"[Bird_soc_send]g_lima_gp_socket->snd_data_len = %d, %d,%s",g_lima_gp_socket->snd_data_len,g_lima_gp_socket->snd_counter,(g_lima_gp_socket->snd_buffer + g_lima_gp_socket->snd_counter));

	//rmmi_write_to_uart((kal_uint8*)"send data:", strlen("send data:"), KAL_TRUE);
	//for(j = 0; j < g_lima_gp_socket->snd_data_len - g_lima_gp_socket->snd_counter; j++)
	//	rmmi_write_to_uart((kal_uint8*)(g_lima_gp_socket->snd_buffer + g_lima_gp_socket->snd_counter + j), 1, KAL_FALSE);
    ...
    kal_prompt_trace(MOD_SOCKET, "[Bird_soc_send]Http send request result, sent_bytes:%d", ret);
    if (ret > 0)
    {
    ...
    {
        kal_prompt_trace(MOD_SOCKET,"[Bird_soc_send]LIMA_SOC_SEND_ERR");
        g_socket_send.g_n_send_state=BIRD_SOC_SENTED;
        //StartTimer(BIRD_SOCKET_SEND, 2000, Bird_soc_send);
        Rj_stop_timer(BIRD_TASK_SOCKET_SEND);
        Rj_start_timer(BIRD_TASK_SOCKET_SEND, 1000, Bird_soc_send,NULL);
    ...
    kal_prompt_trace(MOD_SOCKET, "[Bird_soc_send]send ret:%d", ret);
...
void Bird_soc_connbyIp()
{
    ...
    kal_prompt_trace(MOD_SOCKET,"[Bird_soc_connbyIp]server_ip_addr:%d.%d.%d.%d and port: %d",
                     p_soc_add->addr[0], p_soc_add->addr[1],p_soc_add->addr[2],
                     p_soc_add->addr[3],    p_soc_add->port);

    kal_prompt_trace(MOD_SOCKET,"[Bird_soc_connbyIp]Http connect result is %d", ret);
    if (ret == SOC_SUCCESS)
    {
        //LEDÉÁË¸
        ...
        g_lima_gp_socket->state = LIMA_SOC_HTTP_TCP_CON_CREATED;  /*ÒÑ¾­Á¬½Ó*/
        g_n_reconn = 0;
    ...
    {
        //error
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Http_Send_Request]LIMA_SOC_CONNECT_ERR");
        mmi_frm_clear_protocol_event_handler(MSG_ID_APP_SOC_NOTIFY_IND, (PsIntFuncPtr)Lima_Soc_Socket_Notify);
        mmi_frm_set_protocol_event_handler(MSG_ID_APP_SOC_NOTIFY_IND,(PsIntFuncPtr)Lima_Soc_Socket_Notify,MMI_TRUE);
    }
...
void Bird_save_ip()
{
    ...
    kal_prompt_trace(MOD_SOCKET,"[Bird_save_ip] get: %s",getaddr);
    if(strcmp(saveaddr,getaddr)==0)
    {
        kal_prompt_trace(MOD_SOCKET,"[Bird_save_ip]: same");
    }
    else
    {
    ...
    kal_prompt_trace(MOD_SOCKET,"[Bird_save_ip]: %s",saveaddr);
}
...
void Bird_soc_conn()
{
    ...
    if(Lima_get_soc_conn_flag())
    {
        ...
        nrestresion = Bird_get_reseterr();
        kal_prompt_trace(MOD_SOCKET," [Bird_soc_conn] error flag%d",nrestresion);
        if(nrestresion!=0)
        {
            Bird_set_reseterr(BIRD_NORMAL);
            yd_send_save_nv_msg();
    ...
    else
    {
		...
		//add by zyzÑÓÊ±15ÃëÔÙÖØÐÂÁ¬½ÓµÈ´ýÖ®Ç°µÄsocket closeÍê³É£¬²»È»ÓÐ¿ÉÄÜÁ¬½Ó³É¹¦ºóµÈµ½Òì²½µÄcloseÃüÁî¡£
    	if(g_lima_is_soc_close_finished == FALSE)
    	{
			Rj_stop_timer(BIRD_TASK_SOCKET_RECON);
			Rj_start_timer(BIRD_TASK_SOCKET_RECON, RJ_GPS_APP_15S, Bird_soc_conn,NULL);   
			g_lima_is_soc_close_finished = TRUE;
			return;
    	}
        ...
        {
            g_n_reconn ++;
            kal_prompt_trace(MOD_SOCKET,"[Bird_soc_conn]: g_n_reconn=%d",g_n_reconn);
            //if(g_n_reconn==2)//modify by zyz
			if(0)//((g_signal_flag==0)&&(g_n_reconn==2))
            {
                kal_prompt_trace(MOD_SOCKET," [Bird_soc_conn] error with static and weak signal");
                g_n_reconn=2;
            ...
            else if(g_n_reconn==3)
            {
                ...
                //g_n_reconn = 0;
                kal_prompt_trace(MOD_SOCKET,"g_n_reconn 30 min");
                Rj_stop_timer(BIRD_TASK_SOCKET_RECON);
            ...
            {
                g_n_reconn = 0;
                kal_prompt_trace(MOD_SOCKET,"g_n_reconn reboot");
    ...
    if(!Lima_get_soc_init_flag())
    {
        ...
        if (LIMA_SOC_NO_ERROR == ret)
        {
            ...
            {
                kal_prompt_trace(MOD_SOCKET,"[g_lima_is_getip]:%d",g_lima_is_getip);
				//add by zyz
                if(0)//g_n_reconn==2)
                {
...
{
    S32 send_index=0;
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_sendbufAdd2 \n");
    ...
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_sendbufAdd2 buf_len,indexAdd %d,%d",_sendinfo->buf_len,send_index);
...
{
    S32 send_index=0;
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_alarm_sendbufAdd2 \n");
    ...
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_alarm_sendbufAdd2 buf_len,indexAdd%d,%d",_sendinfo->buf_len,send_index);
    g_socket_send_alarm.g_n_send_indexAdd++;
}

//ÎªÊ²Ã´3¸öÒ»×é´ò°üºóÔÙ·¢ËÍå
void Bird_soc_alarm_to_send()
{
    ...
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_alarm_to_send");
    /*
...
void Bird_soc_Set_Header(S8 nflag)
{
    ...
    {
        if(g_gettime_flag==0)
            header.p_url        = (S8 *)YD_TK001_HTTP2;//"169.254.211.131:9090";//YD_TK001_HTTP2;
        else
    ...
    {
        if(g_gettime_flag==0)
            header.p_url        = (S8 *)YD_TK001_IP2;//"169.254.211.131:8090";//YD_TK001_IP2;
        else
...
static kal_uint32 add_new_acc_prof_id(const WCHAR* account_name)
{
    ...
    prof_gprs.APN = "bdnet";	// Õâ¸öÖµÔÚÓ¦ÓÃÊ¹ÓÃµÄÊ±ºò»áÐÞ¸ÄµÄ
    ...
    kal_prompt_trace(MOD_SOCKET,"add acc_prof_id =%d",acc_prof_id);
...
kal_uint32 update_acc_prof_id(kal_uint32 acc_prof_id, U8 *apn, U8 *px_addr, U16 px_port,U8 *px_authid, U8 *px_authpw, U8 *primaryaddr, U8 *secondaryaddr,U8 *homepage)
{
    ...
    /*
     kal_prompt_trace(MOD_SOCKET,"apn1 %s  \n",apn);
     ...
     kal_prompt_trace(MOD_SOCKET,"apn2 %s  \n",prof_gprs.APN);
     */
    ...
    kal_prompt_trace(MOD_SOCKET,"px_addr1 %s  \n",px_addr);
    ...
    kal_prompt_trace(MOD_SOCKET,"px_addr2 %s  \n",prof_gprs.prof_common_header.px_addr);
    kal_prompt_trace(MOD_SOCKET,"px_port1 %d  \n",px_port);
    if((px_port !=0) ||(px_port !=NULL))
    {
        prof_gprs.prof_common_header.px_port = px_port;
    }

    kal_prompt_trace(MOD_SOCKET,"px_port2 %d  \n",prof_gprs.prof_common_header.px_port);
    kal_prompt_trace(MOD_SOCKET,"px_authid1 %s  \n",px_authid);
    ...
    kal_prompt_trace(MOD_SOCKET,"px_authid2 %s  \n",prof_gprs.prof_common_header.px_authid);

    kal_prompt_trace(MOD_SOCKET,"px_authpw1 %s  \n",px_authpw);
    ...
    kal_prompt_trace(MOD_SOCKET,"px_authpw2 %s  \n",prof_gprs.prof_common_header.px_authpw);

    kal_prompt_trace(MOD_SOCKET,"PrimaryAddr1 %s  \n",primaryaddr);
    ...
    kal_prompt_trace(MOD_SOCKET,"PrimaryAddr2 %s  \n",prof_gprs.prof_common_header.PrimaryAddr);


    kal_prompt_trace(MOD_SOCKET,"SecondaryAddr1 %s  \n",secondaryaddr);
    ...
    kal_prompt_trace(MOD_SOCKET,"SecondaryAddr2 %s  \n",prof_gprs.prof_common_header.SecondaryAddr);

    kal_prompt_trace(MOD_SOCKET,"HomePage1 %s  \n",homepage);
    ...
    kal_prompt_trace(MOD_SOCKET,"HomePage2 %s  \n",prof_gprs.prof_common_header.HomePage);
    ...
    kal_prompt_trace(MOD_SOCKET,"update acc_prof_id =%d",acc_prof_id);
...
// add by lqy
    ...
    kal_prompt_trace(MOD_SOCKET, "[BIRD_Soc_Init] Enter");
    index = 0;
    ...
    if (NULL == g_lima_gp_soc_pool)
    {
        ...
        if (Lima_plat_get_yys()==lima_operator_unicom)
        {
        	kal_prompt_trace(MOD_SOCKET, "[BIRD_Soc_Init] uninet");
        	srv_dtcnt_get_acc_id_by_apn("uninet",&acc);
        	//srv_dtcnt_get_acc_id_by_apn("uniwap",&acc);
        }
        else
        {
        	kal_prompt_trace(MOD_SOCKET, "[BIRD_Soc_Init] cmnet");
        ...
        bird_get_nw_plmn();
        kal_prompt_trace(MOD_SOCKET,"bird_get_nw_plmn()=%s",bird_get_nw_plmn());
        ...
        {
            kal_prompt_trace(MOD_SOCKET,"factory srv_dtcnt_store_qry_ids sucess");
            kal_prompt_trace(MOD_SOCKET,"factory acct_id_query num_ids=%d",acct_id_query.num_ids);
            if(acct_id_query.num_ids>0)
            {
                for (i=0; i<(acct_id_query.num_ids); i++)
                {

                    kal_prompt_trace(MOD_SOCKET,"accound id=%d",acct_id_query.ids[i]);
                    ...
                    kal_prompt_trace(MOD_SOCKET,"prof_str_info_qry name=%s",charname);
                    // get apn
                    prof_str_info_qry.dest = (S8*)name;
                    prof_str_info_qry.dest_len = sizeof(name);
                    srv_dtcnt_get_apn(acct_id_query.ids[i],&prof_str_info_qry,SRV_DTCNT_ACCOUNT_PRIMARY);
                    mmi_ucs2_to_asc((char *)charname,(char *)prof_str_info_qry.dest);
                    kal_prompt_trace(MOD_SOCKET,"prof_str_info_qry apn name=%s",charname);
            ...
            else
            {
                ...
                if (result == SRV_DTCNT_RESULT_SUCCESS)
                {
                    kal_prompt_trace(MOD_SOCKET,"user srv_dtcnt_store_qry_ids sucess");
                    kal_prompt_trace(MOD_SOCKET,"user acct_id_query num_ids=%d",acct_id_query.num_ids);
                    if(acct_id_query.num_ids>0)
                    {
                        for (i=0; i<(acct_id_query.num_ids); i++)
                        {
                    ...
                    else
                    {
                        ...
                        //reboot
                        {
                            kal_prompt_trace(MOD_SOCKET,"Lima_Soc_Init reboot");
                            RJ_SYS_ReStart();
                        }
                    }
                }
                ...
                {
                    kal_prompt_trace(MOD_SOCKET,"srv_dtcnt_store_qry_ids fail = %d",result);
                }
            }
        }
        else
        ...
        if(acct_id_query.num_ids>0)
        {
...
#if defined(__MMI_DUAL_SIM_MASTER__) && defined(__MTK_TARGET__)//Ñ¡Ôñ¿¨Ò»£¬¿¨¶þÁªÍø
            //index = Lima_plat_Get_Priv_Simcard();
            kal_prompt_trace(MOD_SOCKET,"aact_id=%d",aact_id);
            index = Get_Simcard();
            kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Init] SIM  :Get Sim Id =%d!!",index);
            if ( MMI_SIM1 == index)
            {
            ...
            {
                kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Init] SIM  :Get Sim Id Or Net Registration Fail!!");
            }
#else
            g_lima_gp_socket->nwt_acount_id = cbm_encode_data_account_id(acct_id_query.ids[0], CBM_SIM_ID_SIM1, g_lima_cbm_app_id, 0);
#endif
        ...
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Init] g_lima_gp_socket->nwt_acount_id = %d", g_lima_gp_socket->nwt_acount_id);
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Init]  g_lima_gp_socket->socket_id = %d",g_lima_gp_socket->socket_id);
    return ret;
}
...
Functions        :        ÊÍ·Å×ÊÔ´£¬È¡ÏûSocketÇëÇó
Parameters        :    0
    input        :    NONE
    output        :    NONE
    return        :    NONE
...
void Lima_Soc_Dinit(void)
{
    ...
    Lima_set_soc_init_flag(FALSE);
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Dinit] Enter:bird_gps_nwt_acount_id,socket_id=%d,%d",g_lima_nwt_acount_id,g_lima_gp_socket->socket_id);
    ...
    if ((NULL != g_lima_gp_soc_adm_id) && (NULL != g_lima_gp_soc_pool))
    {
        if (NULL != g_lima_gp_socket)
        {
            if (g_lima_gp_socket->socket_id >= 0)
            {
				...
				g_lima_is_soc_close_finished = FALSE;
            }
    ...
    kal_prompt_trace(MOD_SOCKET, "[bird_Soc_Dinit]  g_lima_gp_socket->socket_id = %d",g_lima_gp_socket->socket_id);
    kal_prompt_trace(MOD_SOCKET, "[bird_Soc_Dinit] Leave ");
}
...
Functions        :        Á¬½Ó(½ÓÊÕ)Ê±¼ä³¬Ê±µÄ»Øµ÷º¯Êý
Parameters        :    0
    input        :    NONE
    output        :    NONE
    return        :    NONE
...
GuoJin                      2009-08-07            Ôö¼Ó½ÓÊÕÊý¾Ý³¬Ê±
**************************************************************************************************/
static void Lima_Soc_Time_Out(void)
{
    S16                  result;

    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Time_Out]Entry");
    if ((NULL != g_lima_gp_socket) && (NULL != g_lima_gp_socket->callback))
    {
        ...
        Lima_set_soc_init_flag(FALSE);//»Ö¸´¡£ÖØÐÂ¿ªÊ¼
...
Functions        :        Ð´ÈëÒòÈ¡Ïû»ò´íÎóÖÐ¶Ï½ÓÊÕÊý¾ÝµÄÁ¬½Ó²ÎÊý
Parameters        :    1
    input        :    soc
    output        :    NONE
    return        :    NONE
...
void Lima_Soc_Write_Continue_Dl_Para(lima_soc_transaction_struct *soc)
{
    ...
    //kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Write_Continue_Dl_Para]Entry");
...
void Lima_Soc_Del_Continue_Dl_Para(lima_soc_transaction_struct *soc)
{
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Del_Continue_Dl_Para]Entry");
...
void Lima_Soc_Set_Continue_Dl_Para(lima_soc_transaction_struct *soc)
{
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Set_Continue_Dl_Para]Entry");
...
U8 Lima_Soc_is_creating_status(void)
{
    ...
    kal_prompt_trace(MOD_SOCKET,"[Lima_Soc_is_creating_status] g_lima_gp_socket->state:%d",g_lima_gp_socket->state);
    return ret;
}
...
static void Lima_Soc_Set_Req_Header(lima_soc_req_header_struct *header)
{
    ...
    if (NULL != header->p_url)
    {
        ...
        {
            kal_prompt_trace(MOD_SOCKET,"[Lima_Soc_Send_Post_Req] url length is too long !");
        }
    }
...
static U16 Lima_Soc_Create_Socket(void)
{
    ...
    kal_prompt_trace(MOD_SOCKET,"[Lima_Soc_Create_Socket] Enter g_lima_gp_socket->socket_id = %d",g_lima_gp_socket->socket_id);
    if (g_lima_gp_socket->socket_id >= 0)
    {
        if (soc_setsockopt(g_lima_gp_socket->socket_id, SOC_NBIO, &val, sizeof(val)) < 0)
        {
            kal_prompt_trace(MOD_SOCKET,"[Lima_Soc_Create_Socket] nonblock mode error !!");
            return LIMA_SOC_CREATE_ERR;
        }
        val = SOC_READ | SOC_WRITE | SOC_CLOSE | SOC_CONNECT;
        if (soc_setsockopt(g_lima_gp_socket->socket_id, SOC_ASYNC, &val, sizeof(val)) < 0)
        {
            kal_prompt_trace(MOD_SOCKET,"[Lima_Soc_Create_Socket] set socket mode error !!");
            return LIMA_SOC_CREATE_ERR;
        }
...
static void Lima_Soc_Http_Get_Parsed_Url(const S8* url, lima_soc_http_url_struct *p_http_url)
{
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Http_Get_Parsed_Url] Entry");
    kal_prompt_trace(MOD_SOCKET,"Lima_Soc_Http_Get_Parsed_Url url%s  \n",url);
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Http_Get_Parsed_Url] slash %d",slash);
    if (NULL == slash)
    {
        colon = strchr(hostname, ':');
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Http_Get_Parsed_Url] colon %d",colon);
        if (colon)
        {
            ...
            kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Http_Get_Parsed_Url] %s",p_http_url->host_name);
            colon++;
            p_http_url->ser_port_num = atoi(colon);
            kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Http_Get_Parsed_Url] %d",p_http_url->ser_port_num);
        }
        else
        {
...
BOOL Lima_Soc_Get_Server_Ip()
{
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Get_Server_Ip] entry");
    ...
    while (host_name[i])
    {
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Get_Server_Ip] host_name %d",host_name[i]);
        if (host_name[i]!='.')
        {
...
Functions        :        ½âÎöÖ÷»úÃû
Parameters        :    1
    input        :    hostname        Ö÷»úÃû
    output        :    NONE
    return        :    NONE
< Author >            < Date >              < Notes >
GuoJin                2009-05-15        create:Ö»ÓÃÓÚCMNET
**************************************************************************************************/
static void Lima_Soc_Get_Host_By_Name(const S8 *hostname)
{
    ...
    kal_prompt_trace(MOD_SOCKET,"[Lima_Soc_Get_Host_By_Name] Entry hostname = %s", hostname);
    ret = soc_gethostbyname(KAL_FALSE,
                            MOD_MMI,
                            (kal_int32) sSocketReq,//request_id,app_id,¿ÉÄÜÐèÒªÐÞ¸Ä
                            (kal_char*) hostname,
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Get_Host_By_Name] ret = %d", ret);
    ...
    {
        kal_prompt_trace(MOD_SOCKET,"[Lima_Soc_Get_Host_By_Name]LIMA_SOC_GET_HOST_ERR");
    }
}
...
Functions        :        ½âÎöÖ÷»úÃû£¬MSG_ID_APP_SOC_GET_HOST_BY_NAME_IND»Øµ÷º¯Êý
Parameters        :    0
    input        :    NONE
    output        :    NONE
    return        :    NONE
...
static MMI_BOOL Lima_Soc_Get_Host_By_Name_Ind(void *inMsg)
{
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Get_Host_By_Name_Ind] entry");
    ...
    if(dns_ind->request_id==(kal_int32)SOCKETREQID)
    {
        ...
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Get_Host_By_Name_Ind]Http send request to %d.%d.%d.%d and port: %d",
...
static MMI_BOOL Lima_Soc_Socket_Notify(void *inMsg)
{
    ...
    //kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify]Entry");
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify]soc_notify->event_type = %d", soc_notify->event_type);
	kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify]notify->socket_id = %d, g_lima_gp_socket->socket_id = %d", soc_notify->socket_id, g_lima_gp_socket->socket_id);
    //Ìí¼ÓÒì²½´¦Àí2009-06-11
    if ((NULL == g_lima_gp_socket)||(soc_notify->socket_id != g_lima_gp_socket->socket_id))
    {
        //¼ÙÈçÊÇÒÑ¾­´¦ÀíÍêµÄsocket,²»ÔÚ´¦Àí
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify]notify->socket_id = %d, g_lima_gp_socket->socket_id = %d", soc_notify->socket_id, g_lima_gp_socket->socket_id);
        return FALSE;
    }
    ...
    case SOC_CONNECT:
    {
        if (soc_notify->result == KAL_TRUE)
        {
			...
			kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify] g_n_ydislogin = %d", g_n_ydislogin);
			// add by zyz Ôø¾­Á´½Ó³É¹¦¹ý£¬µ±Ç°Ã»ÓÐÁ´½Ó
            if(0)//(g_n_ydislogin!=0)&&(g_n_ydislogin!=3)&&!Lima_get_soc_conn_flag())
            //if((g_n_ydislogin!=1)&&!Lima_get_soc_conn_flag())
            {
                applib_dt_get_rtc_time(&curtime);
                if((applib_dt_compare_time(&curtime,&g_login_time,NULL)!= DT_TIME_LESS))
                {
                	kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify]Yd_conn_logintxbox");
                    Yd_conn_logintxbox();
        ...
        else
        {
            ...
            kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify] SOC_CONNECT error_cause = %d,detail_cause = %d", soc_notify->error_cause, soc_notify->detail_cause);
    ...
    case SOC_READ:
    {
        ...
        //Lima_set_soc_rev_state(1); //ÊÕµ½Êý¾Ý±êÖ¾
        Lima_Soc_Recv_Response();
    }
    break;
    case SOC_CLOSE:
    {
        //RCVD blockºóµ÷ÓÃMSG_ID_APP_SOC_NOTIFY_IND»Øµ÷£¬µ±×÷½ÓÊÕÍê³É´¦Àí

        kal_prompt_trace(MOD_SOCKET, "g_lima_gp_socket->rcvd_data_len:%d",g_lima_gp_socket->rcvd_data_len);
        if (0 == g_lima_gp_socket->rcvd_data_len)
        {

        }
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify] SOC_CLOSE");
        Lima_set_soc_init_flag(FALSE);
        Lima_set_soc_conn_flag(FALSE);
        Lima_Soc_Dinit();
		g_lima_is_soc_close_finished = TRUE;
    ...
    break;
    default:
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify] default");
        //en_soc_output_result(EN_SOC_ERROR, NULL, 0);
        break;
    }
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Socket_Notify] End");
}


/**************************************************************************************************
Function name    :    Lima_Soc_Recv_Response
Functions        :        Socket½ÓÊÕ»Ø´«Êý¾Ý
Parameters        :    0
    input        :    NONE
    output        :    NONE
    return        :    NONE
...
static void Lima_Soc_Recv_Response(void)
{
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Recv_Response] entry");
    if(g_lima_gp_socket)
    {
        kal_prompt_trace(MOD_SOCKET, "Lima_Soc_Recv_Response g_lima_gp_socket->state= %d",g_lima_gp_socket->state);
    }
    if (g_lima_gp_socket && g_lima_gp_socket->state < LIMA_SOC_RSP_RCVD)
    {
        ...
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Recv_Response]Write To Buffer");
        ret_val = soc_recv(g_lima_gp_socket->socket_id, (kal_uint8*)g_lima_gp_socket->rcvd_buffer,
                           MAX_LIMA_RCV_BUFFER_SIZE,
                           0);

        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Recv_Response]ret_val=  %d", ret_val);
        ...
        if (ret_val > 0)
        {
            ...
            if (g_lima_gp_socket->rcvd_counter == 0)
            {
                ...
                //Ôö¼Ó¶Ôchunked·½Ê½µÄ´¦Àí£¬Ô¤ÏÈ²»ÖªµÀÊý¾ÝÌå³¤¶È2009-06-12(HTTP1.1)
                ...
                kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Recv_Response]First Recv  rcvd_data_len = %d,rcvd_head_len = %d, rcvd_total_len = %d", g_lima_gp_socket->rcvd_data_len,g_lima_gp_socket->rcvd_head_len,g_lima_gp_socket->rcvd_total_len);
    ...
    {
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Recv_Response]ERROR!!!!!!!! ");
        Lima_set_soc_init_flag(FALSE);
        Lima_Soc_Dinit();
    }
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Recv_Response]End ");
}
...
#else
    S8 *pCh = NULL; //lrf
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Ascii_Lwr]Entry");
...
Functions        :        µÃµ½»Ø´«Êý¾ÝÏûÏ¢Í·³¤¶È
Parameters        :    2
    input        :    chlwr       ·þÎñÆ÷·µ»ØÊý¾Ý(Ð¡Ð´×ÖÄ¸)
                    is_chunk  Transfer-EncodingÊÇ·ñÎªchunked
    output        :    NONE
    return        :    len          ÏûÏ¢Í·³¤¶È
< Author >            < Date >              < Notes >
GuoJin                2009-05-15        create:
...
static U32 Lima_Soc_Get_Rcvd_Head_Len(S8 *chlwr, U8 is_chunk)
{
    ...
    S8  *pCh = NULL;
    //"chunked\r\n\r\n"
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Get_Rcvd_Head_Len] entry");
    if (NULL != chlwr)
    {
        pCh = chlwr;
        if (TRUE == is_chunk)
...
Functions        :        µÃµ½»Ø´«Êý¾ÝÄÚÈÝ³¤¶È
Parameters        :    1
    input        :    chlwr       ·þÎñÆ÷·µ»ØÊý¾Ý(Ð¡Ð´×ÖÄ¸)
    output        :    NONE
    return        :    len          Êý¾ÝÄÚÈÝ³¤¶È
< Author >            < Date >              < Notes >
GuoJin                2009-05-15        create:
GuoJin                2009-05-22             note  :"Content-length:"´óÐ¡Ð´¼°ºó¸ú¿Õ¸ñÎÊÌâ
**************************************************************************************************/
static U32 Lima_Soc_Get_Rcvd_Data_Len(S8 *chlwr)
{
    ...
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Get_Rcvd_Data_Len]Entry");
...
Functions        :        µÃµ½HTTP ×´Ì¬Âë
Parameters        :    1
    input        :    chlwr  ·þÎñÆ÷·µ»ØÊý¾Ý
    output        :    NONE
    return        :    status          HTTP×´Ì¬Âë
< Author >            < Date >              < Notes >
...
static U16 Lima_Soc_Get_Http_Rsp_Status(S8 *chlwr)
{
    U16 status    = 0;
    S8  *pCh      = NULL;
    kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Get_Http_Rsp_Status] entry");
    pCh = chlwr + strlen("HTTP/1.1") + 1;//»ò"HTTP/1.0 "È¡µÃ×´Ì¬Âë
    while(*pCh == ' ')
    {
        pCh += 1;
    }
...
static void Lima_Soc_Reyake_Request(void)
{
    ...
    if (LIMA_SOC_CREATE_ERR != ret)
    {
        //CMWAP·½Ê½Ö÷»úÓòÃûÓÉ´úÀí·þÎñÆ÷½âÎö
    ...
    {
        kal_prompt_trace(MOD_SOCKET, "[Lima_Soc_Send_Get_Req] LIMA_SOC_CREATE_ERR");
    }
}
...
static BOOL Lima_Soc_Handle_Location_Url()
{
    ...
    begin_offset=strstr(app_strtolower(g_lima_gp_socket->rcvd_buffer),"location:");  //ÕÒµ½locationÎ»ÖÃ

    if (begin_offset!=NULL)
    {
        end_offset=strstr(app_strtolower(begin_offset),CRLF);  //ÕÒµ½locationÎ»ÖÃ
        if (end_offset==NULL)
...
void Bird_soc_rev_command_deal(U8 *p_receive_data)
{
    ...
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_rev_command_deal  %s", p_receive_data);
    p2 = p_receive_data;
    psplit = strchr(p2, ',');
    while(psplit!=NULL)
    {
    ...
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_rev_command_deal str1 %s", str1);
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_rev_command_deal str2 %s", str2);
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_rev_command_deal str3 %s", str3);
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_rev_command_deal str4 %s", str4);
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_rev_command_deal str5 %s", str5);
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_rev_command_deal str6 %s", str6);
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_rev_command_deal str7 %s", str7);
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_rev_command_deal str8 %s", str8);

    kal_prompt_trace(MOD_SOCKET,"Bird_soc_rev_command_deal COMMAND:%s",str2);
    if(strcmp(app_strtolower(str2), "s1") == 0)      /* login */
    {
        count_judge_gps_send_timer = 0; // ·¢ËÍ±êÖ¾ÇåÁã
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_rev_command_deal deal login");
    ...
    {
        bird_set_defense_res(2); //ÓÀ¾ÃÐÔÉè·À
        count_judge_gps_send_timer = 0;
    }
#endif
    ...
    {
        bird_soc_cq_res();
        yd_send_reboot_control_msg();/*ÖØÆô*/
    }
    else if(strcmp(app_strtolower(str2), "s10") == 0)      /* dw */
    {
        bird_soc_dealdw(str3,str4);/*¶¨Î»*/
    }
    ...
    else if(strcmp(app_strtolower(str2), "s15") == 0)      /* Ëø³µ/½âËø */
    {
#ifdef BIRD_ECU_MMI_SUPPORT
...
void Bird_soc_rev_deal()
{
    ...
    ngetbgid = 0;
    ngetendid = 0;
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_rev_deal rev index %d  %d",g_socket_rev.g_n_rev_index,g_socket_rev.g_n_rev_indexEnd);

    //for(zy_i=g_socket_rev.g_n_rev_index;zy_i<g_socket_rev.g_n_rev_indexEnd;zy_i++)
    //	{
    //		rev_index = (zy_i % RECEIVE_DATA_LENGTH);
    //		kal_prompt_trace(MOD_SOCKET," %x ",g_socket_rev.rev_buf[rev_index]);
    //	}
    ...
    for(zy_index=g_socket_rev.g_n_rev_index; zy_index<g_socket_rev.g_n_rev_indexEnd; zy_index++)
    {
        ...
        if(ngetbg==1 && ngetend==1)
        {
            ...
            kal_prompt_trace(MOD_SOCKET,"rev index get hd %d  %d",g_socket_rev.g_n_rev_index,g_socket_rev.g_n_rev_indexEnd);
    ...
    kal_prompt_trace(MOD_SOCKET,"rev end index %d  %d",g_socket_rev.g_n_rev_index,g_socket_rev.g_n_rev_indexEnd);
}
...
void Bird_soc_tbox_rev_command_deal(U8 *p_receive_data, U32 receive_length)
{
    ...
    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_tbox_rev_command_deal %d", receive_length);
	...
	for (i = 0; i < receive_length - 1; i++)
		check_code ^= p_receive_data[i];
	if(check_code != p_receive_data[receive_length - 1])
	{
	    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_tbox_rev_command_deal checksum error %x,%x", check_code,p_receive_data[receive_length - 1]);	
		return;
	}
    ...
    {
        kal_prompt_trace(  MOD_SOCKET,"Bird_soc_tbox_rev_command_deal vin not correct");
        return;
    }

    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_tbox_rev_command_deal command %d %d", command[0],command[1]);

    if((command[0]==0x07)&&(command[1]==0x01))
    {
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal heart");
        Rj_stop_timer(BIRD_TASK_HEART_WAIT_TIMER);
        count_judge_gps_send_timer = 0; //·¢ËÍ±êÖ¾ÇåÁã
        return;
    }

    kal_prompt_trace(  MOD_SOCKET,"Bird_soc_tbox_rev_command_deal %d %d", receive_length-length,data_len);
    ...
    {
        count_judge_gps_send_timer = 0; // ·¢ËÍ±êÖ¾ÇåÁã
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal login");
        Bird_soc_set_loginflag();
        Yd_tboxlogin_res(rest_buf,data_len);
    }
    else if((command[0]==0x04)&&(command[1]==0x01))
    {
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal logout");
        count_judge_gps_send_timer = 0; //·¢ËÍ±êÖ¾ÇåÁã
    }
    else if((command[0]==0x08)&&(command[1]==0x01))
    {
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal timing");
        count_judge_gps_send_timer = 0; //·¢ËÍ±êÖ¾ÇåÁã
        Yd_calib_res(rest_buf,data_len);
    }
    else if((command[0]==0x02)&&(command[1]==0x01)) // ÊµÊ±ÐÅÏ¢ÕýÈ·Ó¦´ð
    {
        ...
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal realinfo %d %d %d %d %d %d",current_time.nYear,current_time.nMonth,
	...
	else if((command[0]==0x02)&&(command[1]==0x02)) // ÊµÊ±ÐÅÏ¢´íÎóÓ¦´ð
	{
        applib_time_struct current_time = {0};
        memcpy(time_rec, rest_buf,6);
        txboxdecimal(time,time_rec);
        ...
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal realinfo %d %d %d %d %d %d",current_time.nYear,current_time.nMonth,
                         current_time.nDay,current_time.nHour,current_time.nMin,current_time.nSec);
        if(applib_dt_is_valid(&current_time))
            Bird_soc_setreduceflag_posfile(current_time,TRUE);
        if(g_third_alarm_flag==0)
        {
            //StopTimer(BIRD_READ_FILE);
            //StartTimer(BIRD_READ_FILE, 1000, Yd_readposfile);
    ...
    else if((command[0]==0x03)&&(command[1]==0x01))
    {
        ...
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal resendinfo %d %d %d %d %d %d",current_time.nYear,current_time.nMonth,
                         current_time.nDay,current_time.nHour,current_time.nMin,current_time.nSec);
        if(applib_dt_is_valid(&current_time))
            Bird_soc_setflag_posfile(current_time);
        if(g_third_alarm_flag==0)
    ...
    else if((command[0]==0x03)&&(command[1]==0x02)) //´íÎóÓ¦´ð
    {
        applib_time_struct current_time = {0};
        memcpy(time_rec, rest_buf,6);
        txboxdecimal(time,time_rec);
    ...
    {
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal search");
        bird_tbox_search_param_res(rest_buf,data_len);
        count_judge_gps_send_timer = 0; //·¢ËÍ±êÖ¾ÇåÁã
    }
    else if((command[0]==0x81)&&(command[1]==0xfe))
    {
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal set");
        bird_tbox_set_param_res(rest_buf,data_len);
        count_judge_gps_send_timer = 0; //·¢ËÍ±êÖ¾ÇåÁã
    }
    else if((command[0]==0x82)&&(command[1]==0xfe))
    {
        kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_command_deal control");
...
void Bird_soc_tbox_rev_deal()
{
    ...
    kal_prompt_trace(MOD_SOCKET,"Bird_soc_tbox_rev_deal rev index %d  %d",g_socket_rev.g_n_rev_index,g_socket_rev.g_n_rev_indexEnd);
    for(zy_index=g_socket_rev.g_n_rev_index; zy_index<g_socket_rev.g_n_rev_indexEnd; zy_index++)
    {
        rev_index = (zy_index % RECEIVE_DATA_LENGTH);
...
static U8 Lima_Soc_Write_Recv_buf(lima_soc_transaction_struct *p_socket)
{
    S32  handle;
    U32  len;
    S8   *pBegin = NULL; //lrf //Ð´ÈëÄÚÈÝ¿ªÊ¼
    U32  wtLen;//Ð´ÈëÄÚÈÝ³¤¶È
    S32  offset;//ÎÄ¼þ¶ÁÐ´Ö¸ÕëÆ«ÒÆÁ¿
    BOOL bRet;
    S32  file_ret;
    S8 filename[64]= {0};
...
#ifdef LIMA_SOC_CONTINUE_DL//¶ÏµãÐø´«
...
#endif
    {
	    ...
	    //rmmi_write_to_uart((kal_uint8*)"recived data:", strlen("recived data:"), KAL_TRUE);
        ...
        for(zy_i=0; zy_i<wtLen; zy_i++)
        {
			...
			//rmmi_write_to_uart((kal_uint8*)g_socket_rev.rev_buf[rev_index], 1, KAL_FALSE);
            g_socket_rev.g_n_rev_indexEnd++;
        }
        kal_prompt_trace(MOD_SOCKET,"Lima_Soc_Write_Recv_buf rev index %d  %d",g_socket_rev.g_n_rev_index,g_socket_rev.g_n_rev_indexEnd);
        if(g_gettime_flag==0)
            Bird_soc_rev_deal();
        else
...
Functions        :        ½«16½øÖÆµÄÊý×Ö×Ö·û´®×ª»¯ÎªÊý×Ö(²»´¦Àí0xÍ·)
Parameters        :    1
    input        :    hex
    output        :    NONE
    return        :    NONE
...
kal_uint32 HTTP_GetContentLength(kal_int8 *revbuf)
{
    ...
    kal_uint32 HTTP_Body = 0;//ÄÚÈÝÌå³¤¶È
...
kal_uint32  HTTP_GetRecvLength(kal_int8 *revbuf)
{
    kal_int8 *p1 = NULL;
    kal_uint32 HTTP_Body = 0;//ÄÚÈÝÌå³¤¶È
    kal_uint32 HTTP_Head = 0;//HTTP Ð­ÒéÍ·³¤¶È
...
static U16 HTTP_Get_Rsp_Status(S8 *chlwr)
{
    ...
    status = atoi(pCh);
    kal_prompt_trace(MOD_SOCKET,"HTTP_Get_Rsp_Status =%d",status);
    return status;
...
/*
¹¦ÄÜ:»ñÈ¡ÏÂÔØurlÖÐµÄÎÄ¼þÃû,×îºóÒ»¸ö/ºóµÄ×Ö·û
²ÎÊý:
·µ»ØÖµ:
0-----------³É¹¦
-1----------Ê§°Ü
...
/*
¹¦ÄÜ:»ñÈ¡ÏÂÔØurlÖÐµÄÂ·¾¶,µÚÒ»¸ö/ºóµÄ×Ö·û
²ÎÊý:
·µ»ØÖµ:
0-----------³É¹¦
-1----------Ê§°Ü
...
/*
¹¦ÄÜ:»ñÈ¡ÏÂÔØurlÖÐµÄipºÍport,ipÖ§³ÖÓòÃû,¶Ë¿ÚÄ¬ÈÏÎª80
²ÎÊý:
·µ»ØÖµ:
1-----------ÓòÃûÊ½
2-----------ip portÊ½
...
void BD_socket_create(U8 * url)
{
    ...
    kal_uint8 val_async;                            //ÉèÖÃsocketÎªÒì²½µÄvalÖµ
    kal_bool val_nonblocking=KAL_TRUE;                //ÉèÖÃsocketÎª·Ç×èÈûµÄvalÖµ
    ...
    kal_prompt_trace(MOD_SOCKET," BD_socket_create");
    ...
    kal_prompt_trace(MOD_SOCKET," HTTP_Get_IP_PORT");

    recv_buf = NULL;
    if (NULL == g_lima_gp_soc_down_pool)
    {
        kal_prompt_trace(MOD_SOCKET,"g_lima_gp_soc_down_pool NULL host,port=%s,%s",g_host,g_port);
    ...
    {
        kal_prompt_trace(MOD_SOCKET,"g_lima_gp_soc_down_pool host,port=%s,%s",g_host,g_port);
        recv_buf       = kal_adm_alloc(g_lima_gp_soc_down_adm_id, MAX_BIRD_RCV_BUFFER_SIZE);
    }

    bird_get_nw_plmn();
    kal_prompt_trace(MOD_SOCKET,"BD_socket_create host,port=%s,%s",g_host,g_port);
    memset(&acct_id_query,0,sizeof(acct_id_query));
    acct_id_query.qry_info.filters = SRV_DTCNT_STORE_QRY_TYPE_ACC_TYPE;
    acct_id_query.qry_info.acc_type_info = SRV_DTCNT_PROF_TYPE_FACTORY_CONF;
        ...
        else
        {
            ...
            if (result == SRV_DTCNT_RESULT_SUCCESS)
            {
                ...
                else
                {
                    ...
                    //reboot
                    {
                        kal_prompt_trace(MOD_SOCKET,"BD_socket_create reboot");
                        RJ_SYS_ReStart();
                    }
                }
            }
    ...
    if(acct_id_query.num_ids>0)
    {
        ...
        if (0 == downapp_id)
        {
            ...
            memset(&info,0,sizeof(info));                    //³õÊ¼»¯info
            info.app_str_id = 999;            //×Ô¼º¶¨ÒåµÄ×Ö´®
            info.app_icon_id = 0;            //×Ô¼º¶¨ÒåµÄicon
            info.app_type = DTCNT_APPTYPE_EMAIL; //Í¨¹ýsmartÔ­Ôò£¬Æ¥ÅäapnÎªcmnetµÄdata account
            cbm_register_app_id_with_app_info(&info,&downapp_id);        //×¢²áapp id
        }
    ...
    //×¢²áÒì²½µÄsocket optionÑ¡ÏîÏà¹ØµÄnotify´¦Àíº¯Êý
    ...
    socket_id = soc_create(SOC_PF_INET,SOC_SOCK_STREAM,0,MOD_MMI, account_id);//´´½¨socket id
    if(socket_id < 0)
    {
        kal_prompt_trace(MOD_SOCKET," BD_socket_create soc_create  fail");
        BD_socket_close();
        return ;
    }
    kal_prompt_trace(MOD_SOCKET,"BD_socket_create appid,accountid,socket=%d,%d,%d",downapp_id,account_id,socket_id);
    if(soc_setsockopt(socket_id,SOC_NBIO, &val_nonblocking,sizeof(val_nonblocking))<0)//ÉèÖÃsocketÑ¡ÏîÎª·Ç×èÈû
    {
        kal_prompt_trace(MOD_SOCKET," soc_setsockopt  SOC_NBIO fail");
        BD_socket_close();
        return ;
    }
    kal_prompt_trace(MOD_SOCKET," soc_setsockopt SOC_ASYNC enter");
    val_async = SOC_WRITE | SOC_READ| SOC_CONNECT | SOC_CLOSE;//ÉèÖÃsocketÑ¡ÏîÎªÒì²½
    if(soc_setsockopt(socket_id,SOC_ASYNC, & val_async,sizeof(val_async))<0)
    {
        kal_prompt_trace(MOD_SOCKET," soc_setsockopt SOC_ASYNC fail");
        BD_socket_close();
        return ;
    }
    kal_prompt_trace(MOD_SOCKET," soc_gethostbyname enter");
    ret_domain=soc_gethostbyname(KAL_FALSE,MOD_MMI,sAgpsnReq,g_host,recvaddr,&addr_len,0, account_id);                 //¶ÔÄ¿±êÓòÃû½øÐÐ½âÎö£¬»ñµÃÄ¿±êserverµÄip¡£
    kal_prompt_trace(MOD_SOCKET, "[soc_gethostbyname] ret_domain = %d", ret_domain);
    if(ret_domain == SOC_SUCCESS)
    {
        kal_prompt_trace(MOD_SOCKET," soc_gethostbyname SOC_SUCCESS");
        memcpy(g_bd_ip.addr,recvaddr,addr_len); //½«ÓòÃû½âÎöµÃµ½µÄip±£´æµ½½á¹¹ÌåÖÐ
        g_bd_ip.addr_len = addr_len;            //µØÖ·³¤¶È
        g_bd_ip.port = 80;                    //¶Ë¿Ú
        g_bd_ip.sock_type = SOC_SOCK_STREAM;        //socket typeÎªtcp
        ret_connect = soc_connect(socket_id,&g_bd_ip);    //Óë·þÎñÆ÷½¨Á¢Á¬½Ó
        if(ret_connect == SOC_SUCCESS)
        {
            BD_socket_send();                            //Á¬½Ó³É¹¦ËÍhttp request
        }
        else if(ret_connect != SOC_WOULDBLOCK)            //Á¬½ÓÊ§°Ü
        {
            kal_prompt_trace(MOD_SOCKET,"connect fail");
            BD_socket_close();
            return ;
        }
    }
    else if(ret_domain != SOC_WOULDBLOCK)                //ÓòÃû½âÎöÊ§°Ü
    {
        kal_prompt_trace(MOD_SOCKET," soc_gethostbyname fail");
        BD_socket_close();
        return ;
    }
}
...
void BD_socket_send()
{
    ...
    kal_prompt_trace(MOD_SOCKET," BD_socket_send ");
    ...
    kal_prompt_trace(MOD_SOCKET,"BD_socket_send buf= %s",send_buf);
    ret_send=soc_send(socket_id, send_buf,strlen(send_buf)+1,0);
    kal_prompt_trace(MOD_SOCKET,"BD_socket_send SEND= %d",ret_send);
    if(ret_send > 0)
    {
        BD_socket_receive();
    }
...
void BD_socket_receive()
{
    ...
    kal_prompt_trace(MOD_SOCKET," BD_socket_receive ");
    ret_recv = soc_recv(socket_id,(kal_uint8*)recv_buf,MAX_LIMA_RCV_BUFFER_SIZE,0);
    kal_prompt_trace(MOD_SOCKET,"BD_socket_receive ret_recv %d",ret_recv);

    if(ret_recv > 0)
    {
#if 0//add by zyz
        uart_sendbuf[1] = 0;
        for(j = 0; j < ret_recv; j++)
        {
            if(recv_buf[j] >= 0x20&&recv_buf[j]<0x80)
            {
                uart_sendbuf[0] = recv_buf[j];
                kal_prompt_trace(MOD_SOCKET, "%s", uart_sendbuf);
            }
            else
                kal_prompt_trace(MOD_SOCKET, "?");
        }
        ...
        if(g_is_need_continue==0)
        {
            ...
            {
                   kal_prompt_trace(MOD_SOCKET,"HTTP_is_Content_type_file FALSE");
            	return;
            }*/
            g_is_need_continue=1;
            g_body_len = HTTP_GetContentLength(recv_buf);
            kal_prompt_trace(MOD_SOCKET,"BD_socket_receive g_body_len %d",g_body_len);
            memset(g_revbuf,0,sizeof(g_revbuf));
            p = strstr(recv_buf,"\r\n\r\n");
            if(p == NULL)//jia ru duan dian baocun
            {
                kal_prompt_trace(MOD_SOCKET,"BD_socket_receive success NOT FOUND");
            }
            else
            {
                ...
                kal_prompt_trace(MOD_SOCKET,"BD_socket_receive g_already_rev=%d,g_getapsnnlen=%d",g_already_rev,g_getapsnnlen);
            }
        ...
        if( (g_already_rev == g_body_len) && (g_already_rev>0) )
        {
            ...
            kal_prompt_trace(MOD_SOCKET,"BD_socket_receive success end isneed,body,once,alreay, %d,%d,%d,%d",g_is_need_continue,g_body_len,ret_recv,g_already_rev);
            ...
            kal_prompt_trace(MOD_SOCKET,"BD_UART_Write success  g_uart_sendlen %d",g_uart_sendlen);
            if (g_uart_sendlen <g_already_rev)
            {
                ...
                kal_prompt_trace(MOD_SOCKET,"BD_UART_Write success writelen =%d, return tmplen %d",g_already_rev-g_uart_sendlen,tmplen);
                g_uart_sendlen =  g_uart_sendlen + tmplen;
                kal_prompt_trace(MOD_SOCKET,"BD_UART_Write success end g_uart_sendlen %d",g_uart_sendlen);
            }
#else
            ...
            /*
            kal_sleep_task(400);       //must add
            kal_prompt_trace(MOD_SOCKET,"BD_socket_receive success2 end isneed,body,once,alreay %d,%d,%d,%d",g_is_need_continue,g_body_len,ret_recv,g_already_rev);
                  // write to uart apgs data
            memset(uart_sendbuf,0,sizeof(uart_sendbuf));
            memcpy(uart_sendbuf,g_revbuf,2048);
               //g_uart_sendlen = BD_UART_Write(g_revbuf,g_already_rev);
            g_uart_sendlen = BD_UART_Write(uart_sendbuf,2048);
            kal_prompt_trace(MOD_SOCKET,"BD_UART_Write success2  g_uart_sendlen %d",g_uart_sendlen);
            if (g_uart_sendlen <g_already_rev)
            {
                       kal_sleep_task(400);       //must add
                 memset(uart_sendbuf,0,sizeof(uart_sendbuf));
            memcpy(uart_sendbuf,g_revbuf+g_uart_sendlen,g_already_rev-g_uart_sendlen);
                // memcpy(g_revbuf,g_revbuf+g_uart_sendlen,g_already_rev-g_uart_sendlen);
                 //tmplen = BD_UART_Write(g_revbuf,g_already_rev-g_uart_sendlen);
                 tmplen = BD_UART_Write(uart_sendbuf,g_already_rev-g_uart_sendlen);
            kal_prompt_trace(MOD_SOCKET,"BD_UART_Write success2 writelen =%d, return tmplen %d",g_already_rev-g_uart_sendlen,tmplen);
                 g_uart_sendlen =  g_uart_sendlen + tmplen;
                 kal_prompt_trace(MOD_SOCKET,"BD_UART_Write success2 end g_uart_sendlen %d",g_uart_sendlen);
            }	*/
        }
    }
...
static MMI_BOOL BD_socket_notify (void* msg_ptr)
{
    ...
    kal_int8 ret_connect=SOC_ERROR;
    kal_prompt_trace(MOD_SOCKET, "[BD_socket_notify]Entry");
    if(bd_soc_notify->socket_id != socket_id)
    {
        kal_prompt_trace(MOD_SOCKET, "[BD_socket_notify] socket_id != socket_id fail");
        return MMI_FALSE;
    }
    kal_prompt_trace(MOD_SOCKET, "[BD_socket_notify]deal");
    ...
    case SOC_WRITE:
        ...
        break;
    case SOC_CONNECT:
        kal_prompt_trace(MOD_SOCKET, "[BD_socket_notify] SOC_CONNECT enter");
        if(bd_soc_notify->result == KAL_FALSE)
        {
            ret_connect = soc_connect(socket_id,&g_bd_ip);    //Óë·þÎñÆ÷½¨Á¢Á¬½Ó
            if(ret_connect == SOC_SUCCESS)
            {
                kal_prompt_trace(MOD_SOCKET, "[BD_socket_notify] soc_connect SOC_SUCCESS");
                BD_socket_send();                        //Á¬½Ó³É¹¦ËÍhttp request
            }
            else if(ret_connect != SOC_WOULDBLOCK)            //Á¬½ÓÊ§°Ü
            {
                kal_prompt_trace(MOD_SOCKET, "[BD_socket_notify] soc_connect != SOC_WOULDBLOCK");
                BD_socket_close();
            }
        ...
        {
            kal_prompt_trace(MOD_SOCKET, "[BD_socket_notify] bd_soc_notify->result == KAL_TRUE");
            BD_socket_send();
        }
        break;
    case SOC_CLOSE:
        BD_socket_close();
        kal_prompt_trace(MOD_SOCKET, "[BD_socket_notify] SOC_CLOSE");
        break;
    }
    return MMI_TRUE;
}
...
static MMI_BOOL BD_socket_get_host_by_name(void *msg_ptr)
{
    ...
    kal_prompt_trace(MOD_SOCKET, "[BD_socket_get_host_by_name] entry");
    if(dns_ind->result == KAL_TRUE)
    {
        if(dns_ind->request_id==(kal_int32)AGPSREQID)
        {
            ...
            kal_prompt_trace(MOD_SOCKET, "[BD_socket_get_host_by_name]Http send request to %d.%d.%d.%d and port: %d",
                             g_bd_ip.addr[0],
                             g_bd_ip.addr[1],
                             g_bd_ip.addr[2],
                             g_bd_ip.addr[3],
            ...
            kal_prompt_trace(MOD_SOCKET, "[BD_socket_get_host_by_name] soc_connect ret_connect= %d", ret_connect);
            if(ret_connect == SOC_SUCCESS)
            {
                kal_prompt_trace(MOD_SOCKET, "[BD_socket_get_host_by_name]soc_connect SOC_SUCCESS ");
                BD_socket_send();
            }
            else if(ret_connect != SOC_WOULDBLOCK)
            {
                kal_prompt_trace(MOD_SOCKET, "[BD_socket_get_host_by_name]soc_connect !SOC_WOULDBLOCK ");
                BD_socket_close();
            }
        }
        else
    ...
    {
        kal_prompt_trace(MOD_SOCKET, "[BD_socket_get_host_by_name]soc_connect KAL_FALSE ");
        BD_socket_close();
    }
    return MMI_TRUE;
}
...
static void Bird_Log2(U8 n_log_type)
{
    if(0)
    {
        ...
        S32                 hFile;
        kal_prompt_trace(MOD_SOCKET,"Bird_Log2 entry");
        ...
        kal_prompt_trace(MOD_SOCKET,"Bird_Log data %s",p_soc.Log_buffer);
        if(n_log_type==1)
        {
            /*CONN*/
