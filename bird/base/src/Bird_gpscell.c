...
{
    U8 ret = 0;
    //kal_prompt_trace( MOD_GPS,"___RJ_GPS_is_working num ,valid =%d,%d",rj_position.satellite_num,g_n_saltllite_snr_isvalid);
...
{
    kal_prompt_trace( MOD_GPS,"[RJ_GPS_Gps_Init] ");
...
S8 bird_get_gsm_strength()
{
    ...
    kal_prompt_trace(MOD_GPS,"bird_get_gsm_strength %d,%d",bird_sim_signal_strength_in_percentage,num);
    if(num>9)
    {
        num = 9;
    }
...
S8 bird_get_bat_num()
{
    ...
    kal_prompt_trace(MOD_GPS,"bird_get_bat_num %d",num);
    if(num>9)
    {
        num = 9;
    }
...
void bird_set_gpspositon(S8 updatekind)
{
    ...
    kal_prompt_trace(MOD_GPS,"gps_position.ishavedate =%d",gps_position.ishavedate);
}
...
void bird_ini_gpsposition()
{
    ...
    {
        //kal_prompt_trace(MOD_GPS,"bird_ini_gpsposition no position");
    ...
    {
        kal_prompt_trace(MOD_GPS,"bird_ini_gpsposition have position");
        memset(&gps_position, 0, sizeof(gps_position));
        ...
        kal_prompt_trace(MOD_GPS, "bird_ini_gpsposition la=%s", doublechar);
        memset(doublechar,0,sizeof(doublechar));
        sprintf(doublechar,"%f", gps_position.lg);
        kal_prompt_trace(MOD_GPS, "bird_ini_gpsposition lg=%s", doublechar);
...
void BIRD_Gps_restart(void)
{
    ...
    kal_prompt_trace( MOD_GPS,"BIRD_Gps_restart");
    {
...
//S8 fjchar2[16];
    ...
    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback enter");
    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback gga_data->satellites =%d",gga_data->sat_in_view);
    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback gga_data->quality  =%d",gga_data->quality);
    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback gga_data =%d %d",gga_data->east_west,gga_data->north_south);
    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback gga_data->latitude  =%d %d",(U16)gga_data->latitude,gps_remove_timer);
    ...
    if(RJ_GPS_is_working())
    {
        ...
        Bird_Log_print(MOD_GPS, "lg = %s,sa=%d", doublechar, gga_data->sat_in_view);
        memset(doublechar,0,sizeof(doublechar));
        sprintf(doublechar,"%f", gga_data->latitude);
        Bird_Log_print(MOD_GPS, "la = %s,ql=%d", doublechar, gga_data->quality);
#endif
    ...
    {
        gps_restart_timer++;
        kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback gps_restart_timer=%d",gps_restart_timer);

        if(gps_restart_timer==120)
            StartTimer(BIRD_GPS_RESTART_TIMER, 100, BIRD_Gps_restart);

    }
    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback g_n_saltllite_snr_isvalid=%d,rj_position.satellite_num= %d",g_n_saltllite_snr_isvalid,rj_position.satellite_num);
    g_gpscheck=1;
    g_pos_data[0]=0x00;
    ...
    if(RJ_GPS_is_working())
    {
        ...
        kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback g_pos_data %d,%d",rj_position.north_south,rj_position.east_west);
    }
    ...
    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback g_pos_data %d,%d,%d,%d,%d",(U32)rj_position.lg,g_pos_data[1],
    ...
    //kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback quality,cur %d,%s",gga_data->quality,fjchar1);

    /*
    memset(fjchar2,0,sizeof(fjchar2));
    sprintf(fjchar2,"%f",gga_data->longitude);

    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gga_callback %s,%s",fjchar1,fjchar2);
    */
    /*
    if(!b_GPS_led)
...
static void RJ_GPS_nmea_gll_callback(mdi_gps_nmea_gll_struct *param)
{
    ...
    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_gll_callback %d",gll_data->status);
...
static void RJ_GPS_nmea_rmc_callback(mdi_gps_nmea_rmc_struct *param)
{
    ...
    U32 dis =0;
    kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_rmc_callback status,navstatus: %d,%d",rmc_data->status,rmc_data->nav_status);
    /*
...
static void RJ_GPS_nmea_vtg_callback(mdi_gps_nmea_vtg_struct *param)
{
    ...
    if(vtg_data->hspeed_km < 300&&RJ_GPS_is_working())
    {
        ...
        kal_prompt_trace(MOD_GPS," RJ_GPS_nmea_vtg_callback %d",bird_get_corner_value());
#if 1
        if(ncorner>0)
        {
    ...
    else
    {
        /* 卫星数为0，啥也不干 */
        //alarm_speed_times = 0;
        kal_prompt_trace( MOD_GPS,"satellite_num is 0");
    }
}
...
static void RJ_GPS_nmea_gsv_callback(mdi_gps_nmea_gsv_struct *param)
{
    ...
    */
    ...
    kal_prompt_trace( MOD_GPS, "RJ_GPS_nmea_gsv_callback SNR=%s,x=%d,snrsalnum=%d,num_sv_trk=%d", gpsinfo,RJ_GPS_get_satellite_num(),gsv_data->sates_in_view,gsv_data->num_sv_trk);
    //nsnryxnum1 = n;
    //nsnryxnum2 = n2;
...
void RJ_GPS_Callback(mdi_gps_parser_info_enum type, void *buffer, U32 length)
{
    ...
    case MDI_GPS_PARSER_MA_STATUS:
        ...
        kal_prompt_trace( MOD_GPS,"MDI_GPS_PARSER_MA_STATUS");
...
{
    kal_prompt_trace( MOD_GPS,"Bird_SleepDisable gps,acc,flag = %d,%d,%d",m_gps_status,b_acc_gpio_status,gps_bird_disable_sleep_status);
...
{
    kal_prompt_trace( MOD_GPS,"Bird_SleepEnable gps,acc,flag = %d,%d,%d",m_gps_status,b_acc_gpio_status,gps_bird_disable_sleep_status);
...
#if BIRD_GPS_DEBUG
    Bird_Log_print(MOD_GPS, "RJ_GPS_Enter");
#endif
    RJ_GPS_SetGpsModule(RJ_GPS_STATUS_ON);
...
#if BIRD_GPS_DEBUG
    Bird_Log_print(MOD_GPS, "RJ_GPS_Stop");
#endif
...
{
    kal_prompt_trace( MOD_GPS,"BIRD_Gps_Stop");
