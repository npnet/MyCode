...
void checksum_test()
{
	...
	kal_prompt_trace(MOD_SOC, "the checksum result is temp3 = %x, ", temp3);
}
...
void BIRD_GPS_Write_Pos_Data(void)
{
...
#if BIRD_AGPS_DEBUG			
	kal_prompt_trace(MOD_SOC, "BIRD_GPS_Write_Pos_Data checksum = %x", checksum);
#endif
	...
	{
		kal_prompt_trace(MOD_SOC, "agps_aid_pos_buf[%d] = %x", i, agps_aid_pos_buf[i]);
	}
#endif	
}
...
void BIRD_GPS_Write_Postime_Data(void)
{
...
#if BIRD_AGPS_DEBUG		
	kal_prompt_trace(MOD_SOC, "dt is[%02d-%02d %02d:%02d] however utc_time is [%02d-%02d %02d:%02d]", dt.nMonth,dt.nDay,dt.nHour, dt.nMin, utc_time.nMonth,utc_time.nDay,utc_time.nHour, utc_time.nMin);
#endif
...
#if BIRD_AGPS_DEBUG			
	kal_prompt_trace(MOD_SOC, "BIRD_GPS_Write_Postime_Data checksum = %x", checksum);
#endif
	...
	{
		kal_prompt_trace(MOD_SOC, "agps_aid_postime_buf[%d] = %x", i, agps_aid_postime_buf[i]);
	}
#endif	
}
#endif
...
{
 	S8 agps_cold_start_buf[9] = {0xF1, 0xD9, 0x06, 0x40, 0x01, 0x00, 0x01, 0x48, 0x22};
	kal_prompt_trace(MOD_SOC, "BIRD_GPS_Cold_Start_First");
...
{
	kal_prompt_trace(MOD_SOC, "bird_gps_write_peph_data_timeout_ex");
...
void bird_write_peph_data_ex(peph_op_struct *peph_op)
{
	...
	for (k = peph_op->index; k < peph_op->agps_len; k++){
		if ((peph_op->agps_buf[k] == 0xF1) && (peph_op->agps_buf[k+1] == 0xD9)){
			...
			if (count == 1){
				pos1 = k;
				kal_prompt_trace(MOD_SOC, "bird_write_peph_data_ex pos1 = %d", k);
			}	
			else {
				pos2 = k;
				kal_prompt_trace(MOD_SOC, "bird_write_peph_data_ex pos2 = %d", k);
				break;
			}
		}
	}
...
void bird_socket_write_agps_data(void)
{
	...
	kal_prompt_trace(MOD_SOC, "bird_socket_write_agps_data total_item = %d", agps_peph_op.total_item);
}
...
void RJ_GET_AGPS(peph_op_struct *peph_op)
{
	...
	if (gps_close_location_flag == KAL_FALSE){
		...
		kal_prompt_trace(MOD_SOC, "gps_close_location_flag false");
	}	
	else
	{
		kal_prompt_trace(MOD_SOC, "gps_close_location_flag true");
		...
		{
			kal_prompt_trace(MOD_SOC, "gps_close_time > 60m");
			HD8020_AGPS(&agps_peph_op);
		}
	}
}
...
#if BIRD_AGPS_DEBUG			
			kal_prompt_trace(MOD_SOC, "agps addr=%s", BD_AGNSS_AGPS_1);
#endif
		}
		else
		{
...
#if BIRD_AGPS_DEBUG					
			kal_prompt_trace(MOD_SOC, "agps addr=%s", bird_get_agps_address());
#endif
		}
	} 
	else
...
void bird_agps_ack_handle(kal_uint8 *Buffaddr, kal_uint16 Length)
{
	...
	if (nmea_find != 0) {
		...
		kal_prompt_trace(MOD_SOC, "agps_peph_op.peph_ack_ok_num = %d", agps_peph_op.peph_ack_ok_num);
		bird_deal_send_msg_handler	(MSG_ID_YD_TK001_MSG_WRITE_PEPH, 0, 0);	
		StopTimer(BIRD_AGPS_WRITE_TIMEOUT_TIMER);
...
void bird_get_agps_pos(kal_uint32 *lati, kal_uint32 *logi, kal_uint32 *alti)
{
	...
	*lati = yd_tk001_info.agps_pos_lati;
	...
	for(i = 0; i<4; i++)
		kal_prompt_trace(MOD_SOC, "bird_get_agps_pos pos_value[%d] = %x", i, pos_value[i]);	
#endif	
...
void bird_get_agps_llapos(kal_uint32 *lati, kal_uint32 *logi, kal_uint32 *alti)
{
	...
	*lati = yd_tk001_info.agps_llapos_lati;
	...
	memcpy(pos_value, lati, 4);
	for(i = 0; i<4; i++)
		kal_prompt_trace(MOD_SOC, "bird_get_agps_llapos pos_value[%d] = %x", i, pos_value[i]);	
#endif	
