...
#else
    GDI_TRACE(GDI_TRC_PAIRED_LAYER, GDI_TRC_LAYER_CREATE,
        id, handle, cf,offset_x,offset_y,width,height,is_double_layer,buf_size,buf_ptr,buf_ptr1, 0);
#endif
            ...
            S32 outside_memory_size)
{
    ...
    *handle_ptr = handle = gdi_layer_find_valid_handle();
    GDI_ASSERT(handle);
    {
        ...
        gdi_layer_create_internal(handle, offset_x, offset_y, width, height, cf, FALSE, buf, NULL, outside_memory_size
                            #ifdef GDI_DEBUG_SUPPORT
                                , (void*)creator_lr
                            #endif
                                );
...
GDI_RESULT gdi_layer_test_set_clip(gdi_rect_struct *rect)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void gdi_rotate_map_layer_to_lcd(U8 rotate_value, S32 *x, S32 *y)
{
    ...
    if (GDI_LCD_LAYER_ROTATE_90 == rotate_value
        || GDI_LCD_LAYER_ROTATE_180 == rotate_value
        || GDI_LCD_LAYER_ROTATE_270 == rotate_value
        || GDI_LCD_LAYER_ROTATE_0_MIRROR == rotate_value
        || GDI_LCD_LAYER_ROTATE_90_MIRROR == rotate_value
        || GDI_LCD_LAYER_ROTATE_180_MIRROR == rotate_value
        || GDI_LCD_LAYER_ROTATE_270_MIRROR == rotate_value)
    {
        *x = tx;
        *y = ty;
    }
            ...
            )
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef defined(GDI_6_LAYERS) && defined(GDI_NB_BLT_SUPPORT)
...
#else
    GDI_TRACE(GDI_TRC_LAYER, GDI_LAYER_FLATTEN_ENTER, handle0, handle1, handle2, handle3, 0, 0,
        x1, y1, x2, y2, 0, 0);
    ...
    if (
        ...
        drv_fla_result != FLATTEN_RESULT_OK)
    {
#endif
            ...
            )
{
    ...
    kal_get_time(&start_time);
#if defined(GDI_NB_BLT_SUPPORT) && defined(GDI_6_LAYERS)
    GDI_TRACE(GDI_TRC_LAYER, GDI_LAYER_FLATTEN_ENTER, handle0, handle1, handle2, handle3, handle4, handle5,
        x1, y1, x2, y2, (kal_uint32)non_blocking_callback, start_time);
#else
    GDI_TRACE(GDI_TRC_LAYER, GDI_LAYER_FLATTEN_ENTER, handle0, handle1, handle2, handle3, 0, 0,
        x1, y1, x2, y2, 0, start_time);
#endif
    /* check flatten region */
    GDI_CLIP_TWO_RECT(x1, y1, x2, y2, 0, 0, output_buf_w - 1, output_buf_h - 1, GDI_DEBUG_ASSERT(0));
    ...
    for (i = GDI_LAYER_HW_LAYER_COUNT - 1; i >= 0; --i)
    {
        if (gdi_layer_config_element(GDI_LCD_V2_CTX_FLATTEN, i, temp_hw_layer_handle[i], output_buf_rotate_value, KAL_TRUE, output_buf_w, output_buf_h, output_buf_offset_x, output_buf_offset_y, KAL_FALSE))
        {
            bg_color = ((gdi_layer_struct*)temp_hw_layer_handle[i])->background;
        }
    }
...
/* under construction !*/
    ...
    #else
        WFCboolean wait_for_composition = WFC_TRUE;
    #endif /* GDI_NB_BLT_SUPPORT */
                    ...
                    )
{
    ...
    kal_get_time(&start_time);
#if defined(GDI_6_LAYERS) && defined(GDI_NB_BLT_SUPPORT)
...
#else
    GDI_TRACE(GDI_TRC_LAYER, GDI_LAYER_BLT_ENTER, handle0, handle1, handle2, handle3, 0, 0,
        x1, y1, x2, y2, 0, start_time);
#endif
    do
    {
        ...
        for (i = GDI_LAYER_HW_LAYER_COUNT - 1; i >= 0; --i)
        {
            if (gdi_layer_config_element(ctx_index, i, GDI_LCD->blt_handle[i], GDI_LCD->rotate_value, (kal_bool)GDI_LCD->rotate_by_layer, lcd_width, lcd_height, 0, 0, KAL_TRUE))
            {
                bg_color = ((gdi_layer_struct*)GDI_LCD->blt_handle[i])->background;
            }
        }
...
#else
    GDI_TRACE(GDI_TRC_LAYER, GDI_LAYER_SET_BLT_LAYER,//[gdi_layer_set_blt_layer_ext_internal]: H0:%x H1:%x H2:%x H3:%x H4:%x H5:%x
        handle0, handle1, handle2, handle3, 0, 0);
#endif
...
GDI_RESULT gdi_layer_set_blt_layer_previous(void)
{
    ...
    {
        result = gdi_layer_set_blt_layer(GDI_LCD->act_layer_handle, 0, 0, 0);
    }
...
GDI_RESULT gdi_layer_set_blt_layer(
             ...
             gdi_handle handle3)
{
    return gdi_layer_set_blt_layer_ext(
             handle0,
             handle1,
             handle2,
             handle3
...
{
    return gdi_layer_flatten_to_base_ext(
             src_handle0,
             src_handle1,
             src_handle2,
             src_handle3
...
GDI_RESULT gdi_image_draw_blend2layers(
             S32 x,
             S32 y,
             U8 *img_src)
{
...
GDI_RESULT gdi_image_draw_resized_blend2layers(
             S32 x,
             S32 y,
             S32 w,
             S32 h,
...
GDI_RESULT gdi_image_draw_mem_blend2layers(
             S32 x,
             S32 y,
             U8 *img_src,
             U8 img_type,
...
GDI_RESULT gdi_anim_draw(
             S32 x, S32 y,
             U8* img_src,
             gdi_handle * handle_ptr)
{
    return gdi_anim_codec_draw_internal(
             GDI_IMAGE_SRC_FROM_RESOURCE,
             img_src, GDI_IMAGE_TYPE_INVALID ,0,
             x, y,
             0, 0,
             0,
             handle_ptr,
...
GDI_RESULT gdi_anim_draw_resized(
             S32 x, S32 y,
             S32 w, S32 h,
             U8* img_src,
             gdi_handle * handle_ptr)
...
gdi_handle gdi_anim_nb_draw_resized_file(
                    ...
                    U8 *img_name)
{
    return gdi_image_nb_draw_internal(
             GDI_IMAGE_SRC_FROM_FILE,
             img_name, GDI_IMAGE_TYPE_INVALID, 0,
             x,
             y,
             w,
             h,
