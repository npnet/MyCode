...
#define MMI_CONN_TRC_FUNC            TRACE_FUNC
#define MMI_CONN_TRC_STATE           TRACE_STATE
#define MMI_CONN_TRC_INFO            TRACE_INFO
#define MMI_CONN_TRC_WARNING         TRACE_WARNING
#define MMI_CONN_TRC_ERROR           TRACE_ERROR
       
#define MMI_CONN_TRC_G1_DLAGENT      TRACE_GROUP_1
#define MMI_CONN_TRC_G2_SYNCML       TRACE_GROUP_2
#define MMI_CONN_TRC_G3_DM           TRACE_GROUP_3
#define MMI_CONN_TRC_G4_FOTA         TRACE_GROUP_4
#define MMI_CONN_TRC_G5_IRDA         TRACE_GROUP_5
#define MMI_CONN_TRC_G6_DTCNT        TRACE_GROUP_6
#define MMI_CONN_TRC_G6_CBM          TRACE_GROUP_6
#define MMI_CONN_TRC_G7_BT           TRACE_GROUP_7
#define MMI_CONN_TRC_G8_DCD	         TRACE_GROUP_8
#define MMI_CONN_TRC_G9_NTPD	     TRACE_GROUP_9
#define MMI_CONN_TRC_G10_TETHERING   TRACE_GROUP_10
#define MAX_CONN_TRACE_LOG_TYPE      (MMI_CONN_TRC_G10_RESERVED+1)
...
#endif
BEGIN_TRACE_MAP(MOD_MMI_CONN_APP)

    /* --------------------------------------------------------------- */
    /* ----------------------- MODULE NAME:ExtDevice ------------------ */
    /* --------------------------------------------------------------- */

    //IrdaMMI.c
    /* Irda MMI Init functions */
    TRC_MSG(MMI_IRDA_FUNC_INIT_CNTX, "[IrdaMMI.c] mmi_irda_init_cntx")
    TRC_MSG(MMI_IRDA_FUNC_INIT_PROTOCOL_EVENT, "[IrdaMMI.c] mmi_irda_init_protocol_events")

    /* Irda MMI Internal functions */
    TRC_MSG(MMI_IRDA_FUNC_OPEN_IR, "[IrdaMMI.c] mmi_irda_init_ir")
    //TRC_MSG(MMI_IRDA_FUNC_SHOW_SENDING_SCR, "[IrdaMMI.c] mmi_irda_show_sending_screen")
    TRC_MSG(MMI_IRDA_OBEX_FUNC_SND_ABORT, "[IrdaMMI.c] mmi_irda_obex_snd_abort")
    TRC_MSG(MMI_IRDA_OBEX_FUNC_SND_DISCONNECT, "[IrdaMMI.c] mmi_irda_obex_snd_disconnect")
    TRC_MSG(MMI_IRDA_OBEX_FUNC_SND_CONNECT_TERM, "[IrdaMMI.c] mmi_irda_obex_snd_connect_terminated")

    /* Irda MMI Export functions */
    TRC_MSG(MMI_IRDA_FUNC_SND_OBJ, "[IrdaMMI.c] mmi_irda_send_obj")

    /* Irda MMI OBEX <--> MMI Message handler */
    TRC_MSG(MMI_IRDA_OBEX_FUNC_CONNECT_IND_HDLR, "[IrdaMMI.c] mmi_irda_obex_connect_ind_handler")
    TRC_MSG(MMI_IRDA_OBEX_FUNC_CONNECT_CNF_HDLR, "[IrdaMMI.c] mmi_irda_obex_connect_cnf_handler")
    TRC_MSG(MMI_IRDA_OBEX_FUNC_DISCONNECT_IND_HDLR, "[IrdaMMI.c] mmi_irda_obex_disconnect_ind_handler")
    TRC_MSG(MMI_IRDA_OBEX_FUNC_PUT_OBJ_IND_HDLR, "[IrdaMMI.c] mmi_irda_obex_put_object_ind_handler")
    TRC_MSG(MMI_IRDA_OBEX_FUNC_PUT_OBJ_CNF_HDLR, "[IrdaMMI.c] mmi_irda_obex_put_object_cnf_handler")

    /* Irda MMI Send Message functions */
    TRC_MSG(MMI_IRDA_FUNC_SEND_MSG_TO_OBEX, "[IrdaMMI.c] mmi_irda_send_msg_to_obex")
    TRC_MSG(MMI_IRDA_FUNC_SEND_MSG_TO_LMP, "[IrdaMMI.c] mmi_irda_send_msg_to_lmp")

    /* Irda MMI trace state */
    TRC_MSG(MMI_IRDA_STATE_TRANSITION, "[IrdaMMI] state transition. (%d -> %d)")
    TRC_MSG(MMI_IRDA_STATE_ERROR, "[IrdaMMI] wrong state. (%d)")




    /* pbap pbap pbap pbap pbap pbap */
    ...
    /* FOTA -- devicemanagement */
    TRC_MSG(TRC_MMI_FWU_FOTA_TFTP_START_CNF, "[FOTA]mmi_dm_fwu_tftp_start_tftp_cnf(), result = %d")
    TRC_MSG(TRC_MMI_FWU_FOTA_TFTP_RESULT_IND, "[FOTA]mmi_dm_fwu_tftp_result_ind(), result = %d")
    TRC_MSG(TRC_MMI_FWU_FOTA_HTTP_GET_PROF_CNF, "[FOTA]mmi_dm_fwu_http_get_profile_content_cnf(), result = %d")    
    TRC_MSG(TRC_MMI_FWU_FOTA_HTTP_SET_CHANNEL_CNF, "[FOTA]mmi_dm_fwu_http_set_channel_cnf(), result = %d")    
    TRC_MSG(TRC_MMI_FWU_FOTA_HTTP_SEND_HTTP_CNF, "[FOTA]mmi_dm_fwu_http_send_http_cnf(), result = %d, status = %d")    
    TRC_MSG(TRC_MMI_FWU_FOTA_HTTP_READ_CNT_CNF, "[FOTA]mmi_dm_fwu_http_read_content_cnf(), result = %d")    
    TRC_MSG(TRC_MMI_FWU_FOTA_FOTA_INIT_RSP, "[FOTA]mmi_dm_fwu_fmt_fota_init_rsp(), result = %d")    
    TRC_MSG(TRC_MMI_FWU_FOTA_WRITE_DATA_RSP, "[FOTA]mmi_dm_fwu_fmt_fota_write_data_rsp(), result = %d")    
    TRC_MSG(TRC_MMI_FWU_FOTA_FINALIZE_RSP, "[FOTA]mmi_dm_fwu_fmt_fota_finalize_rsp(), result = %d")    
    TRC_MSG(TRC_MMI_FWU_FOTA_FINAL_CHECKSUM_RSP, "[FOTA]mmi_dm_fwu_fmt_fota_finalize_rsp(), Check Sum error, result = %d")    
    ...
    TRC_MSG(MMI_DTCNT_WIFI_IPADDR_UPDATE, "[DATA ACCOUNT] Ipaddress updated to %d . %d . %d . %d")  
    
    /* WIFI-CBM STATUS RELATED */
    TRC_MSG(MMI_DTCNT_WIFI_ASSIGN_G_ALWAYS_ASK_CONNECT_FLAG, "[DATA ACCOUNT WIFI-CBM] assign g_always_ask_connect_flag = %d")  
    TRC_MSG(MMI_DTCNT_WIFI_ASSIGN_G_ALWAYS_ASK_BEFORE_CONNECT_FLAG, "[DATA ACCOUNT WIFI-CBM] assign g_always_ask_before_connect_flag = %d")  
    TRC_MSG(MMI_DTCNT_WIFI_ASSIGN_G_ALWAYS_ASK_SEND_CNF_FLAG, "[DATA ACCOUNT WIFI-CBM] assign g_always_ask_send_cnf_flag = %d")

/* SRV_DA_NEW_END */

    /*DCD -- */
	TRC_MSG(MMI_DCD_INITIALIZE, "[DCD] mmi_dcd_initialize")
	TRC_MSG(MMI_DCD_SWITCH_VALUE, "[DCD] dcd switch value: switch=%d")
	TRC_MSG(MMI_DCD_AFTER_AGENT_CREATED, "[DCD] dcd after agent created: agent_handle=%d")
	TRC_MSG(MMI_DCD_FAILED_TO_CONFIG_SERVER, "[DCD] dcd failed to config server")
	TRC_MSG(MMI_DCD_FAILED_TO_START, "[DCD] dcd failed to start")
	TRC_MSG(MMI_DCD_REPLACE_SIM, "[DCD] sim replaced")
	TRC_MSG(MMI_DCD_FINALIZE, "[DCD] mmi_dcd_finalize")
	TRC_MSG(MMI_DCD_STOP_TTL_TIMER, "[DCD] dcd stop ttl timer")
	TRC_MSG(MMI_DCD_GET_AGENT_HANDLE, "[DCD] mmi_dcd_get_agent_handle handle=%d")
	TRC_MSG(MMI_DCD_IS_SWITCH_ON, "[DCD] mmi_dcd_is_switch_on switch=%d")
	TRC_MSG(MMI_DCD_LINE_TAKEN, "[DCD] mmi_dcd_line_taken")
	TRC_MSG(MMI_DCD_SYNC_SUSPENDED, "[DCD] dcd sync suspended")
	TRC_MSG(MMI_DCD_LINE_TAKEN_VALUE, "[DCD] mmi_dcd_line_taken line_taken=%d")
	TRC_MSG(MMI_DCD_LINE_GIVEN, "[DCD] mmi_dcd_line_given")
	TRC_MSG(MMI_DCD_LINE_GIVEN_VALUE, "[DCD] mmi_dcd_line_given line_given=%d")
	TRC_MSG(MMI_DCD_SYNC_RESUMED, "[DCD] dcd sync resumed")
	TRC_MSG(MMI_DCD_POSTPONED_SYNC_RESUMED, "[DCD] dcd postponed sync resumed: type=%d")
	TRC_MSG(MMI_DCD_NETWORK_INDICATION, "[DCD] mmi_dcd_network_indication roaming=%d")
	TRC_MSG(MMI_DCD_IS_READY_TO_SYNC, "[DCD] dcd is ready to sync: ready=%d")
	TRC_MSG(MMI_DCD_ROAMING_STATUS, "[DCD] dcd roaming status: is_roaming=%d, foreign_roaming=%d")
	TRC_MSG(MMI_DCD_SWITCH_TO_SYNC_FUNC, "[DCD] mmi_dcd_switch_to_sync on=%d")
	TRC_MSG(MMI_DCD_CONN_TO_SYNC_FUNC, "[DCD] mmi_dcd_conn_to_sync conn=%d")
	TRC_MSG(MMI_DCD_CONN_STATUS, "[DCD] dcd conn status: conn_mode=%d, is_roaming=%d")
	TRC_MSG(MMI_DCD_ROAM_TO_SYNC, "[DCD] mmi_dcd_roam_to_sync shift=%d")
	TRC_MSG(MMI_DCD_USER_TO_SYNC, "[DCD] mmi_dcd_user_to_sync")
	TRC_MSG(MMI_DCD_SYNCING_STATUS, "[DCD] dcd sync status: is_syncing=%d, is_retrying=%d")
	TRC_MSG(MMI_DCD_READY_TO_SYNC, "[DCD] mmi_dcd_ready_to_sync")
	TRC_MSG(MMI_DCD_DEVICE_STATUS, "[DCD] dcd device status: is_new_sim=%d, is_reset=%d")
	TRC_MSG(MMI_DCD_TTL_EXPIRED, "[DCD] dcd ttl expired")
	TRC_MSG(MMI_DCD_TTL_VALUE, "[DCD] dcd ttl value: left_ttl=%d")
	TRC_MSG(MMI_DCD_MSG_SYNC_START, "[DCD] mmi_dcd_message_proc: SYNC_START")
	TRC_MSG(MMI_DCD_MSG_SYNC_DOWNLOAD_SYNCPKG_COMPLETE, "[DCD] mmi_dcd_message_proc: DOWNLOAD_SYNCPKG_COMPLETE")
	TRC_MSG(MMI_DCD_MSG_SYNCPKG_PARSING_SUCCESS, "[DCD] mmi_dcd_message_proc: SYNCPKG_PARSING_SUCCESS")
	TRC_MSG(MMI_DCD_MSG_DOWNLOAD_CONTENT_SUCCESS, "[DCD] mmi_dcd_message_proc: DOWNLOAD_CONTENT_SUCCESS")
	TRC_MSG(MMI_DCD_MSG_CONTENT_PARSING_SUCCESS, "[DCD] mmi_dcd_message_proc: CONTENT_PARSING_SUCCESS")
	TRC_MSG(MMI_DCD_MSG_SYNC_COMPLETE, "[DCD] mmi_dcd_message_proc: SYNC_COMPLETE")
	TRC_MSG(MMI_DCD_MSG_ERR_SYNC, "[DCD] mmi_dcd_message_proc: ERR_SYNC code=%d")
    TRC_MSG(MMI_DCD_MSG_SYNC_DEFAULT, "[DCD] mmi_dcd_message_proc: DEFAULT code=%d")
	TRC_MSG(MMI_DCD_TTL_TIMER_HANDLER, "[DCD] mmi_dcd_ttl_timer_handler")
	TRC_MSG(MMI_DCD_RETRY_TIMER_HANDLER, "[DCD] mmi_dcd_retry_timer_handler")
	TRC_MSG(MMI_DCD_START_SYNC, "[DCD] mmi_dcd_start_sync type=%d")
	TRC_MSG(MMI_DCD_SYNC_POSTPONED, "[DCD] dcd sync postponed: line_taken=%d")
	TRC_MSG(MMI_DCD_AFTER_SYNC_REQUEST, "[DCD] dcd after sync request: result=%d")
	TRC_MSG(MMI_DCD_STOP_SYNC, "[DCD] mmi_dcd_stop_sync type=%d")
	TRC_MSG(MMI_DCD_SYNC_STATE, "[DCD] dcd sync state: state=%d")
	TRC_MSG(MMI_DCD_SYNC_TYPE_VALUE, "[DCD] dcd sync type: type=%d")
	TRC_MSG(MMI_DCD_POSTPONED_SYNC_TYPE_VALUE, "[DCD] dcd postponed sync type: postponed_type=%d")
	TRC_MSG(MMI_DCD_RETRY, "[DCD] mmi_dcd_retry type=%d")
	TRC_MSG(MMI_DCD_RETRY_TIMES_VALUE, "[DCD] dcd retry times: times=%d")
	TRC_MSG(MMI_DCD_STOP_RETRY, "[DCD] mmi_dcd_stop_retry type=%d")
    
    TRC_MSG(MMI_DCD_ENTRY_PUSH_HANDLER, "[DCD][function] mmi_dcd_receive_da_file")
    TRC_MSG(MMI_DCD_PUSH_HEADER,     "[DCD][push] get push header")
    TRC_MSG(MMI_DCD_PUSH_IS_DCD,     "[DCD][push] push is for dcd app:%d")
    TRC_MSG(MMI_DCD_PUSH_URL,        "[DCD][push] get push url")
    TRC_MSG(MMI_DCD_CONN_TO_SYNC,    "[DCD] connection mode:%d -> %d")
    TRC_MSG(MMI_DCD_SWITCH_TO_SYNC,  "[DCD] switch flag:%d -> %d")
	TRC_MSG(MMI_DCD_ENTER_SLEEPIN_HDLR,  "[DCD][function] mmi_dcd_sleepin_hdlr")
	TRC_MSG(MMI_DCD_ENTER_SLEEPOUT_HDLR, "[DCD][function] mmi_dcd_sleepout_hdlr")
    TRC_MSG(MMI_DCD_ENGINE_SETVIEWMODE,  "[DCD][engine] emDCDAgent_SetViewMode = %d")
    TRC_MSG(MMI_DCD_ENGINE_GETVIEWMODE,  "[DCD][engine] emDCDAgent_GetViewMode = %d")	
    TRC_MSG(MMI_DCD_ENGINE_SHOWSTART,    "[DCD][engine] emDCDAgent_ShowStart = %d")
    TRC_MSG(MMI_DCD_ENGINE_SHOWPAUSE,    "[DCD][engine] emDCDAgent_Pause = %d")
    TRC_MSG(MMI_DCD_ENGINE_SHOWRESUME,   "[DCD][engine] emDCDAgent_Resume = %d")
    TRC_MSG(MMI_DCD_ENGINE_SHOWEND,      "[DCD][engine] emDCDAgent_ShowEnd = %d")
    
    TRC_MSG(MMI_DCD_TRC_SET_VIEW_MODE,    "[DCD][mmi] mmi_dcd_set_view_mode = %d")
    TRC_MSG(MMI_DCD_TRC_GET_VIEW_MODE,    "[DCD][mmi] mmi_dcd_get_view_mode = %d")	
    TRC_MSG(MMI_DCD_TRC_SHOW_RESET,       "[DCD][mmi] mmi_dcd_show_reset")
    TRC_MSG(MMI_DCD_TRC_SHOW_START,       "[DCD][mmi] mmi_dcd_show_start = %d")
    TRC_MSG(MMI_DCD_TRC_SHOW_PAUSE,       "[DCD][mmi] mmi_dcd_show_pause = %d")
    TRC_MSG(MMI_DCD_TRC_SHOW_RESUME,      "[DCD][mmi] mmi_dcd_show_resume = %d")
    TRC_MSG(MMI_DCD_TRC_SHOW_END,         "[DCD][mmi] mmi_dcd_show_end = %d")    
    TRC_MSG(MMI_DCD_SYNC_RESULT_PROMPT,   "[DCD]mmi_dcd_sync_prompt = %d") 
    TRC_MSG(MMI_DCD_TIMER_INTO_SLEEP,     "[DCD]mmi_dcd_shift_timer_into_sleep = %d")
    TRC_MSG(MMI_DCD_TIMER_OUT_SLEEP,      "[DCD]mmi_dcd_shift_timer_out_sleep = %d")
    TRC_MSG(MMI_DCD_START_TIMER,          "[DCD]mmi_dcd_start_timer type=%d delay=%d")
    TRC_MSG(MMI_DCD_STOP_TIMER,           "[DCD]mmi_dcd_stop_timer")    

    TRC_MSG(MMI_DCD_TRC_MANUAL_UPDATE_CALLBACK_ENTER, "[DCD] mmi_dcd_manual_update_callback Enter")
    TRC_MSG(MMI_DCD_TRC_MANUAL_UPDATE_CALLBACK_SUCCESS, "[DCD] mmi_dcd_manual_update_callback Popup Success")
    TRC_MSG(MMI_DCD_TRC_MANUAL_UPDATE_CALLBACK_ERROR, "[DCD] mmi_dcd_manual_update_callback Popup Error")
    TRC_MSG(MMI_DCD_TRC_MANUAL_UPDATE_ABORT,   "[DCD]abort flag = %d") 
    TRC_MSG(MMI_DCD_TRC_MANUAL_UPDATE_STOP,   "[DCD]manual update stop flag = %d") 

    TRC_MSG(MMI_DCD_URL_INDEX, "[DCD] mmi_dcd_go_to_url_index =%d")
    TRC_MSG(MMI_DCD_ENTRY_LINK, "[DCD] mmi_dcd_entry_link =%d")
    TRC_MSG(MMI_DCD_GET_LINKURL, "[DCD] mmi_dcd_get_linkURL =%d")
    TRC_MSG(MMI_DCD_CONTENT_HANDLE, "[DCD] mmi_dcd_content_handle =%d")
    TRC_MSG(MMI_DCD_IMAGE_IS, "[DCD] there is no image")
    TRC_MSG(MMI_DCD_OBJECT_IS, "[DCD] there is no object")
    TRC_MSG(MMI_DCD_ITEM_INDEX, "[DCD] item index is greater than object items")

    TRC_MSG(MMI_DCD_TRC_PUSH_TO_SYNC,      "[DCD] mmi_dcd_push_to_sync push=%s")
    TRC_MSG(MMI_DCD_TRC_SYNC_RESULT,       "[DCD] mmi_dcd_start_sync type=%d result=%d")
    TRC_MSG(MMI_DCD_TRC_ENTER_SYNC_BEFORE, "[DCD] mmi_dcd_sync_before")
    TRC_MSG(MMI_DCD_TRC_ENTER_SYNC_AFTER,  "[DCD] mmi_dcd_sync_after")
    TRC_MSG(MMI_DCD_TRC_SOC_NOTIFICATION,  "[DCD] mmi_dcd_hdlr_soc_notify_ind")
    TRC_MSG(MMI_DCD_TRC_SOC_NOTIFY_DETAIL, "[DCD] mmi_dcd_hdlr_soc_notify_ind soc=%d event=%d")
    TRC_MSG(MMI_DCD_TRC_BEARER_STATUS_IND, "[DCD] mmi_dcd_bearer_status_cb")
    TRC_MSG(MMI_DCD_TRC_BEARER_STAT_DETAIL,"[DCD] mmi_dcd_bearer_status_cb account=%d state=%d")
    TRC_MSG(MMI_DCD_TRC_GET_HOST_BY_NAME,  "[DCD] mmi_dcd_hdlr_get_host_by_name_ind")
    TRC_MSG(MMI_DCD_TRC_GET_HOST_BY_NAME_HANDLED,  "[DCD] mmi_dcd_hdlr_get_host_by_name_ind handled")
    
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_Q_STATE_DUMP,  "[DCD] Enter dcd_sock_queue_state_dump")
    TRC_MSG(MMI_DCD_TRC_SOCK_Q_STATE,             "[DCD] index=%d: sock=%d  state=%d")
    TRC_MSG(MMI_DCD_TRC_ENTER_OPEN_SOCK,          "[DCD] enter open socket type=%d")
    TRC_MSG(MMI_DCD_TRC_OPEN_SOCK_FAILED,         "[DCD] Open Socket failed!")
    TRC_MSG(MMI_DCD_TRC_OPEN_SOCK_RETURN,         "[DCD] Open Socket return: sock=%d")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_CONN,          "[DCD] Enter Socket Connect socket=%d.")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_CLOSE,         "[DCD] Enter Socket CLOSE socket=%d.")
    TRC_MSG(MMI_DCD_TRC_SOCK_CLOSE_BLOCK,         "[DCD] Socket Close: eSEN_PAL_NET_WOULDBLOCK")
    TRC_MSG(MMI_DCD_TRC_SOCK_CLOSE_FAILED,        "[DCD] Socket Close: failed")
    TRC_MSG(MMI_DCD_TRC_SOCK_CLOSE_SUCC,          "[DCD] Socket Close: successfully")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_SEND,          "[DCD] Enter Socket send: socket=%d")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_RECV,          "[DCD] Enter Socket Recv: socket=%d")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_REG_CB,        "[DCD] Enter ISocket_RegisterCB socket=%d.")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_RECV_FROM,     "[DCD] Enter Socket RecvFrom socket=%d.")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_SET_BLOCK,     "[DCD] Enter Socket SetBlocking socket=%d.")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_SEND_TO,       "[DCD] Enter Socket SendTo socket=%d.")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_LISTEN,        "[DCD] Enter Socket Listen socket=%d.")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_BIND,          "[DCD] Enter Socket Bind socket=%d.")
    TRC_MSG(MMI_DCD_TRC_ENTER_SOCK_ACCEPT,        "[DCD] Enter Socket Accept socket=%d.")
    
    TRC_MSG(MMI_DCD_TRC_ENTER_GETHOSTBYNAME,      "[DCD] Enter INetMgr_GetHostByName")
    TRC_MSG(MMI_DCD_TRC_NETMGR_INIT,              "[DCD] Network Manager Initialize:%d")
    TRC_MSG(MMI_DCD_TRC_NETMGR_FINALIZE,          "[DCD] Network Manager Finailze:%d")
    TRC_MSG(MMI_DCD_TRC_NETMGR_OPEN,              "[DCD] Network Manager NetOpen:%d")    
    TRC_MSG(MMI_DCD_TRC_NETMGR_CLOSE,             "[DCD] Network Manager NetClose:%d")    
    TRC_MSG(MMI_DCD_TRC_NETMGR_GETSTATE,          "[DCD] Network Manager GetNetState:%d")
    
    TRC_MSG(MMI_DCD_TRC_HEAP_MALLOC,              "[DCD] Heap Malloc size=%d.")    
    TRC_MSG(MMI_DCD_TRC_HEAP_MAX_ALLOC,           "[DCD] Heap max alloc size=%d.")
    TRC_MSG(MMI_DCD_TRC_HEAP_TOTAL_LEFT,          "[DCD] Heap total left size=%d.")

    TRC_MSG(MMI_DCD_TRC_SIM_UNAVAILABLE,            "[DCD] mmi_dcd_sim_unavailable_hdlr.")
    TRC_MSG(MMI_DCD_TRC_CLOSE_GROUP,                "[DCD] Close DCD screen group.")

    /* DCD 3.0 --APP-- */
    TRC_MSG(MMI_DCD_V30_APP_STORY_CONTENT_REQUEST, "[DCD][APP] mmi_dcd_story_content_request()")
    TRC_MSG(MMI_DCD_V30_APP_INITALITZE, "[DCD][APP] mmi_dcd_initialize()")
    TRC_MSG(MMI_DCD_V30_APP_INITALITZE_FAIL, "[DCD][APP] mmidcd initialize fail")
    TRC_MSG(MMI_DCD_V30_APP_CALLBACK, "[DCD][APP] [DCD][APP] mmi_dcd_app_callback(), evt: %d")
    TRC_MSG(MMI_DCD_V30_APP_PRE_ENTRY_CITY_LIST, "[DCD][APP] mmi_dcd_pre_entry_city_list() page_index=%d")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_CITY_LIST, "[DCD][APP] mmi_dcd_entry_city_list()")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_CONNECT_SYNC, "[DCD][APP] mmi_dcd_entry_connect_sync()")
    TRC_MSG(MMI_DCD_V30_APP_CONTENT_SYNC_SCREEN_LEAVE, "[DCD][APP] mmi_dcd_content_sync_screen_leave g_isSyncStopped=%d g_onlyPkgSync=%d")
    TRC_MSG(MMI_DCD_V30_APP_SPEED_IS_SET, "[DCD][APP] speed is set to %d")
    TRC_MSG(MMI_DCD_V30_APP_MESSAGE_PROC, "[DCD][APP] mmi_dcd_message_proc(), msgcode: %d")
    TRC_MSG(MMI_DCD_V30_APP_PRE_ENTRY_STORY_VIEWER, "[DCD][APP] mmi_dcd_pre_entry_story_viewer()")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_STORY_VIEWER, "[DCD][APP] mmi_dcd_entry_story_viewer")
    TRC_MSG(MMI_DCD_V30_APP_GET_NEXT_STORY_PAGE, "[DCD][APP] mmi_dcd_entry_get_next_story_page()")
    TRC_MSG(MMI_DCD_V30_APP_NEXT_STORY_PAGE_INFO, "[DCD][APP] story_total_count is: %d request pageorder is %d")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_NO_CONTENT, "[DCD][APP] no content in this entry")
    TRC_MSG(MMI_DCD_V30_APP_CONTENT_HANDLE, "[DCD][APP] content handle is %d")
    TRC_MSG(MMI_DCD_V30_APP_PRE_ENTRY_BACKGROUND_LIST, "[DCD][APP] mmi_dcd_pre_entry_background_list()")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_BACKGROUND_LIST, "[DCD][APP] mmi_dcd_entry_background_list()")
    TRC_MSG(MMI_DCD_V30_APP_BACKGROUND_LIST_MOVE, "[DCD][APP] mmi_dcd_entry_background_list_page_move() page_index=%d")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_STORY_ITEM_MOVE, "[DCD][APP]mmi_dcd_entry_story_item_move() item_move is %d")    
    TRC_MSG(MMI_DCD_V30_APP_DCD_SWITCH, "[DCD][APP]mmi_dcd_switch, %d")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_SEARCH_CITY, "[DCD][APP] mmi_dcd_entry_search_city()")
    TRC_MSG(MMI_DCD_V30_APP_RESULT, "[DCD][APP] result is %d")
    TRC_MSG(MMI_DCD_V30_APP_NO_NEW_UNREAD_ENTRY, "[DCD][APP] no new_unread_entry")
    TRC_MSG(MMI_DCD_V30_APP_GET_CURRENT_IDLE_CHANNEL_INDEX, "[DCD][APP] mmi_dcd_get_current_idle_channel_index() current index is %d")
    TRC_MSG(MMI_DCD_V30_APP_SET_CURRENT_IDLE_CHANNEL_INDEX, "[DCD][APP] mmi_dcd_set_current_idle_channel_index() current index is %d")
    TRC_MSG(MMI_DCD_V30_APP_GET_ENTRY_CONTENT_COMPLETE, "[DCD][APP] mmi_dcd_get_entry_content_complete() pkg_complete is %d")
    TRC_MSG(MMI_DCD_V30_APP_GET_ENTRY_REFRESH, "[DCD][APP] mmi_dcd_get_entry_content_refresh()")
    TRC_MSG(MMI_DCD_V30_APP_GET_BACKGROUND_LIST_COMPLETE, "[DCD][APP] mmi_dcd_get_background_list_complete()")
    TRC_MSG(MMI_DCD_V30_APP_GET_CITY_LIST_COMPLETE, "[DCD][APP] mmi_dcd_get_city_list_complete() pageIndex is %d")
    TRC_MSG(MMI_DCD_V30_APP_SEARCH_CITY_COMPLETE, "[DCD][APP] mmi_dcd_search_city_complete()")
    TRC_MSG(MMI_DCD_V30_APP_ADD_CITY_COMPLETE, "[DCD][APP] mmi_dcd_add_city_complete()")
    TRC_MSG(MMI_DCD_V30_APP_REFRESH_CHANNEL_DETAIL, "[DCD][APP] mmi_dcd_refresh_channel_detail()")
    TRC_MSG(MMI_DCD_V30_APP_REFRESH_WEATHER_CITY, "[DCD][APP] mmi_dcd_refresh_weather_city()")
    TRC_MSG(MMI_DCD_V30_APP_DELETE_CITY_COMPLETE, "[DCD][APP] mmi_dcd_delete_city_complete()")
    TRC_MSG(MMI_DCD_V30_APP_MANUAL_UPDATE_COMPLETE, "[DCD][APP] mmi_dcd_manual_update_complete() pkg_complete is %d")    
    TRC_MSG(MMI_DCD_V30_APP_REDRAW_FIXED_LIST, "[DCD][APP] redraw_fixed_list")
    TRC_MSG(MMI_DCD_V30_APP_MAIN_UPDATE_COMPLETE, "[DCD][APP] mmi_dcd_main_update_complete() pkg_complete is %d")
    TRC_MSG(MMI_DCD_V30_APP_SYNC_FAIL, "[DCD][APP] mmi_dcd_sync_fail()")
    TRC_MSG(MMI_DCD_V30_APP_ADD_CITY_SELECTED_QUERY, "[DCD][APP] mmi_dcd_entry_add_city_selected_query() city_code is %s")
    TRC_MSG(MMI_DCD_V30_APP_ADD_CITY_SELECTED_TO_LIST, "[DCD][APP] mmi_dcd_add_city_selected_to_list()")
    TRC_MSG(MMI_DCD_V30_APP_GET_FOCUS_CITY, "[DCD][APP] mmi_dcd_get_city_block_focus()")
    TRC_MSG(MMI_DCD_V30_APP_FORWORD_SMS, "[DCD][APP] mmi_dcd_entry_forward_sms()")
    TRC_MSG(MMI_DCD_V30_APP_FORWORD_MMS, "[DCD][APP] mmi_dcd_entry_forward_mms()")
    TRC_MSG(MMI_DCD_V30_APP_FORWORD_EMAIL, "[DCD][APP] mmi_dcd_entry_forward_email()")
    TRC_MSG(MMI_DCD_V30_APP_COMMON_FAIL, "[DCD][APP] error, line: %d")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_CHANNEL_LIST_INT, "[DCD][APP] mmi_dcd_entry_channel_list_int")
    TRC_MSG(MMI_DCD_V30_APP_ABORT_CONTENT_SYNC, "[DCD][APP] user abort sync, g_onlyPkgSync is %d")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_MY_FAVORITE, "[DCD][APP] mmi_dcd_entry_my_favorite")
    TRC_MSG(MMI_DCD_V30_APP_PRE_ENTRY_CHANNEL_DETAIL, "[DCD][APP] mmi_dcd_pre_entry_channel_detail")
    TRC_MSG(MMI_DCD_V30_APP_VIEW_MODE, "[DCD][APP] view mode is %d")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_CHANNEL_DETAIL, "[DCD][APP] mmi_dcd_entry_channel_detail")
    TRC_MSG(MMI_DCD_V30_APP_CLEAR_CITY_BLOCK, "[DCD][APP] Clear city block")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_PRESET_CHANNEL_DETAIL, "[DCD][APP] mmi_dcd_entry_preset_channel_detail")
    TRC_MSG(MMI_DCD_V30_APP_PRE_ENTRY_CHANNEL_LIST_FROM_DETAIL, "[DCD][APP] mmi_dcd_pre_entry_channel_list_from_channel_detail, fromIdle is %d")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_CHANNEL_LIST_FROM_DETAIL, "[DCD][APP] mmi_dcd_entry_channel_list_from_channel_detail")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_SAVED_STORY_VIEWER, "[DCD][APP] mmi_dcd_entry_saved_story_viewer")
    TRC_MSG(MMI_DCD_V30_APP_STOP_ENTRY_CONTENT_SYNC, "[DCD][APP] stop entry content sync")
    TRC_MSG(MMI_DCD_V30_APP_PROC, "[DCD][APP] mmi_dcd_proc, evt id is %d")
    TRC_MSG(MMI_DCD_V30_APP_PRE_ENTRY_WEATHER_CITY, "[DCD][APP] mmi_dcd_pre_entry_weather_city")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_WEATHER_CITY, "[DCD][APP] stop entry content sync")
    TRC_MSG(MMI_DCD_V30_APP_ENTRY_STORY_PAGE_MOVE, "[DCD][APP] mmi_dcd_entry_story_page_move")
    

    /* DCD 3.0 --SRV-- */
    TRC_MSG(MMI_DCD_V30_TRC_SRV_INITIALIZE, "[DCD][SRV] mmi_dcd_srv_initialize()")
    TRC_MSG(MMI_DCD_V30_TRC_SRV_FINALIZE, "[DCD][SRV] mmi_dcd_finalize()")
    TRC_MSG(MMI_DCD_V30_TRC_CHECK_SWITCH_ON, "[DCD][SRV] mmi_dcd_is_switch_on()")
    TRC_MSG(MMI_DCD_V30_TRC_SWITCH_DETAIL_INFO, "[DCD][SRV] dcd_switch: %d, sim_avalible: %d, flight_mode: %d, switch_on: %d")
    TRC_MSG(MMI_DCD_V30_TRC_SETTING_INIT, "[DCD][SRV] mmi_dcd_setting_init()")
    TRC_MSG(MMI_DCD_V30_TRC_SAVE_SETTING_DATA, "[DCD][SRV] mmi_dcd_save_setting_data()")
    TRC_MSG(MMI_DCD_V30_TRC_SET_SERVER_URL, "[DCD][SRV] mmi_dcd_set_server_url()")
    TRC_MSG(MMI_DCD_V30_TRC_SET_MAX_PERMANENCE_NUM, "[DCD][SRV] mmi_dcd_set_max_permanence_num(), num: %d")
    TRC_MSG(MMI_DCD_V30_TRC_SET_UPDATE_MODE, "[DCD][SRV] mmi_dcd_set_update_mode()")
    TRC_MSG(MMI_DCD_V30_TRC_SRV_CNTX_INIT, "[DCD][SRV] mmi_dcd_srv_cntx_init()")
    TRC_MSG(MMI_DCD_V30_TRC_ENGINE_INIT, "[DCD][SRV] mmi_dcd_engine_init()")
    TRC_MSG(MMI_DCD_V30_TRC_SRV_MSG_PROC, "[DCD][SRV] mmi_dcd_srv_message_proc(), msg: %d")
    TRC_MSG(MMI_DCD_V30_TRC_SRV_CALLBACK, "[DCD][SRV] srv callback, evt: %d")
    TRC_MSG(MMI_DCD_V30_TRC_GET_PKG_TTL, "[DCD][SRV] pkg ttl: %d")
    TRC_MSG(MMI_DCD_V30_TRC_CHECK_SYNC_ALLOWED, "[DCD][SRV] mmi_dcd_check_sync_allowed(), sync_type: %d, conn_mode: %d, is_roaming: %d")
    TRC_MSG(MMI_DCD_V30_TRC_SYNC_ALLOWED_RESULT, "[DCD][SRV] sync allowed result: %d")
    TRC_MSG(MMI_DCD_V30_TRC_CHECK_MANUAL_SYNC, "[DCD][SRV] mmi_dcd_is_manual_sync(), sync_type: %d, include_user_sync: %d")
    TRC_MSG(MMI_DCD_V30_TRC_CHECK_CALLBACK_NEEDED, "[DCD][SRV] mmi_dcd_is_callback_needed(), sync_type: %d, msg: %d")
    TRC_MSG(MMI_DCD_V30_TRC_CHECK_ERR_MSG, "[DCD][SRV] mmi_dcd_check_error_msg(), msg: %d")
    TRC_MSG(MMI_DCD_V30_TRC_USER_TO_SYNC, "[DCD][SRV] mmi_dcd_user_to_sync(), req_type: %d, is_syncing: %d")
    TRC_MSG(MMI_DCD_V30_TRC_START_SYNC_RESULT, "[DCD][SRV] start sync result: %d")
    TRC_MSG(MMI_DCD_V30_TRC_STARTUP_TO_SYNC, "[DCD][SRV] mmi_dcd_startup_to_sync()")
    TRC_MSG(MMI_DCD_V30_TRC_PUSH_TO_SYNC, "[DCD][SRV] mmi_dcd_push_to_sync()")
    TRC_MSG(MMI_DCD_V30_TRC_RESET_TO_SYNC, "[DCD][SRV] mmi_dcd_reset_to_sync()")
    TRC_MSG(MMI_DCD_V30_TRC_SWITCH_TO_SYNC, "[DCD][SRV] mmi_dcd_switch_to_sync(), switch_on: %d")
    TRC_MSG(MMI_DCD_V30_TRC_CONN_TO_SYNC, "[DCD][SRV] mmi_dcd_conn_to_sync(), conn_mode previous: %d, current: %d, is_roaming: %d")
    TRC_MSG(MMI_DCD_V30_TRC_ROAM_TO_SYNC, "[DCD][SRV] mmi_dcd_roam_to_sync(), shift: %d, conn_mode: %d")
    TRC_MSG(MMI_DCD_V30_TRC_FLYMODE_SWITCH, "[DCD][SRV] mmi_dcd_flymode_switch_finish_hdlr(), gsm_select_mode: %d")
    TRC_MSG(MMI_DCD_V30_TRC_START_SYNC, "[DCD][SRV] mmi_dcd_start_sync(), req_type: %d")
    TRC_MSG(MMI_DCD_V30_TRC_SYNC_REQUEST, "[DCD][SRV] mmi_dcd_sync_request(), req_type: %d")
    TRC_MSG(MMI_DCD_V30_TRC_STOP_SYNC, "[DCD][SRV] mmi_dcd_stop_sync()")
    TRC_MSG(MMI_DCD_V30_TRC_START_RETRY, "[DCD][SRV] mmi_dcd_retry(), retry_times: %d")
    TRC_MSG(MMI_DCD_V30_TRC_STOP_RETRY, "[DCD][SRV] mmi_dcd_stop_retry()")
    TRC_MSG(MMI_DCD_V30_TRC_RETRY_TIMER_HDLR, "[DCD][SRV] mmi_dcd_retry_timer_hdlr()")
    TRC_MSG(MMI_DCD_V30_TRC_TTL_TIMER_HDLR, "[DCD][SRV] mmi_dcd_ttl_timer_hdlr()")
    TRC_MSG(MMI_DCD_V30_TRC_USER_SYNC_FAIL_ABORT, "[DCD][SRV] mmi_dcd_user_sync_fail_abort()")
    TRC_MSG(MMI_DCD_V30_TRC_RECEIVE_DA_FILE, "[DCD][SRV] mmi_dcd_receive_da_file()")
    TRC_MSG(MMI_DCD_V30_TRC_SYNC_BEFORE, "[DCD][SRV] mmi_dcd_sync_before()")
    TRC_MSG(MMI_DCD_V30_TRC_SYNC_AFTER, "[DCD][SRV] mmi_dcd_sync_after()")
    TRC_MSG(MMI_DCD_V30_TRC_SOC_NOTIFY_IND, "[DCD][SRV] mmi_dcd_hdlr_soc_notify_ind()")
    TRC_MSG(MMI_DCD_V30_TRC_SOC_NOTIFY_INFO, "[DCD][SRV] socket_id: %d, event_type: %d, result: %d, msg_len: %d, ref_count: %d, error_cause: %d, detail_cause: %d")
    TRC_MSG(MMI_DCD_V30_TRC_GET_HOST_IND, "[DCD][SRV] mmi_dcd_hdlr_get_host_by_name_ind()")
    TRC_MSG(MMI_DCD_V30_TRC_BEARER_STATUS_CB, "[DCD][SRV] mmi_dcd_bearer_status_cb(), acnt_id: %d, state: %d")
    TRC_MSG(MMI_DCD_V30_TRC_POST_MESSAGE, "[DCD][SRV] mmi_dcd_postmessage(), event_type: %d, platformType: %d, param1: %d, param2: %d")
    TRC_MSG(MMI_DCD_V30_TRC_NETWORK_IND, "[DCD][SRV] mmi_dcd_network_indication(), dcd_initialized: %d, evt_id: %d")
    TRC_MSG(MMI_DCD_V30_TRC_FINALIZE_EVT_HDLR, "[DCD][SRV] mmi_dcd_finalize_evt_hdlr()")
    TRC_MSG(MMI_DCD_V30_TRC_SIM_UNAVAILABLE_HDLR, "[DCD][SRV] mmi_dcd_sim_unavailable_hdlr()")
    TRC_MSG(MMI_DCD_V30_TRC_FAIL_WITH_CAUSE, "[DCD][SRV] error, cause: %d, line: %d")
    TRC_MSG(MMI_DCD_V30_TRC_COMMON_FAIL, "[DCD][SRV] error, line: %d")

    /* DCD 3.0 --PLX-- */
    TRC_MSG(MMI_DCD_V30_PLX_SET_VIEW_MODE, "[DCD][PLX] mmi_dcd_set_view_mode(), curr_mode: %d, view_mode: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SHOW_RESET, "[DCD][PLX] mmi_dcd_show_reset()")
    TRC_MSG(MMI_DCD_V30_PLX_SHOW_START, "[DCD][PLX] mmi_dcd_show_start(), view_mode: %d, show_state: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SHOW_PAUSE, "[DCD][PLX] mmi_dcd_show_pause(), view_mode: %d, show_state: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SHOW_END, "[DCD][PLX] mmi_dcd_show_end(), view_mode: %d, show_state: %d")
    TRC_MSG(MMI_DCD_V30_PLX_MEM_INITIALIZE, "[DCD][PLX] mmi_dcd_mem_initialize()")
    TRC_MSG(MMI_DCD_V30_PLX_MEM_FINALIZE, "[DCD][PLX] mmi_dcd_mem_finalize()")
    TRC_MSG(MMI_DCD_V30_PLX_HEAP_MALLOC, "[DCD][PLX] pl_IHeap_Malloc(), size: %d")
    TRC_MSG(MMI_DCD_V30_PLX_MEM_INFO, "[DCD][PLX] max_alloc_size: %d, total_left_size: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SOCKET_OPEN, "[DCD][PLX] pl_ISocket_OpenSocket(), socket_type: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SOCKET_CONNECT, "[DCD][PLX] pl_ISocket_Connect(), a_nSocket: %d, m_nIP: %d, m_nPort: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SOCKET_CLOSE, "[DCD][PLX] pl_ISocket_Close(), a_nSocket: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SOCKET_SEND, "[DCD][PLX] pl_ISocket_Send(), a_nSocket: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SOCKET_RECEIVE, "[DCD][PLX] pl_ISocket_Recv(), a_nSocket: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SOCKET_RECVFROM, "[DCD][PLX] pl_ISocket_RecvFrom(), a_nSocket: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SOCKET_SETBLOCKING, "[DCD][PLX] pl_ISocket_SetBlocking(), a_nSocket: %d")
    TRC_MSG(MMI_DCD_V30_PLX_SOCKET_SENDTO, "[DCD][PLX] pl_ISocket_SendTo(), a_nSocket: %d")
    TRC_MSG(MMI_DCD_V30_PLX_FAIL_WITH_CAUSE, "[DCD][PLX] error, cause: %d, line: %d")
    TRC_MSG(MMI_DCD_V30_PLX_COMMON_FAIL, "[DCD][PLX] error, line: %d")

    /* NTP PLUTO */
    TRC_MSG(APP_NTP_SYNC_PROC,              "[NTP APP] mmi_ntp_sync_result_proc(), result: %d, offset: %d, line: %d")
    TRC_MSG(APP_NTP_SETTING_SAVE,           "[NTP APP] mmi_ntp_setting_save(), availability: %d --> %d, line: %d")


    /* for mmi_wprintf */
    TRC_MSG(MMI_CONN_APP_BUFFER_TRACE, " *** Get data in [lower Panel] ***")
	...
	TRC_MSG(MMI_CBM_DISPLAY_CSD_ICON, "[CBM] CBM display CSD icon: sim=%d state=%d")
	TRC_MSG(TRC_MMI_CBM_WLAN_SCAN_CALLBACK, "[CbmApp] WLAN scan callback, result[%d], ap_num[%d], wait[%d], query_type[%d].")


	/*App Name: APP_BTREMOTECAPTURE*/

	TRC_MSG(TRC_MMI_BT_RMTCAP_LAUNCH_REQ,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_launch_req()")
	TRC_MSG(TRC_MMI_BT_RMTCAP_WRITE_DEFAULT_PIC_REQ_0,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_write_default_pic().0")
	TRC_MSG(TRC_MMI_BT_RMTCAP_WRITE_DEFAULT_PIC_REQ_1,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_write_default_pic().1, result: %d")
	TRC_MSG(TRC_MMI_BT_RMTCAP_SHOW_LOADING,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_loading_show()")
	TRC_MSG(TRC_MMI_BT_RMTCAP_CAPTURE,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_main_capture(), cmdlen:%d cmdResult:%d")
	TRC_MSG(TRC_MMI_BT_RMTCAP_EXIT_CAPTURE,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_main_exit_capture(), cmdlen:%d cmdResult:%d")
	TRC_MSG(TRC_MMI_BT_RMTCAP_MAIN_SHOW,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_main_show(), rmtLaunched:%d")
	TRC_MSG(TRC_MMI_BT_RMTCAP_LAUNCED_RSP,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_launch_rsp(), ret_is_num: %d, ret_code:%d")
	TRC_MSG(TRC_MMI_BT_RMTCAP_CAPTURE_RSP,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_capture_rsp(), ret_is_num: %d, ret_code:%d")
	TRC_MSG(TRC_MMI_BT_RMTCAP_EVENT_HDLR,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_event_hdlr(), curr_state: %d")
	TRC_MSG(TRC_MMI_BT_RMTCAP_EVENT_HDLR2,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_event_hdlr()2, curr_state: %d")
	TRC_MSG(TRC_MMI_BT_RMTCAP_BOOTUP_INIT,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_bootup_init()")
	TRC_MSG(TRC_MMI_BT_RMTCAP_PIC_SHOW,	            "[MMI][BTRMTCAP]mmi_bt_rmtcap_pic_show()")
	...
	/*App Name: TETHERING APP*/

	TRC_MSG(MMI_TETHERING_USB_LAUNCH, "[USB_TETHERING APP] mmi_tethering_usb_launch")
	TRC_MSG(MMI_TETHERING_USB_ENTRY_CONFIRM, "[USB_TETHERING APP] mmi_tethering_usb_entry_confirm")
	TRC_MSG(MMI_TETHERING_USB_RECONNECT, "[USB_TETHERING APP] mmi_tethering_usb_reconnect, %d")
	TRC_MSG(MMI_TETHERING_USB_RECONNECT_CONFIRM, "[USB_TETHERING APP] mmi_tethering_usb_reconnect_confirm")
	TRC_MSG(MMI_TETHERING_USB_CANCEL_RECONNECT, "[USB_TETHERING APP] mmi_tethering_usb_cancel_reconnect")
	TRC_MSG(MMI_TETHERING_USB_RECONNECT_SCREEN_LEAVE_PROC, "[USB_TETHERING APP] mmi_tethering_usb_reconnect_screen_leave_proc")
	TRC_MSG(MMI_TETHERING_USB_TURN_ON, "[USB_TETHERING APP] mmi_tethering_usb_turn_on")
	TRC_MSG(MMI_TETHERING_USB_SWITCH_HDLR, "[USB_TETHERING APP] mmi_tethering_usb_switch_hdlr, result = %d")
	TRC_MSG(MMI_TETHERING_USB_GROUP_CLOSE, "[USB_TETHERING APP] mmi_tethering_usb_group_close")
	TRC_MSG(MMI_TETHERING_DISCONNECT_NOTIFY_HDLR, "[USB_TETHERING APP] mmi_tethering_disconnect_notify_hdlr, %d")
	...
	TRC_MSG(MMI_AVRCP_TRC_INIT_CLEAR_CONN, "[AVRCPMMI] srv_avrcp_init() clear cm conn, channel num = %d, cmgr_id = %d")
	...
	TRC_MSG(MMI_AVRCP_TRC_ACTIVATE_CNF_HDLR, "[AVRCPMMI] srv_avrcp_activate_cnf_hdlr()")
	...
	TRC_MSG(MMI_AVRCP_TRC_CONNECT_REQ, "[AVRCPMMI] srv_avrcp_connect_req() role = %d, channel num = %d, indication_connect_id = %d")
	TRC_MSG(MMI_AVRCP_TRC_CONNECT_CNF_HDLR, "[AVRCPMMI] srv_avrcp_connect_cnf_hdlr() result = %d, channel num = %d")
	TRC_MSG(MMI_AVRCP_TRC_CONNECT_IND_HDLR, "[AVRCPMMI] srv_avrcp_connect_ind_hdlr() connect_id = %d, role = %d")
	TRC_MSG(MMI_AVRCP_TRC_CONNECT_IND_HDLR_INT, "[AVRCPMMI] srv_avrcp_connect_ind_hdlr_int() role = %d, channel num = %d, connect_id = %d")
	TRC_MSG(MMI_AVRCP_TRC_DISCONNECT_REQ, "[AVRCPMMI] srv_avrcp_disconnect_req() channel num = %d, connect_id = %d")
	TRC_MSG(MMI_AVRCP_TRC_DISCONNECT_CT_REQ, "[AVRCPMMI] srv_avrcp_disconnect_ct_req() channel num = %d")
	TRC_MSG(MMI_AVRCP_TRC_DISCONNECT_TG_REQ, "[AVRCPMMI] srv_avrcp_disconnect_tg_req() channel num = %d")
	TRC_MSG(MMI_AVRCP_TRC_DISCONNECT_TG_CT_REQ, "[AVRCPMMI] srv_avrcp_disconnect_tg_ct_req() role = %d, channel num = %d")
	TRC_MSG(MMI_AVRCP_TRC_DISCONNECT_CNF_HDLR, "[AVRCPMMI] srv_avrcp_disconnect_cnf_hdlr() connect_id = %d, found channel num = %d, request chnl = %d")
	...
	TRC_MSG(MMI_AVRCP_TRC_AVRCP_VERSION_IND_HDLR, "[AVRCPMMI] srv_avrcp_version_indication_hdlr() result = %d, version = %d, channel num = %d")
	...
	TRC_MSG(MMI_SYNC_SRV_START_RET, "[SYNC]srv_sync_start(), ret=%d")
	TRC_MSG(MMI_SYNC_RESET_DB, "[SYNC]srv_sync_reset_database(db=%d), i=%d")
	TRC_MSG(MMI_SRV_SYNC_CM_PWR_SWITCH, "[SYNC]srv_sync_cm_bt_pwr_switch_notify(), state=%d")
	TRC_MSG(MMI_SRV_SYNC_CM_BT_IS_STATE, "[SYNC]srv_sync_cm_bt_is_state()curr_state=%d,state=%d")
	TRC_MSG(MMI_SRV_SYNC_CM_BT_ACTIVE, "[SYNC]srv_sync_cm_bt_activate(), bt_state=%d")
	TRC_MSG(MMI_SRV_SYNC_CM_BT_DEACTIVE, "[SYNC]srv_sync_cm_bt_deactivate(), bt_state=%d")
	TRC_MSG(MMI_SRV_SYNC_CM_BT_ASK_CONN, "[SYNC][SyncSrvCmBT.c]srv_sync_on_bt_ask_conn()")
	TRC_MSG(MMI_SRV_SYNC_CM_BT_ASK_CONN_FAIL, "[SYNC][SyncSrvCmBT.c]srv_sync_on_bt_ask_conn(),conn fail")
	TRC_MSG(MMI_SRV_SYNC_CM_CONN_IND, "[SYNC]srv_sync_connected_ind_hdlr(), bt_state=%d")
	TRC_MSG(MMI_SRV_SYNC_CM_BT_INIT, "[SYNC]srv_sync_cm_bt_init()")
	TRC_MSG(MMI_SRV_SYNC_CM_BT_CONNECTED, "[SYNC]srv_sync_cm_bt_on_connnected(), bt_state=%d")
	TRC_MSG(MMI_SRV_SYNC_CM_BT_DISCONNECTED, "[SYNC]srv_sync_cm_bt_disconnect(), bt_state=%d")
	TRC_MSG(MMI_SRV_SYNC_CM_BT_ON_DISCONN, "[SYNC]srv_sync_cm_bt_on_disconn(), bt_state=%d")
	TRC_MSG(MMI_SRV_SYNC_PHONE_ACCNT_READ, "[SYNCML]srv_sync_phone_read_accnt(index=%d),recent_idx=%d")
	TRC_MSG(MMI_SRV_SYNC_PHONE_ACCNT_WRITE, "[SYNCML]srv_sync_phone_write_accnt(index=%d),recent_idx=%d")
	TRC_MSG(MMI_SRV_SYNC_PHONE_ACCNT_ANCHOR, "[SYNCML][SyncSrvPhoneAccnt.c]srv_sync_phone_accnt_anchor(accnt_index=%d,db_id=%d)")
	TRC_MSG(MMI_SRV_SYNC_PHONE_SYNC_ON_RSP, "[SYNC]srv_sync_phone_on_ask_sync_rsp()")
	TRC_MSG(MMI_SRV_SYNC_PHONE_START_SYNC, "[SYNC]srv_sync_phone_start_sync()")
	TRC_MSG(MMI_SRV_SYNC_PHONE_START, "[SYNC]srv_sync_phone_start()")
	TRC_MSG(MMI_SRV_SYNC_PHONE_ON_FINISH, "[SYNCML]srv_sync_phone_on_finish()")
	TRC_MSG(MMI_SRV_SYNC_PHONE_ON_DB_INFO, "[SYNC]srv_sync_on_database_info()")
	TRC_MSG(MMI_SRV_SYNC_PHONE_GET_SEL_DB, "[SYNC]srv_sync_phone_get_selected_db()=%d")
	TRC_MSG(MMI_SRV_SYNC_PHONE_DESTROY, "[SYNC][SyncSrvPhoneSync.c]srv_sync_phone_destroy()")
	TRC_MSG(MMI_SRV_SYNC_PHONE_CREATE, "[SYNC][SyncSrvPhoneSync.c]srv_sync_phone_create(accnt=%d, user_data=%d)")
	TRC_MSG(MMI_SRV_SYNC_OTA_INSTALL, "[SYNC][OTA]srv_sync_ota_install_new_accnt(dest_idx=%d, cca_status=%d)")
	TRC_MSG(MMI_SRV_SYNC_MATCH_SYNC, "[SYNC][SyncSrvSync.]srv_sync_match(sync=%d) obj_sync=%d")
	TRC_MSG(MMI_SRV_SYNC_MATCH_ACCNT, "[SYNC][SyncSrvSync.]srv_sync_match_accnt(accnt_index=%d) obj_active_accnt=%d")
	TRC_MSG(MMI_SYNC_SRV_ERR_RET,	"[SYNCML][SyncSrv]abnomal return, line=%d")
	TRC_MSG(MMI_SYNC_SRV_INIT, 		"[SYNCML][SyncSrv.c]srv_sync_init()")
	TRC_MSG(MMI_SYNC_SRV_DEINIT, 	"[SYNCML][SyncSrv.c]srv_sync_deinit()")
	TRC_MSG(MMI_SYNC_SRV_START, 	"[SYNCML][SyncSrv.c]srv_sync_start(sync=%d,index=%d,db=%d)")
	TRC_MSG(MMI_SYNC_SRV_ABORT, 	"[SYNCML][SyncSrv.c]srv_sync_abort(sync=%d)")
	TRC_MSG(MMI_SYNC_SRV_SET_STATE, "[SYNCML][SyncSrv.c]srv_sync_set_state(state=%d)")
	TRC_MSG(MMI_SYNC_SRV_IS_STATE,  "[SYNCML][SyncSrv.c]srv_sync_is_state(%d)curr_state=%d")
	TRC_MSG(MMI_SYNC_SRV_IS_SYNC, 	"[SYNCML][SyncSrv.c]srv_sync_is_synchronizing(db=%d)=%d, active_db=%d")
	TRC_MSG(MMI_SYNC_SRV_TOTAL_DB,  "[SYNCML][SyncSrv.c]srv_sync_get_total_database()=%d")
	TRC_MSG(MMI_SYNC_SRV_RESET_CNTX, "[SYNCML][SyncSrv.c]srv_sync_reset_cntx()")
	TRC_MSG(MMI_SYNC_SRV_INIT_MSG,  	"[SYNCML][SyncSrvMsg.c]srv_sync_msg_init()")
	TRC_MSG(MMI_SYNC_SRV_SET_RECORD_REQ, "[SYNCML][SyncSrvMsg.c]srv_sync_set_record_req()")
	TRC_MSG(MMI_SYNC_SRV_SET_RECORD_RSP, "[SYNCML][SyncSrvMsg.c]srv_sync_set_record_rsp()")
	TRC_MSG(MMI_SYNC_SRV_GET_RECORD_REQ, "[SYNCML][SyncSrvMsg.c]srv_sync_get_record_req()")
	TRC_MSG(MMI_SYNC_SRV_GET_RECORD_RSP, "[SYNCML][SyncSrvMsg.c]srv_sync_get_record_rsp()")
	TRC_MSG(MMI_SYNC_SRV_QUERY_LUID_REQ, "[SYNCML][SyncSrvMsg.c]srv_sync_query_luid_list_req()")
	TRC_MSG(MMI_SYNC_SRV_QUERY_LUID_RSP, "[SYNCML][SyncSrvMsg.c]srv_sync_query_luid_list_rsp()")
	TRC_MSG(MMI_SYNC_SRV_CLEAN_LUID_REQ, "[SYNCML][SyncSrvMsg.c]srv_sync_clean_luid_list_req()")
	TRC_MSG(MMI_SYNC_SRV_CLEAN_LUID_RSP, "[SYNCML][SyncSrvMsg.c]sync_srv_clean_luid_list_rsp()")
	TRC_MSG(MMI_SYNC_SRV_FINISH, 		 "[SYNCML][SyncSrvMsg.c]srv_sync_finish_ind()")
	TRC_MSG(MMI_SYNC_SRV_PROG, 			 "[SYNCML][SyncSrvMsg.c]srv_sync_progress_info_ind()=%d")
	TRC_MSG(MMI_SYNC_SRV_PS_BUSY, 		 "[SYNCML][SyncSrvMsg.c]srv_sync_ps_busy_ind_hdlr()")
	TRC_MSG(MMI_SYNC_SRV_SEND_ABORT, 	 "[SYNCML][SyncSrvMsg.c]srv_sync_send_abort_sync()")
	TRC_MSG(MMI_SYNC_SRV_GET_IMEI_REQ,   "[SYNCML][SyncSrvMsg.c]srv_sync_get_imei_req()")
	TRC_MSG(MMI_SYNC_SRV_GET_IMEI_RSP,   "[SYNCML][SyncSrvMsg.c]sync_sync_get_imei_rsp()")
	TRC_MSG(MMI_SYNC_SRV_SESSION_ABORT,  "[SYNCML][SyncSrvMsg.c]srv_sync_session_aborted_ind()")
	TRC_MSG(MMI_SYNC_SRV_CLOG_IS_WRITABLE,  "[SYNCML][SyncSrvClog.c]srv_sync_changelog_is_writable()")
	TRC_MSG(MMI_SYNC_SRV_CLOG_CONT_CREATE,  "[SYNCML][SyncSrvDbContClog.c]srv_sync_changelog_conatct_create()")
	TRC_MSG(MMI_SYNC_SRV_CLOG_CONT_DESTROY,  "[SYNCML][SyncSrvDbContClog.c]srv_sync_changelog_contact_destroy()")
	TRC_MSG(MMI_SYNC_SRV_CLOG_CONT_FIRST_SYNC, "[SYNCML][SyncSrvDbContClog.c]srv_sync_changelog_contact_is_firstsync(sync=%d,index=%d)=%d")
	TRC_MSG(MMI_SYNC_SRV_CLOG_CONT_ADD,		   "[SYNCML][SyncSrvDbContClog.c]srv_sync_contact_addition_notification(index=%d)")
	TRC_MSG(MMI_SYNC_SRV_CLOG_CONT_MODIFY, 	   "[SYNCML][SyncSrvDbContClog.c]srv_sync_contact_modification_notification(index=%d)")
	TRC_MSG(MMI_SYNC_SRV_CLOG_CONT_DELETE, 	   "[SYNCML][SyncSrvDbContClog.c]srv_sync_contact_deletion_notification(index=%d)")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_CREATE,  	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_create()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_DESTROY,  	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_destroy()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_PROG,  		"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_on_progress(%d)")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_ATTCH_ACCNT,  "[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_attatch_accnt()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_UPDATE_DBINFO,  "[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_update_dbinfo()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_ON_GET,  	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_on_get_record()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_ON_SET,  	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_on_set_record()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_ON_CLEAN,  "[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_on_clean_luid_list()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_ON_QUERY,	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_on_query_luid_list()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_ON_START,	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_on_start()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_ON_FINISH,	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_on_finish()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_ON_RECORD_RSP,	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_on_record_rsp()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_PROG_NOTIFY, "[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_progress_notify(action=%d,curr=%d,total=%d)")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_UPDATE_STATE, "[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_update_stat(result=%d,action=%d)")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_LOCK, 		"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_lock()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_UNLOCK, 	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_unlock()")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_IS_READY,  "[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_is_ready()=%d")
	TRC_MSG(MMI_SYNC_SRV_DB_CONT_MATCH, 	"[SYNCML][SyncSrvDbCont.c]srv_sync_database_contact_match()=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_ACCNT_CREATE,  "[SYNCML][SyncSrvRmtAccnt.c]srv_sync_accnt_remote_create(index=%d)")
	TRC_MSG(MMI_SYNC_SRV_RMT_ACCNT_DESTROY, "[SYNCML][SyncSrvRmtAccnt.c]srv_sync_accnt_remote_destroy()")
	TRC_MSG(MMI_SYNC_SRV_RMT_ACCNT_INIT, 	"[SYNCML][SyncSrvRmtAccnt.c]srv_sync_remote_accnt_init()")
	TRC_MSG(MMI_SYNC_SRV_RMT_ACCNT_READ, 	"[SYNCML][SyncSrvRmtAccnt.c]srv_sync_remote_read_accnt(readIdx=%d,recentIdx=%d)")
	TRC_MSG(MMI_SYNC_SRV_RMT_ACCNT_WRITE, 	"[SYNCML][SyncSrvRmtAccnt.c]srv_sync_remote_write_accnt(readIdx=%d,recentIdx=%d)")
	TRC_MSG(MMI_SYNC_SRV_RMT_ACCNT_NEW_IDX, "[SYNCML][SyncSrvRmtAccnt.c]srv_sync_remote_accnt_new_index()=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_ACCNT_ANCHOR, 	"[SYNCML][SyncSrvRmtAccnt.c]srv_sync_remote_accnt_anchor(accnt=%d, db=%d)=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_ACCNT_REGULAR, "[SYNCML][SyncSrvRmtAccnt.c]srv_sync_remote_accnt_regular(index=%d)=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_ACCNT_DEL_DB, 	"[SYNCML][SyncSrvRmtAccnt.c]srv_sync_accnt_remote_selected_db()=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_INIT,  		"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_init()")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_CBM_DISCONN,  "[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_cbm_disconn_callback()")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_APPID,  		"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_app_id()")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_PRECHECK,  	"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_precheck()=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_START,  		"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_start()")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_ABORT,  		"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_abort(), sub_state=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_SEL_DB,  		"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_get_selected_db()")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_DESTROY,  	"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_destroy()")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_MATCH,  		"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_match()=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_MATCH_ACCNT,  "[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_match_accnt()=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_ON_FINISH,  	"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_on_finish()")
	TRC_MSG(MMI_SYNC_SRV_RMT_SYNC_CREATE,  		"[SYNCML][SyncSrvRmtSync.c]srv_sync_remote_create()")
	TRC_MSG(MMI_SYNC_SRV_RMT_REG_CHECK,  "[SYNCML][SyncSrvRmtRegular.c]srv_sync_remote_regular_check(regular=%d)")
	TRC_MSG(MMI_SYNC_SRV_RMT_RMDR_NOTIFY,  "[SYNCML][SyncSrvRmtRegular.c]srv_sync_remote_rmdr_notify(notify=%d)")
	TRC_MSG(MMI_SYNC_SRV_RMT_SET_RMDR,  "[SYNCML][SyncSrvRmtRegular.c]srv_sync_remote_regular_set_reminder(index=%d)")
	TRC_MSG(MMI_SYNC_SRV_RMT_EVT_INIT_ID,  "[SYNCML][SyncSrvRemoteEvtHandlr.c]srv_sync_remote_idbuff_init()netid=[%d][%d]")
	TRC_MSG(MMI_SYNC_SRV_RMT_EVT_CHECK_ID,  "[SYNCML][SyncSrvRemoteEvtHandlr.c]srv_sync_remote_idbuff_check()=%d, index=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_EVT_DEL_NOTIFY,  "[SYNCML][SyncSrvRemoteEvtHandlr.c]srv_sync_remote_dtcnt_del_notify(), num=%d")
	TRC_MSG(MMI_SYNC_SRV_RMT_EVT_PWRON_NOTIFY,  "[SYNCML][SyncSrvRemoteEvtHandlr.c]srv_sync_remote_pwron_notify()")
	TRC_MSG(MMI_SYNC_SRV_RMT_EVT_HDLR,  "[SYNCML][SyncSrvRemoteEvtHandlr.c]srv_sync_remote_accnt_evt_handlr()")
	TRC_MSG(MMI_SYNC_SRV_RMT_WRONG_PX,  "[SYNCML][SyncSrvRemoteEvtHandlr.c]srv_sync_remote_start() get PX fail")
	TRC_MSG(MMI_SYNC_SRV_CBM_CREATE,  "[SYNCML][SyncSrvCmCbm.c]srv_sync_cm_cbm_create(net_id=%d)")
	TRC_MSG(MMI_SYNC_SRV_CBM_DESTROY,  "[SYNCML][SyncSrvCmCbm.c]srv_sync_cm_cbm_destroy()")
	TRC_MSG(MMI_SYNC_SRV_CBM_ON_DISCONN,  "[SYNCML][SyncSrvCmCbm.c]srv_sync_cm_cbm_on_disconn() connecting=%d")
	TRC_MSG(MMI_SYNC_SRV_CBM_CONN,  "[SYNCML][SyncSrvCmCbm.c]srv_sync_cm_cbm_conn() bearer_ret=%d")
	TRC_MSG(MMI_SYNC_SRV_CBM_CALLBACK,  "[SYNCML][SyncSrvCmCbm.c]srv_sync_cm_cbm_bearer_callback() error=%d, state=%d, connecting=%d")
	TRC_MSG(MMI_SYNC_SRV_CBM_ON_CONN,  "[SYNCML][SyncSrvCmCbm.c]srv_sync_cm_cbm_on_connected() sim=%d")
	TRC_MSG(MMI_SYNC_SRV_QUERY_LOG, "[SyncML][SyncSrvMsg.c]srv_sync_query_luid_list_rsp(), Query change log, op = %d, total %d records...")
	TRC_MSG(MMI_SYNC_SRV_OTAP_PROC_CCA, "[SyncML][SyncSrvRemoteAccntOta.c]srv_sync_ota_proc_cca_doc(), conf_id = %d , doc_hdl= %d...")
	TRC_MSG(MMI_SYNC_SRV_OTAP_NEW_OTA_CNTX, "[SyncML][SyncSrvRemoteAccntOta.c]srv_sync_ota_create()")
	TRC_MSG(MMI_SYNC_SRV_OTAP_VALID_PROF_NUM, "[SyncML][SyncSrvRemoteAccntOta.c]srv_sync_ota_get_valid_profile_num():total_valid=%d, num_prof=%d")
	TRC_MSG(MMI_SYNC_SRV_OTAP_NEW_OTA_PROF, "[SyncML][SyncSrvRemoteAccntOtaParser.c]srv_sync_ota_new_profile(), cntx->num_profiles=%d")
	TRC_MSG(MMI_SYNC_SRV_OTAP_NEXT_PROF, "[SyncML][SyncSrvRemoteAccntOta.c]srv_sync_ota_next_accnt()")
	TRC_MSG(MMI_SYNC_SRV_OTAP_CONSTRUCT_DOC_FOR_DM, "[SyncML][SyncSrvRemoteAccntOta.c]srv_sync_ota_construct_cca_doc_for_dm()")  
	TRC_MSG(MMI_SYNC_SRV_OTAP_INSTALL_DOC_FROM_DM, "[SyncML][SyncSrvRemoteAccntOta.c]srv_sync_ota_install_cca_doc_from_dm()")  
	TRC_MSG(MMI_SYNC_SRV_OTAP_PARSE_ERROR_SERVER_IP, "[SYNCML][SyncSrvRemoteAccntOta.c]srv_sync_ota_get_valid_profile_num()Error:i=%d")
	TRC_MSG(MMI_SYNC_SRV_OTAP_PARSE_ERROR_NO_MATCH_PX, "[SYNCML][SyncSrvRemoteAccntOtaParser.c]srv_sync_set_proxy_for_account()Error: No matched proxy")
	TRC_MSG(MMI_SYNC_SRV_OTAP_GET_VALID_PROFILE_NUM, "[SYNCML][SyncSrvRemoteAccntOta.c]srv_sync_ota_get_valid_profile_num()new_profile validation, i=%d, new_prof->account_validation=%0x")
	TRC_MSG(MMI_SYNC_SRV_OTAP_IS_UPDATE_MATCH_RETURN, "[SYNCML][SyncSrvRemoteAccntOta.c]srv_sync_ota_get_valid_profile_num()return value of mmi_syncml_is_update_match, update_prof_idx=%d")
	TRC_MSG(MMI_SYNC_SRV_OTAP_SET_PROXY_FOR_ACCOUNT, "[SYNCML][SyncSrvRemoteAccntOtaParser.c]srv_sync_set_proxy_for_account(), i=%d, net_id=%0x")
	TRC_MSG(MMI_SYNC_SRV_OTAP_CCA_NODES, "[SyncML][SyncSrvRemoteAccntOtaParser.c]srv_sync_ota_proc_cca_nodes(), symbol = %0x")
	TRC_MSG(MMI_SYNC_SRV_OTAP_PARSE_BOOTSTRAP, "[SYNCML][SyncSrvRemoteAccntOtaParser.c]srv_sync_ota_parser_node_bootstrap(), cntx->opr_flag=%0x, state=%d")
	TRC_MSG(MMI_SYNC_SRV_OTAP_PARSE_PROXY, "[SYNCML][SyncSrvRemoteAccntOtaParser.c]srv_sync_ota_parser_node_proxy(), barer_type=%d, param->values.i[0]=%0x")
	TRC_MSG(MMI_SYNC_SRV_OTAP_PARSE_APP_ADDR, "[SYNCML][SyncSrvRemoteAccntOtaParser.c]srv_sync_ota_parser_node_app_addr(), return value=%d")
	TRC_MSG(MMI_SYNC_SRV_OTAP_PARSE_APP_ADDR_AUTH, "[SYNCML][SyncSrvRemoteAccntOtaParser.c]srv_sync_ota_parser_node_app_authinfo() fail, authlevel=%d")
	TRC_MSG(MMI_SYNC_SRV_OTAP_PARSE_RESOURCE, "[SYNCML][SyncSrvRemoteAccntOtaParser.c]srv_sync_ota_parser_node_resource(), param->id=%0x")
	TRC_MSG(MMI_SYNC_SRV_OTAP_GET_CALENDAR_TYPE, "[SYNCML][SyncSrvRemoteAccntOtaParser.c]srv_sync_ota_get_db_type() = calendar")
	TRC_MSG(MMI_SYNC_SRV_OTAP_GET_TASK_TYPE, "[SYNCML][SyncSrvRemoteAccntOtaParser.c]srv_sync_ota_get_db_type() = task")
	...
	TRC_MSG(MMI_CNMGR_UPDATE_BEARER_LIST, "[MMI CNMGR] update_bearer_list: active bearer num %d")
	TRC_MSG(MMI_CNMGR_SHOW_BEARER_LIST, "[MMI CNMGR] show_bearer_list: active bearer num %d")
	TRC_MSG(MMI_CNMGR_IS_BEARER_ACTIVE, "[MMI CNMGR] is_bearer_active: select idx %d, account id %d, connect type %d")
	TRC_MSG(MMI_CNMGR_ENTRY_BEARER_DETAIL_INFO, "[MMI CNMGR] entry_bearer_detail_info")
	TRC_MSG(MMI_CNMGR_SHOW_BEARER_DETAIL_INFO, "[MMI CNMGR] show_bearer_detail_info")
	TRC_MSG(MMI_CNMGR_CHANGE_SELECT_BEARER, "[MMI CNMGR] change select bearer: select idx (%d, %d), account id %d, connect type %d, dialup type %d")
	TRC_MSG(MMI_CNMGR_APP_ENTRY_BEARER_DETAIL_INFO, "[MMI CNMGR] app_entry_bearer_detail_info: type %d, account %d, icon %d")
	TRC_MSG(MMI_CNMGR_REDRAW_BEARER_LIST, "[MMI CNMGR] redraw_bearer_list: old bearer num %d, current bearer num %d")
	TRC_MSG(MMI_CNMGR_BEARER_DATA_UPDATE_NOTIFY_CALLBACK, "[MMI CNMGR] bearer_data_update_notify_callback")
	TRC_MSG(MMI_CNMGR_APP_SHOW_BEARER_DETAIL_INFO, "[MMI CNMGR] app_show_bearer_detail_info: acct_id %d, real_acct_id %d")


	/*App Name: DTCNT*/

	TRC_MSG(TRC_DATA_CONNECTION_SIM_GET_SIM_PREFERENCE, "[Data connection SIM] srv_dtcnt_get_sim_preference, sim_type = %d, ret = %d")
	TRC_MSG(TRC_DATA_CONNECTION_SIM_SET_SIM_PREFERENCE, "[Data connection SIM] srv_dtcnt_set_sim_preference, sim_type = %d, ret = %d")
	TRC_MSG(TRC_DTCNT_IS_ACCOUNT_IN_USE, "[DTCNT API] mmi_dtcnt_is_account_in_use, decoded_pure_id = %d, account_id = %d")
	TRC_MSG(TRC_DTCNT_GET_MNC_LEN_IN_EF_AD, "[DTCNT APP] srv_sim_ctrl_get_mnc_len_in_ef_ad, digits_of_mnc = %d, sim = %d")
	TRC_MSG(TRC_DTCNT_GET_IMSI, "[DTCNT APP] srv_sim_ctrl_get_imsi, imsi[4] = %c, imsi[5] = %c, digits_of_mnc = %d")
	TRC_MSG(TRC_DTCNT_GET_DEFAULT_ACCOUNT, "[DTCNT APP] srv_dtcnt_get_default_account, sim_type = %d, default_account = %d, ret = %d")
	TRC_MSG(TRC_DTCNT_SET_DEFAULT_ACCOUNT, "[DTCNT APP] mmi_dtcnt_set_account_default_app_type, sim_type = %d, default_account = %d, result = %d")
	...
	TRC_MSG(MMI_DTCNT_ACCOUNT_NAME_GET_FROM_L4C_ERROR, "[DATA ACCOUNT] Length of account name gotten from L4C is error. length = [%d]")    
	TRC_MSG(MMI_DTCNT_DELETION_REGISTRATION_REQ, "[DDATA ACCOUNT] Data account deletion notification registration req: app_id = [%d], desire_acct_id = [%d], registration flag = [%d]")    
	TRC_MSG(MMI_DTCNT_DELETION_REGISTRATION_CNF, "[DDATA ACCOUNT] Data account deletion notification registration cnf: app_id = [%d], desire_acct_id = [%d], registration flag = [%d], result = [%d]")    
	TRC_MSG(MMI_DTCNT_DELETION_NOTIFICATION_IND, "[DDATA ACCOUNT] Data account deletion notification: app_id = [%d], desired_acct_id = [%d], bearer_type = [%d], mod_id = [%d]")    
	TRC_MSG(MMI_DTCNT_ADD_ACCOUNT, "[DDATA ACCOUNT] Data account add: current CSD number = [%d], ori_acct_id = [%d], display_index = [%d]") 
	TRC_MSG(MMI_DTCNT_ADD_GPRS_ACCOUNT, "[DDATA ACCOUNT] Data account GPRS add: current GPRS number = [%d], ori_acct_id = [%d], display_index = [%d]")
	TRC_MSG(MMI_DTCNT_EDIT_ACCOUNT, "[DDATA ACCOUNT] Data account edit: current CSD number = [%d], ori_acct_id = [%d], display_index = [%d]") 
	TRC_MSG(MMI_DTCNT_EDIT_GPRS_ACCOUNT, "[DDATA ACCOUNT] Data account GPRS edit: current GPRS number = [%d], ori_acct_id = [%d], display_index = [%d]")
	TRC_MSG(MMI_DTCNT_OTA_INTALL_ABORT, "[DATA ACCOUNT OTA] OTA Install ABORT!")
	TRC_MSG(MMI_DTCNT_OTA_INTALL_ABORT_STEP, "[DATA ACCOUNT OTA] OTA Install ABORT step = %d")
	TRC_MSG(MMI_DTCNT_PROV_IND_HDLR, "[DATA ACCOUNT OTA] prov_ind_hdlr cca_gid = %d, prov_ind = %d!")
	TRC_MSG(MMI_DTCNT_PROV_ENTRY_INSTALL_CSD_GPRS, "[DATA ACCOUNT OTA] prov_entry_install_csd_gprs()")
	TRC_MSG(MMI_DTCNT_PROV_PREPARE_PROF_INFO, "[DATA ACCOUNT OTA] prov_prepare_prof_info()")
	TRC_MSG(MMI_DTCNT_PROV_ENTRY_INSTALL_CSD_GPRS_TYPE, "[DATA ACCOUNT OTA] prov_entry_install_csd_gprs() type = %d")
	TRC_MSG(MMI_DTCNT_ON_SIM_DETECTED, "[DTCNT APP] mmi_dtcnt_on_sim_detected, detection_type = %d, sim = %d")
	TRC_MSG(MMI_DTCNT_ON_SIM_DETECTED_DELETED, "[DTCNT APP] mmi_dtcnt_on_sim_detected, deleted profile id = %d")
	TRC_MSG(MMI_DTCNT_ON_SIM_DETECTED_UNDELETED, "[DTCNT APP] mmi_dtcnt_on_sim_detected, left profile id = %d, left_num = %d")
	TRC_MSG(MMI_DTCNT_ON_BEARER_INFO, "[DTCNT APP] mmi_dtcnt_on_cbm_bearer_info, pure_id = %d, left_profile_num = %d")
	TRC_MSG(MMI_DAPP_ON_HOME_PLMN_CHANGED, "[DTCNT APP] mmi_dtcnt_on_home_plmn_changed, gemini value = %d, process = %d, val = %d")
	TRC_MSG(MMI_DAPP_DELETE_ANY_SIM_PROFILE, "[DTCNT APP] mmi_dtcnt_delete_any_sim_profile, sim type = %d")
	TRC_MSG(MMI_DAPP_IS_CONF_FILE_EXIST, "[DTCNT APP] mmi_dtcnt_is_conf_file_exit, result = %d")
	TRC_MSG(MMI_DAPP_XML_PARSE_SIM, "[DTCNT APP] mmi_dtcnt_xml_parse, sim enum = %d")
	TRC_MSG(MMI_DAPP_INIT, "[DTCNT APP] dtcnt_app_init() id = %d, result = %d")
	TRC_MSG(MMI_DAPP_GET_CSD_PROF, "[DTCNT APP] get_csd_profile() profile = %d proxy address : %d.%d.%d.%d - port = %d")
	TRC_MSG(MMI_DAPP_GET_GPRS_PROF, "[DTCNT APP] get_gprs_profile() profile = %d proxy address : %d.%d.%d.%d - port = %d")
	TRC_MSG(MMI_DAPP_SET_CSD_ACCNT, "[DTCNT APP] set_csd_account() profile = %d")
	TRC_MSG(MMI_DAPP_SET_CSD_PROF, "[DTCNT APP] set_csd_profile() proxy address : %d.%d.%d.%d - port = %d")
	TRC_MSG(MMI_DAPP_SET_GPRS_ACCNT, "[DTCNT APP] set_gprs_account() profile = %d")
	TRC_MSG(MMI_DAPP_SET_GPRS_PROF, "[DTCNT APP] set_gprs_profile() proxy address : %d.%d.%d.%d - port = %d")
	TRC_MSG(MMI_DAPP_SIM_PROC, "[DTCNT APP] dtcnt_sim_proc() evt_id = %d")
	TRC_MSG(MMI_DAPP_PROCESS_SIM_CHANGE, "[DTCNT APP] process_sim_change() id = %d")
	TRC_MSG(MMI_DAPP_SIM_CARD_CHANGED, "[DTCNT APP] is_sim_card_changed() id = %d, result = %d")
	TRC_MSG(MMI_DAPP_PRE_INSTALL_PARSE, "[DTCNT APP] pre_install_parse() id = %d")
	TRC_MSG(MMI_DAPP_ADD_SIM_PROF, "[DTCNT APP] add_sim_prof() prof_id = %d, result = %d")
	TRC_MSG(MMI_DAPP_ADD_CONFIG_ACCOUNT, "[DTCNT APP] add_config_account() id = %d, sim_match = %d")
	TRC_MSG(MMI_DAPP_ACCT_IS_VALID, "[DTCNT APP] mmi_dtcnt_acct_is_valid, encode_acct_id = %d")
	TRC_MSG(MMI_DAPP_ACCT_IS_VALID_APPINFO, "[DTCNT APP] mmi_dtcnt_acct_is_valid, app_str_id = %d, app_icon_id = %d")
	TRC_MSG(MMI_DAPP_SUBSCRIBE_ACCOUNT, "[DTCNT APP] mmi_dtcnt_subscribe_account, acct_old = %d, acct_new = %d, app_str_id = %d, app_icon_id = %d")
	TRC_MSG(MMI_PROV_INSTALL_ONE_DONE, "[DTCNT PROV] prov_install_one_done() result = %d, prov_ind = %")
	TRC_MSG(MMI_PROV_INSTALL_ONE_DONE_REPLACE, "[DTCNT PROV] prov_install_one_done replace = %d")
	TRC_MSG(MMI_PROV_INSTALL_RSP, "[DTCNT PROV] prov_install_rsp() callback = %x")
	TRC_MSG(MMI_PROV_SELECT_REPLACE, "[DTCNT PROV] prov_select_replace() callback = %x")
	TRC_MSG(MMI_PROV_ENTRY_SELECT_PROFILE, "[DTCNT PROV] prov_entry_select_profile() step = %x")
	TRC_MSG(MMI_PROV_OTA_CCA_RSP_HANDLER, "[DTCNT PROV] ota_cca_rsp_handler() callback = %x")
	TRC_MSG(MMI_PROV_SELECT_BACK, "[DTCNT PROV] mmi_dtcnt_prov_select_back()")
	TRC_MSG(MMI_DXML_NODE_VERSION, "[DTCNT XML] dtcnt_node_version() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_SIM, "[DTCNT XML] dtcnt_node_sim() sim_match = %d")
	TRC_MSG(MMI_DXML_NODE_ITEM, "[DTCNT XML] dtcnt_node_item() bearer = %d")
	TRC_MSG(MMI_DXML_NODE_APPTYPE, "[DTCNT XML] dtcnt_node_apptype() data_len = %d, app_type = %d")
	TRC_MSG(MMI_DXML_NODE_TITLE, "[DTCNT XML] dtcnt_node_title() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_NUMBER, "[DTCNT XML] dtcnt_node_number() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_LINETYPE, "[DTCNT XML] dtcnt_node_linetype() data_len = %d, dial_type = %d")
	TRC_MSG(MMI_DXML_NODE_SPEED, "[DTCNT XML] dtcnt_node_speed() data_len = %d, speed_rate = %d")
	TRC_MSG(MMI_DXML_NODE_HOMEPAGE, "[DTCNT XML] dtcnt_node_homepage() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_PROXY, "[DTCNT XML] dtcnt_node_proxy() use_proxy = %d")
	TRC_MSG(MMI_DXML_NODE_USERNAME, "[DTCNT XML] dtcnt_node_username() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_PASSWORD, "[DTCNT XML] dtcnt_node_password() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_PRIMARYDNS, "[DTCNT XML] dtcnt_node_primarydns() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_SECONDDNS, "[DTCNT XML] dtcnt_node_seconddns() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_APN, "[DTCNT XML] dtcnt_node_apn() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_AUTH, "[DTCNT XML] dtcnt_node_authentication() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_IPADDRESS, "[DTCNT XML] dtcnt_node_ipaddress() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_SUBNET, "[DTCNT XML] dtcnt_node_subnet() data_len = %d")
	TRC_MSG(MMI_DXML_NODE_CONNTYPE, "[DTCNT XML] dtcnt_node_conntype() data_len = %d")
	TRC_MSG(MMI_DXML_ELEM_START_HDLR, "[DTCNT XML] mmi_dtcnt_elem_start_hdlr() error = %d")
	TRC_MSG(MMI_DXML_ELEM_END_HDLR, "[DTCNT XML] mmi_dtcnt_elem_end_hdlr() error = %d")
	TRC_MSG(MMI_DXML_DATA_HDLR, "[DTCNT XML] mmi_dtcnt_data_hdlr() error = %d")
	TRC_MSG(MMI_DXML_RESUME_PARSE_HDLR, "[DTCNT XML] mmi_dtcnt_resume_parse_hdlr()")
	TRC_MSG(MMI_DXML_XML_PARSE, "[DTCNT XML] mmi_dtcnt_xml_parse()")
	TRC_MSG(MMI_DXML_ACCOUNT_PARSE_END, "[DTCNT XML] mmi_dtcnt_account_parse_end() ret = %d")
	TRC_MSG(MMI_DXML_ACCOUNT_PARSE_END_DUAL_SIM_DUAL_FILE, "[DTCNT XML]mmi_dtcnt_account_parse_end, dual sim, dual file")
	TRC_MSG(MMI_DXML_ACCOUNT_PARSE_END_DUAL_SIM_SINGLE_FILE, "[DTCNT XML]mmi_dtcnt_account_parse_end, dual sim, single file")
	TRC_MSG(MMI_DXML_ACCOUNT_PARSE_END_SINGLE_SIM_SINGLE_FILE, "[DTCNT XML]mmi_dtcnt_account_parse_end, single sim, single file")
	TRC_MSG(CUI_DTCNT_SELECTION_CREATE_ERROR, "[DTCNT CUI] selection cui create error")
	TRC_MSG(CUI_DTCNT_SELECTION_CREATE_INFO, "[DTCNT CUI] selection cui create info, parent_id = %d, allocate_index = %d")
	TRC_MSG(CUI_DTCNT_SELECTION_INIT_INFO, "[DTCNT CUI] selection cui set parameter info, cui_id = %d, index = %d")
	TRC_MSG(CUI_DTCNT_SELECTION_INIT_DETAIL, "[DTCNT CUI] selection cui set parameter detail bearer = %d, app_id = %d, acct_id = %d, option = %d, sim_info = %d")
	TRC_MSG(CUI_DTCNT_SELECTION_RUN_INFO, "[DTCNT CUI] selection cui run info cui_id = %d, index = %d")
	TRC_MSG(CUI_DTCNT_SELECTION_RUN_ERROR, "[DTCNT CUI] selection cui run info cui_id = %d, index = %d")
	TRC_MSG(CUI_DTCNT_SELECTION_CLOSE_INFO, "[DTCNT CUI] selection cui close cui_id = %d, index = %d")
	TRC_MSG(CUI_DTCNT_SELECTION_CUI_CONTEXT_INFO, "[DTCNT CUI] selection cui context info, index = %d, valid = %d, acct_id = %d")
	...
	TRC_MSG(MMI_BT_CONN_DIALER_AUTO_RECONN, "[BTConn] mmi_bt_dialer_auto_reconn_hdlr(), reconn_flag = %d, accu_time = %d")
	TRC_MSG(MMI_BT_CONN_DIALER_AUTO_RECONN_STATUS, "[BTConn] BT Dialer auto reconn status, conn_status = %d, reconn_flag = %d, accu_time = %d")
	...
	TRC_MSG(MMI_BT_ANTILOST_RSSI_CNF, "[antilost]mmi_bt_cm_read_rssi_cnf_hdler()")
	TRC_MSG(MMI_BT_ANTILOST_RSSI, "[antilost]evt_data:%d, alert_threshold:%d")
	TRC_MSG(MMI_BT_ANTILOST_NMGR_CALLBACK, "[antilost]mmi_bt_antilost_nmgr_callback()")
	TRC_MSG(MMI_BT_ANTILOST_IND, "[antilost]mmi_bt_anti_lost_entry_indication_screen()")
	TRC_MSG(MMI_BT_ANTILOST_IND_CANCEL, "[antilost]mmi_bt_anti_lost_indication_cancle_hdlr()")
	TRC_MSG(MMI_BT_ANTILOST_IND_LSK, "[antilost]mmi_bt_anti_lost_ind_lsk_hdlr()")
	TRC_MSG(MMI_BT_ANTILOST_IND_RESET, "[antilost]mmi_bt_anti_lost_ind_reset()")
	TRC_MSG(MMI_BT_ANTILOST_CONN_EVT_HDLR, "[antilost]mmi_bt_anti_lost_connect_complete_hdlr,result %d")
	TRC_MSG(MMI_BT_ANTILOST_DISCONN_EVT_HDLR, "[antilost]mmi_bt_anti_lost_disconn_complete_hdlr,profile %x")
	TRC_MSG(MMI_BT_ANTILOST_ENTRY_SETTING, "[antilost]mmi_bt_entry_anti_lost()")
	TRC_MSG(MMI_BT_ANTILOST_ENTRY_SETTING_LSK, "[antilost]mmi_bt_anti_lost_setting_lsk_hdlr()")
	TRC_MSG(MMI_BT_ANTILOST_ENTRY_SETTING_SAVE, "[antilost]mmi_bt_anti_lost_setting_lsk_hdlr() current_status: %d, alert_mode: %d")
	TRC_MSG(MMI_BT_ANTILOST_GET_ALERTMODE, "[antilost]mmi_bt_anti_lost_get_alert_mode(): %d")
	TRC_MSG(MMI_BT_ANTILOST_GET_STATUS, "[antilost]mmi_bt_anti_lost_get_status(): %d")
	TRC_MSG(MMI_BT_ANTILOST_TIMER_RESTART, "[antilost]mmi_bt_anti_lost_restart_timer()")
	TRC_MSG(MMI_BT_ANTILOST_TIMER_STOP, "[antilost]mmi_bt_anti_lost_stop_timer()")
	TRC_MSG(MMI_BT_ANTILOST_TIMER_START, "[antilost]mmi_bt_anti_lost_start_timer():time=%d")
	TRC_MSG(MMI_BT_ANTILOST_TIMER_OUT, "[antilost]mmi_bt_anti_lost_timeout_hdlr(")
...
END_TRACE_MAP(MOD_MMI_CONN_APP)
