...
volatile kal_int8 count_judge_gps_app_timer = 0;	//add by lrf
volatile kal_uint16 count_system_time = 0;
volatile kal_int8 count_judge_gps_send_timer = 0;
volatile kal_int32 pwr_get_bat_vol = 0;
/***************************************************************************** 
* Local Function
*****************************************************************************/
...
extern U8 Yd_get_runstatus();//lrf
...
void mmi_charbat_led_hdlr(srv_charbat_evt_notify_struct* p)
{
	
    if(g_charbat_context.ChargingComplete)
		//(p->status==PMIC_CHARGE_COMPLETE)
    {
                    kal_prompt_trace( MOD_WPS,"mmi_charbat_main_evt_hdlr full charging");
	             srv_led_pattern_stop_battery_low();
                    srv_led_pattern_stop_normal();
                    srv_led_pattern_stop_charging();	
                    srv_led_pattern_start_full_charging();
    }
    else if(g_charbat_context.isChargerConnected)
		//((p->status==PMIC_CHARGER_IN)||(p->status==PMIC_USB_CHARGER_IN))
    {
                    kal_prompt_trace( MOD_WPS,"mmi_charbat_main_evt_hdlr is_charging");
                    srv_led_pattern_stop_battery_low();
                    srv_led_pattern_stop_normal();
                    srv_led_pattern_stop_full_charging();
                    srv_led_pattern_start_charging();	
    }
    else if(p->battery_level<=BATTERY_LOW_WARNING)
    {
                    kal_prompt_trace( MOD_WPS,"mmi_charbat_main_evt_hdlr start_battery_low");
                    srv_led_pattern_stop_charging();
                    srv_led_pattern_stop_normal();
                    srv_led_pattern_stop_full_charging();
    ...
    {
                    kal_prompt_trace( MOD_WPS,"mmi_charbat_main_evt_hdlr start_normal");
                    srv_led_pattern_stop_charging();
                    srv_led_pattern_stop_battery_low();
                    srv_led_pattern_stop_full_charging();
                    srv_led_pattern_start_normal();
    }
}
#endif
/*****************************************************************************
...
mmi_ret mmi_charbat_main_evt_hdlr(mmi_event_struct* para)
{
	...
	/**yanchunhai add 20120817    begin  ************************/
		...
		b_charge_usb_in = KAL_FALSE;
             TRACE_UART("mmi_charbat_main_evt_hdlr  b_charge_usb_in=%d",b_charge_usb_in);

		
	/**yanchunhai add 20120817    end  ************************/
...
static void mmi_charger_check_low_power_off(S32 battery_level)
{
    ...
    if (!srv_charbat_is_charging() && !srv_bootup_is_booting())
    {
        ...
        //RJ_GPS_record_date(L"lowpower.dat", "power off" , KAL_FALSE);   
    #endif
...
void BatteryStatusIndication(U8 battery_voltage)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
#ifdef  DW02_PROJECT    //只处理一次	
     static kal_bool b_rj_notice_low_bat_flag = KAL_TRUE;    //only once
#endif   
    static kal_uint8 count_alarm = 0;
    S32 battery_grid_percentage;
    ilm_struct       *rj_ilm = NULL; //lrf
    kal_uint32 m_bat_voltage;
    kal_uint32 m_lbv_voltage;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* reboot after 24 hour */

   if(count_system_time > (24*60*7))
   {
   ...
   else
   {
    ...
    switch (battery_voltage)
    {
            ...
            RJ_GPS_RedLightWink();
              {
                  ...
                  if(!(count_alarm%5) &&(count_alarm<11)&&(KAL_FALSE == b_charge_usb_in))
                  {
                        rj_low_volt_alarm();
                        count_alarm++;
                  }
                  else if (KAL_TRUE == b_charge_usb_in)
                  {
                        count_alarm = 0;
                  }
	 ...
	 #ifdef  DW02_PROJECT    //只处理一次
            if ((KAL_TRUE == b_rj_notice_low_bat_flag) && !srv_charbat_is_charging() && !srv_bootup_is_booting())
            {
                 b_rj_notice_low_bat_flag = KAL_FALSE;
            }	
	 #endif				
        #endif
        */
...
/*
            ...
            battery_grid_percentage = 33 * (battery_voltage - BATTERY_LEVEL_0);
                #ifdef RJ_GPS_APP
		             rj_led_status_info.b_BATTERY_IS_LOW = KAL_FALSE ;
				if(BATTERY_LEVEL_3 == battery_voltage )
				{
					extern kal_bool b_charge_usb_in;
					b_charge_usb_in = KAL_FALSE;
				}          
		                RJ_GPS_RedLightWink();
			#ifdef  DW02_PROJECT
			   //如果不在低电状态，并且也不在充电状态，将标志位恢复，等再次到低电时发短信给用户报警
			  if ((KAL_FALSE == b_rj_notice_low_bat_flag) && !srv_charbat_is_charging() && !srv_bootup_is_booting())
	                {
	                     b_rj_notice_low_bat_flag = KAL_TRUE;
			  }
			#endif		  
                #endif
                */
...

	{
		...
		if(m_bat_voltage<m_lbv_voltage)
		{
			//rj_led_status_info.b_BATTERY_IS_LOW = KAL_TRUE ;
            		//RJ_GPS_RedLightWink();	
      			if(KAL_FALSE == b_charge_usb_in)
      			{
      				//if(!(count_alarm%10) && count_alarm>0)
...
#ifdef RJ_GPS_APP
       kal_prompt_trace(  MOD_WPS,"SetDateTime---------------");
       kal_prompt_trace(  MOD_SOC,"count_judge_gps_app_timer -%d,%d",count_judge_gps_app_timer,count_judge_gps_send_timer);
           if(count_judge_gps_app_timer > 8)
            {
            ...
            {
                count_judge_gps_app_timer ++;
            }  
		if(count_judge_gps_send_timer > 8)
            {              
