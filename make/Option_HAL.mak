...
ifdef CMOS_SENSOR
  ifeq ($(strip $(CMOS_SENSOR)),NONE)
    ...
    ifeq ($(strip $(PLATFORM)),MT6261)
      ifeq ($(findstring _SERIAL,$(CMOS_SENSOR)),)
        $(warning ERROR: For MT6261, please set CMOS_SENSOR as serial sensor XXX_SERIAL)
        DEPENDENCY_CONFLICT = TRUE
      endif
    endif
  endif
...
ifdef SPH_ENH_PACKAGE
  ifeq ($(strip $(SPH_ENH_PACKAGE)),MAGILOUDNESS_SUPPORT)
    COM_DEFS += __MAGILOUDNESS_SUPPORT__
    COMPLIST += dp_engine
  endif
  ifeq ($(strip $(SPH_ENH_PACKAGE)),TDDNC_SUPPORT)
    COM_DEFS += __TDDNC_SUPPORT__
  endif
  ifeq ($(filter TDDNC_SUPPORT MAGILOUDNESS_SUPPORT,$(SPH_ENH_PACKAGE)),)
    $(warning ERROR: SPH_ENH_PACKAGE is not defined. Please define SPH_ENH_PACKAGE as TDDNC_SUPPORT or MAGILOUDNESS_SUPPORT!)
    DEPENDENCY_CONFLICT = TRUE 
  endif
endif
