...
static kal_bool USB_IN_BOOT = KAL_FALSE;

static kal_bool USB_Fast_Meta_mode = KAL_FALSE;
#if defined(MTK_SLEEP_ENABLE) && !defined(__DRV_SUPPORT_LPWR__)
...
//static DCL_STATUS DCL_USB_CTRL_API_USBDL_Is_USB_IN_BOOT(DCL_CTRL_DATA_T *data);
...
static void USB_DMA1_Callback(USB_EP_DIRECTION direction);
static void USB_DMA1TX_Callback(void);
static void USB_DMA1RX_Callback(void);
#if defined(__USB_SUPPORT_MULTIPLE_DMA_CHANNEL__)
static void USB_DMA2_Callback(USB_EP_DIRECTION direction);
static void USB_DMA2TX_Callback(void);
#if !defined(DRV_USB_DMA2_RX_REDUCED)
static void USB_DMA2RX_Callback(void);
#endif
#endif
#else
...
#else
#define DCL_USB_CTRL_API_USBDMA_Drv_Create_ISR NULL
#define DCL_USB_CTRL_API_USBDMA_Drv_Activate_ISR NULL
#endif
...
#else
#define DCL_USB_CTRL_API_USB_Release_Drv NULL
#define DCL_USB_CTRL_API_USB_Switch_DMA_Burst_mode NULL
#define DCL_USB_CTRL_API_USB_Register_DMA_Channel NULL
#define DCL_USB_CTRL_API_USB_Set_DMA_Burst_Mode NULL
#define DCL_USB_CTRL_API_USB_Enter_Test_Mode NULL
#define DCL_USB_CTRL_API_USB_Reset_FIFO NULL
#define DCL_USB_CTRL_API_USB_FIFO_Not_Empty NULL
#define DCL_USB_CTRL_API_USB_Get_RX_EP_ISR_Status NULL
#endif	
...
/* under construction !*/
#else
#define DCL_USB_CTRL_API_USB_Enable_DMA_Timer_Count NULL
#define DCL_USB_CTRL_API_USB_Check_DMA_Time_Out NULL
#define DCL_USB_CTRL_API_USB_Get_DMA_Real_Count NULL
#endif
...
#else
#define DCL_USB_CTRL_API_USB_Set_DMA_Limiter NULL
#endif
...
#else
#define DCL_USB_CTRL_API_USB_UART_Share NULL
#endif
...
//	USBDMA_PPStop(dma_chan);
	...
	if(g_UsbDrvInfo.dma_port[direction][ep_num - 1] != 0)
	{
		if (DMA_CheckRunStat(g_UsbDrvInfo.dma_port[direction][ep_num - 1]))
		{
			DMA_Stop(g_UsbDrvInfo.dma_port[direction][ep_num - 1]);
		}

		savedMask = SaveAndSetIRQMask();
		if (DMA_CheckITStat(g_UsbDrvInfo.dma_port[direction][ep_num - 1]))
		{
			DMA_ACKI(g_UsbDrvInfo.dma_port[direction][ep_num - 1]);
			IRQClearInt(IRQ_DMA_CODE);
		}
		RestoreIRQMask(savedMask);
	}

	g_UsbDrvInfo.dma_pktrdy[direction][ep_num - 1] = KAL_FALSE;
	g_UsbDrvInfo.dma_running[direction][ep_num - 1] = KAL_FALSE;
#else  /* (defined(__MEUT__)||defined(IC_MODULE_TEST)||defined(DRV_USB_DMA_NO_SHARE_CHANNELS) */
	kal_uint32 savedMask;
...
#endif  /* (defined(__MEUT__)||defined(IC_MODULE_TEST)||defined(DRV_USB_DMA_NO_SHARE_CHANNELS) */
#endif  /* defined(DRV_USB_IP_V3) */
}
...
static void USB_DMA1_Callback(USB_EP_DIRECTION direction)
{
	usb_dma_callback callback = g_UsbDrvInfo.dma_callback[direction][0];



	USB_Dbg_Trace(USB_DBG_DMA_CALLBACK, 0, 0);

	g_UsbDrvInfo.dma_callback[direction][0] = NULL;
...
/* under construction !*/
	...
	if(g_UsbDrvInfo.dma_pktrdy[direction][0] == KAL_TRUE)
	{
		if(g_UsbDrvInfo.dma_dir[direction][0] == USB_EP_TX_DIR)
		{
	...
	g_UsbDrvInfo.dma_pktrdy[direction][0] = KAL_FALSE;

	/* If application callback function does not request to clear drv_running set by itself, clear running state here */
	if(g_UsbDrvInfo.dma_callback_upd_run[direction][0] == KAL_FALSE)
		g_UsbDrvInfo.dma_running[direction][0] = KAL_FALSE;
...
static void USB_DMA1TX_Callback(void)
{
	USB_DMA1_Callback(USB_EP_TX_DIR);
}
static void USB_DMA1RX_Callback(void)
{
	USB_DMA1_Callback(USB_EP_RX_DIR);	
}
...
static void USB_DMA2_Callback(USB_EP_DIRECTION direction)
{
	usb_dma_callback callback = g_UsbDrvInfo.dma_callback[direction][1];


	g_UsbDrvInfo.dma_callback[direction][1] = NULL;
...
/* under construction !*/
	...
	/* The last one pkt length not multiple of MAX_PKT_LEN, clear corresponding bits*/
	if (g_UsbDrvInfo.dma_pktrdy[direction][1] == KAL_TRUE)
	{
		if(g_UsbDrvInfo.dma_dir[direction][1] == USB_EP_TX_DIR)
			USB_HW_EP_Tx_Ready(2, USB_ENDPT_BULK);
		else
			USB_HW_EP_Rx_Ready(2, USB_ENDPT_BULK);
	}

	g_UsbDrvInfo.dma_pktrdy[direction][1] = KAL_FALSE;

	/* if application callback function dose not request to clear drv_runnung set by itself, clear running state here*/
	if(g_UsbDrvInfo.dma_callback_upd_run[direction][1] == KAL_FALSE)
		g_UsbDrvInfo.dma_running[direction][1] = KAL_FALSE;
...
static void USB_DMA2TX_Callback(void)
{
	USB_DMA2_Callback(USB_EP_TX_DIR);
}

#if !defined(DRV_USB_DMA2_RX_REDUCED)
static void USB_DMA2RX_Callback(void)
{
	USB_DMA2_Callback(USB_EP_RX_DIR);	
}
#endif
...
//	kal_bool    Rx_FIFO_not_empty = KAL_FALSE;
	...
	drv_trace4(TRACE_FUNC, (kal_uint32)USB_HISR_State, IntrUSB, USBMode, USB_DRV_Reg8(USB_LINE_STATE), USB_DRV_Reg8(USB_OPSTATE));
	drv_trace4(TRACE_FUNC, (kal_uint32)USB_HISR_State, IntrTx, USB_DRV_Reg(USB_INTRTXE), IntrRx, USB_DRV_Reg(USB_INTRRXE));	
...
static void USB_HW_Reset(void)
{
	...
	PDN_CLR(PDN_USB);
		...
		USB_DRV_SetBits32(USB_U1PHYCR0, U1PHYCR0_RG_USB11_RST);
		USB_DRV_ClearBits32(USB_U1PHYCR0, U1PHYCR0_RG_USB11_RST);	
	#endif
#endif
	PDN_SET(PDN_USB);
...
#if (defined(__MEUT__)||defined(IC_MODULE_TEST)||defined(DRV_USB_DMA_NO_SHARE_CHANNELS))
		g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][index] = 0;
		g_UsbDrvInfo.dma_pktrdy[USB_EP_TX_DIR][index] = KAL_FALSE;
		g_UsbDrvInfo.dma_running[USB_EP_TX_DIR][index] = KAL_FALSE;
		g_UsbDrvInfo.dma_callback_upd_run[USB_EP_TX_DIR][index] = KAL_FALSE;
		g_UsbDrvInfo.dma_callback[USB_EP_TX_DIR][index] = NULL;		
		g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][index] = 0;
		g_UsbDrvInfo.dma_pktrdy[USB_EP_RX_DIR][index] = KAL_FALSE;
		g_UsbDrvInfo.dma_running[USB_EP_RX_DIR][index] = KAL_FALSE;
		g_UsbDrvInfo.dma_callback_upd_run[USB_EP_RX_DIR][index] = KAL_FALSE;
		g_UsbDrvInfo.dma_callback[USB_EP_RX_DIR][index] = NULL;				
#else
		g_UsbDrvInfo.dma_pktrdy[index] = KAL_FALSE;
		g_UsbDrvInfo.dma_running[index] = KAL_FALSE;
		g_UsbDrvInfo.dma_callback_upd_run[index] = KAL_FALSE;
...
#if (defined(__MEUT__)||defined(IC_MODULE_TEST)||defined(DRV_USB_DMA_NO_SHARE_CHANNELS))
		if (g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][index] != 0) {
			if (DMA_CheckRunStat(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][index])) 
			{
				DMA_Stop(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][index]);
			}
	
			savedMask = SaveAndSetIRQMask();
			if (DMA_CheckITStat(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][index])) /* check if interrupt is pending and waiting for service */
			{ 
				DMA_ACKI(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][index]);
				IRQClearInt(IRQ_DMA_CODE);
			}
			RestoreIRQMask(savedMask);	
		}
		if (g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][index] != 0) {
			if (DMA_CheckRunStat(g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][index])) {
				DMA_Stop(g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][index]);
			}
	
			savedMask = SaveAndSetIRQMask();
			if (DMA_CheckITStat(g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][index])) { /* check if interrupt is pending and waiting for service */
				DMA_ACKI(g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][index]);
				IRQClearInt(IRQ_DMA_CODE);
			}
			RestoreIRQMask(savedMask);	
		}
...
static DCL_STATUS DCL_USB_CTRL_API_USB_Pull_Up_DP_Line(DCL_CTRL_DATA_T *data)
{
	...
	#else
	USB_DRV_WriteReg(USB_INTRRX, ~USB_DRV_Reg(USB_INTRRX));
	#endif
#elif defined(DRV_USB_IP_V2)
#elif defined(DRV_USB_IP_V1)||defined(DRV_USB_IP_V1_PLUS)
	USB_DRV_Reg8(USB_INTRUSB);
...
static DCL_STATUS DCL_USB_CTRL_API_USB_Initialize_Drv_Phase1(DCL_CTRL_DATA_T *data)
{
	...
	USB_DRV_ClearBits8(USB_U1PHYCR1_1,USB_U1PHYCR1_1_RG_USB11_USBRESERVED_PMU); // switch BC11 path to USB
#endif	

#if defined(DRV_USB_PHY_T55_IP)
	USB_DRV_Reg8(USB_DUMMY);
	USB_DRV_ClearBits32(USB_U1PHYCR1, U1PHYCR1_RG_USB11_PHY_REV_7); // switch BC11 path to USB
	USB_DRV_SetBits32(USB_U1PHYCR1, (3<<17)); // save current
#endif
...
static DCL_STATUS DCL_USB_CTRL_API_USB_Get_DMA_Channel(DCL_CTRL_DATA_T *data)
{
	...
	USB_EP_DIRECTION direction;
#if defined(DRV_USB_IP_V3)||defined(DRV_USB_IP_V4) || defined(__MEUT__) ||defined(IC_MODULE_TEST)
	kal_bool same_chan;
	...
	if(direction == USB_EP_TX_DIR)
	{
		...
		{
		case 1:
				g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_tx_num - 1] = DMA_GetChannel(DMA_USB1TX);
				break;
		#if defined(__USB_SUPPORT_MULTIPLE_DMA_CHANNEL__)
			case 2:
				g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_tx_num - 1] = DMA_GetChannel(DMA_USB2TX);
				break;
		#endif /* __USB_SUPPORT_MULTIPLE_DMA_CHANNEL__ */
			default:
	...
	else if(direction == USB_EP_RX_DIR)
	{
		...
		{
		case 1:
			g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][ep_rx_num - 1] = DMA_GetChannel(DMA_USB1RX);
			break;
	#if defined(__USB_SUPPORT_MULTIPLE_DMA_CHANNEL__)&&(!defined(DRV_USB_DMA2_RX_REDUCED))
		case 2:
			g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][ep_rx_num - 1] = DMA_GetChannel(DMA_USB2RX);
			break;
	#endif /* __USB_SUPPORT_MULTIPLE_DMA_CHANNEL__ */
		default:
			EXT_ASSERT(0, (kal_uint32)ep_rx_num, 0 , 0);
			break;
		}
	}		
#else
...
//	USBDMA_PPStop(dma_chan);
	...
	USB_EP_Check(ep_num, direction, 0);
#if (defined(__MEUT__)||defined(IC_MODULE_TEST)||defined(DRV_USB_DMA_NO_SHARE_CHANNELS))
	if ((g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_num - 1] != 0) && (USB_EP_TX_DIR == direction)) {
		if (DMA_CheckRunStat(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_num - 1])) {
			DMA_Stop(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_num - 1]);
		}
	
		savedMask = SaveAndSetIRQMask();
		if (DMA_CheckITStat(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_num - 1])) {	
			DMA_ACKI(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_num - 1]);
			IRQClearInt(IRQ_DMA_CODE);
		}
		RestoreIRQMask(savedMask);
		g_UsbDrvInfo.dma_pktrdy[USB_EP_TX_DIR][ep_num - 1] = KAL_FALSE;
		g_UsbDrvInfo.dma_running[USB_EP_TX_DIR][ep_num - 1] = KAL_FALSE;		
	}
	
	if ((g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][ep_num - 1] != 0) && (USB_EP_RX_DIR == direction)) {
		if (DMA_CheckRunStat(g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][ep_num - 1])) {
			DMA_Stop(g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][ep_num - 1]);
		}
	
		savedMask = SaveAndSetIRQMask();
		if (DMA_CheckITStat(g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][ep_num - 1])) {	
			DMA_ACKI(g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][ep_num - 1]);
			IRQClearInt(IRQ_DMA_CODE);
		}
		RestoreIRQMask(savedMask);
		g_UsbDrvInfo.dma_pktrdy[USB_EP_RX_DIR][ep_num - 1] = KAL_FALSE;
		g_UsbDrvInfo.dma_running[USB_EP_RX_DIR][ep_num - 1] = KAL_FALSE;		
	}
#else
...
static DCL_STATUS DCL_USB_CTRL_API_USB_Free_DMA_Channel(DCL_CTRL_DATA_T *data)
{
	...
	kal_uint32 ep_tx_num;
	USB_EP_DIRECTION direction;
#if defined(DRV_USB_IP_V3)||defined(DRV_USB_IP_V4)|| defined(__MEUT__)||defined(IC_MODULE_TEST)||defined(DRV_USB_DMA_NO_SHARE_CHANNELS)
	kal_uint32 ep_rx_num;
#endif
#if defined(DRV_USB_IP_V3)||defined(DRV_USB_IP_V4)
	kal_bool same_chan;
	...
	else
	{
		...
		if (ep_tx_num && (g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_tx_num - 1] != 0) && (USB_EP_TX_DIR == direction)) {
			USB_HW_Stop_DMA_Channel(ep_tx_num, direction);
			DMA_FreeChannel(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_tx_num - 1]);
	
					g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_tx_num - 1] = 0;
		}
	
		if (ep_rx_num && (g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][ep_rx_num - 1] != 0) && (USB_EP_RX_DIR == direction)) {
			USB_HW_Stop_DMA_Channel(ep_rx_num, direction);
			DMA_FreeChannel(g_UsbDrvInfo.dma_port[USB_EP_RX_DIR][ep_rx_num - 1]);
...
//				USBDMA_PPStop(dma_chan);
	...
	if (((kal_bool)INT_QueryIsROMSpace(addr)== KAL_TRUE) || 
		...
		ASSERT(g_UsbDrvInfo.dma_running[direction][ep_num - 1]==KAL_FALSE);
		g_UsbDrvInfo.dma_running[direction][ep_num - 1] = KAL_TRUE;
	
		ASSERT(g_UsbDrvInfo.dma_port[direction][ep_num - 1] !=0);
	
		g_UsbDrvInfo.dma_callback[direction][ep_num - 1] = callback;
		g_UsbDrvInfo.dma_callback_upd_run[direction][ep_num - 1] = callback_upd_run;
		g_UsbDrvInfo.dma_dir[direction][ep_num - 1] = direction;
...
/* under construction !*/
		...
		if (ep_num == 1)
		{
			if(direction == USB_EP_TX_DIR)
			{
				...
				dma_input.callback = USB_DMA1TX_Callback;				
			}
			else
			{
				dma_menu.master = DMA_USB1RX;
				dma_input.callback = USB_DMA1RX_Callback;				
			}
		...
		else if (ep_num == 2)
		{
			if(direction == USB_EP_TX_DIR)
			{
				...
				dma_input.callback = USB_DMA2TX_Callback;				
			}
			else
			{
#if !defined(DRV_USB_DMA2_RX_REDUCED)
				dma_menu.master = DMA_USB2RX;
				dma_input.callback = USB_DMA2RX_Callback;				
#else
				ASSERT(0);
#endif
			}
		...
		if (length % USB_EP_BULK_MAXP)
			g_UsbDrvInfo.dma_pktrdy[direction][ep_num - 1] = KAL_TRUE;
		else
			g_UsbDrvInfo.dma_pktrdy[direction][ep_num - 1] = KAL_FALSE;
	
		/* Configure DMA */
		DMA_Config(g_UsbDrvInfo.dma_port[direction][ep_num - 1], &dma_input, KAL_TRUE);
#else
	USB_EP_Check(ep_num, direction, 0);
...
static DCL_STATUS DCL_USB_CTRL_API_USB_DMA_Get_Run_Status(DCL_CTRL_DATA_T *data)
{
	...
	USB_EP_Check(ep_num, direction, 0);
#if (defined(__MEUT__)||defined(IC_MODULE_TEST)||defined(DRV_USB_DMA_NO_SHARE_CHANNELS))
	result = g_UsbDrvInfo.dma_running[direction][ep_num-1];
#else
...
static DCL_STATUS DCL_USB_CTRL_API_USB_DMA_Set_Run_Status(DCL_CTRL_DATA_T *data)
{
	...
	USB_EP_Check(ep_num, direction, 0);
#if (defined(__MEUT__)||defined(IC_MODULE_TEST)||defined(DRV_USB_DMA_NO_SHARE_CHANNELS))
	g_UsbDrvInfo.dma_running[direction][ep_num-1] = status;
#else
...
//		EXT_ASSERT(0, 0, 0, 0);
	...
	/* polling until DMA done */
	while (DMA_CheckRunStat(g_UsbDrvInfo.dma_port[USB_EP_TX_DIR][ep_num - 1]))
	{
		/* check if other interrupt received*/
		g_UsbDrvInfo.is_unMaskUSB = KAL_FALSE;
		USB_HISR();
...
#endif
#endif
#if ((defined(__MEUT__)||defined(IC_MODULE_TEST)||defined(DRV_USB_DMA_NO_SHARE_CHANNELS)) && (defined(DRV_USB_IP_V1)||defined(DRV_USB_IP_V1_PLUS)))
	/* the last one pkt length not multiple of MAX_PKT_LEN, clear corresponding bits*/
	if (g_UsbDrvInfo.dma_pktrdy[USB_EP_TX_DIR][ep_num - 1])
	{
		if(g_UsbDrvInfo.dma_dir[USB_EP_TX_DIR][ep_num-1] != USB_EP_TX_DIR)
		{
...
//	ptr_CDCACM_fun9->u2Retresult= (DCL_BOOL)USB_Set_RNDIS_Mode((kal_bool)ptr_CDCACM_fun9->u2set);					
	...
	ptr_CDCACM_fun9->u2Retresult = KAL_TRUE;
...
#if 1
static DCL_STATUS DCL_USB_CTRL_API_USB_SleepLock(DCL_CTRL_DATA_T *data)
{
	#if defined(MTK_SLEEP_ENABLE) && !defined(__DRV_SUPPORT_LPWR__)
		DCL_BOOL *dcl_data_ptr = (DCL_BOOL*)data;
		kal_bool usb_data = (kal_bool)(*dcl_data_ptr);
		USB_SleepEnable(usb_data);
		return STATUS_OK;		
	#else
		return STATUS_FAIL;
	#endif 
...
DCL_USB_CTRL_API DCL_USB_DRV_API_T[] =
{
	...
	DCL_USB_CTRL_API_USB_SleepLock,
};
...
DCL_STATUS DclUSB_DRV_Control(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	if (cmd > USB_CLASS_CMD_START)
	{
		...
		else if ((cmd > USB_IMAGE_CLASS_START)&&(cmd < USB_IMAGE_CLASS_END))
		{
	...
	else if(cmd < USB_DRV_CMD_END)
	{
		if(DCL_USB_DRV_API_T[cmd] != NULL)
		{
...
#if (defined(DRV_USB_IP_V3)||defined(DRV_USB_IP_V4))&&defined(DRV_USB_PHY_CURRENT_CONSUMPTION)
	USB_Save_Current_Consumption();
#endif //(defined(DRV_USB_IP_V3)&&defined(DRV_USB_PHY_CURRENT_CONSUMPTION))
...
//		USB_DRV_ClearBits8(USB_U2PHYAC0_0,(USB_U2PHYAC0_0_RG_BGR_BGR_EN|USB_U2PHYAC0_0_RG_BGR_ISRC_EN|USB_U2PHYAC0_0_RG_BGR_CHP_EN));
    ...
    //reg[612] bit 3,1,0  = 1
	    ...
	    /* disable all interrupt and read all interrupt status to clear */
		USB_DRV_WriteReg8(USB_INTRIN1E, 0);
		USB_DRV_WriteReg8(USB_INTROUT1E, 0);
		USB_DRV_WriteReg8(USB_INTRUSBE, 0);	
		USB_DRV_Reg8(USB_INTRUSB);
	...
	#elif (defined(DRV_USB_PHY_T55_IP))	
		USB_DRV_ClearBits32(USB_U1PHYCR1, (3<<17)); // recover current
		USB_DRV_ClearBits32(USB_U1PHYCR1, U1PHYCR1_RG_USB11_PHY_REV_7);
...
static DCL_STATUS DCL_USB_CTRL_API_USBDL_Update_USB_Download_Mode(DCL_CTRL_DATA_T *data)
{
  ...
  USB_DRV_Reg8(USB_DUMMY);
...
#if defined(DRV_USB_HW_RST)	
	if(USBDL_mode == KAL_TRUE)
	{
		if (!((INT_BootMode() == MTK_FACTORY_MODE) 
			|| (INT_IsBootForUSBAT() == KAL_TRUE) 
		#if defined(__USB_BOOTUP_TRACE__)
		   || (is_init_log_enable == KAL_TRUE && is_init_stage == KAL_TRUE)
		#endif
		))
		{
			USBDL_mode = KAL_FALSE;
	...
	USB_IN_BOOT = USBDL_mode;
	
	if(USB_IN_BOOT == KAL_FALSE) 
	{				
		#if defined(DRV_USB_IP_V1_PLUS)	
		PDN_SET(PDN_USB);	
		UPLL_Disable(UPLL_OWNER_USB);	
		#if defined(DRV_USB_HW_RST)	&& defined(__USB_ENABLE__)		
		USB_HW_Reset();
		#endif
		#endif	       
	}
