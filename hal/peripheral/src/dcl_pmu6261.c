...
#ifdef __BTRF_BUCK_GPIO_
extern const char gpio_sim1_sio_pin;
extern const char gpio_sim1_srst_pin;
extern const char gpio_sim1_sclk_pin;
extern const char gpio_sim2_sio_pin;
extern const char gpio_sim2_srst_pin;
extern const char gpio_sim2_sclk_pin;
#endif
...
void pmu_long_press_shutdown_config(kal_uint32 enable)
{
	DCL_UINT32 savedMask;

	savedMask = PMU_SaveAndSetIRQMask();
	
	// enable long press feature
	PMU_DRV_SetData16((TEST_CON0 + RG_PWRKEY_RST_EN_OFFSET), RG_PWRKEY_RST_EN_MASK, (enable << RG_PWRKEY_RST_EN_SHIFT));
	// choose long press shutdown or reboot ,  0: reboot, 1: shutdown
	PMU_DRV_SetData16((TEST_CON0 + RG_LNGP_SHUTDOWN_SEL_OFFSET), RG_LNGP_SHUTDOWN_SEL_MASK, (enable << RG_LNGP_SHUTDOWN_SEL_SHIFT));
	// write trigger
...
#ifdef __BTRF_BUCK_GPIO_
char pmu_GPIO_ReturnMode(char port)
{
	DCL_HANDLE handle;
...
void BTRF_GPIO_power_check(void)
{
	DCL_HANDLE handle;
	PMU_CTRL_LDO_BUCK_SET_EN ldoSetEn;
	PMU_CTRL_LDO_BUCK_SET_VSIM_GPLDO_EN ldo_vsim_gpldo_en;

	if(pmu_GPIO_ReturnMode(gpio_sim1_sio_pin)==0 || pmu_GPIO_ReturnMode(gpio_sim1_srst_pin)==0 || pmu_GPIO_ReturnMode(gpio_sim1_sclk_pin)==0)
	{
		handle = DclPMU_Open(DCL_PMU, FLAGS_NONE);
	...
	else if(pmu_GPIO_ReturnMode(gpio_sim2_sio_pin)==0 || pmu_GPIO_ReturnMode(gpio_sim2_srst_pin)==0 || pmu_GPIO_ReturnMode(gpio_sim2_sclk_pin)==0)
	{
		handle = DclPMU_Open(DCL_PMU, FLAGS_NONE);
...
{
	kal_bool long_press_reboot=KAL_TRUE;
	//DCL_UINT32 register_val;
	//DCL_UINT32 product_type;
	...
	//register_val = PMU_DRV_ReadReg32(EFUSE_base + 0x108);
	//product_type = ((register_val & 0x30000000) >> 28);
	
	//if ((product_type == 0x1) || (product_type == 0x2)) // 00:Wild Card, 01:MT2501, 02:MT2502, 11:MT6261
	#if defined(MT2501) || defined(MT2502)
	{
		#ifndef __DRV_MISC_POWER_SWITCH__
		pmu_long_press_shutdown_config(long_press_reboot);
		#endif
	}
	#endif

#ifdef __BTRF_BUCK_GPIO_
	BTRF_GPIO_power_check();
#endif
