...
void chr_det_hisr_sendMes2BMT(module_type srcid, msg_type msgid)
{
   ilm_struct *BMT_ilm;
	...
	BMT_ilm = allocate_ilm(srcid);
	BMT_ilm->src_mod_id = srcid;
	if (chr_plug_status == CHARGER_PLUG_OUT)
	{
	   BMT_ilm->sap_id = PW_NO_CHR;
	}
	else
	{
	   BMT_ilm->sap_id = pw_chr_type;
	}

	BMT_ilm->dest_mod_id = MOD_BMT;
	BMT_ilm->msg_id = msgid;
	BMT_ilm->local_para_ptr = (local_para_struct *)NULL;
	BMT_ilm->peer_buff_ptr = NULL;
...
void Dcl_chr_det_reg_chr_usb(void)
{
	...
	EINT_SW_Debounce_Modify(gCHRDET_EINT_NO, 0); // Set de-bounce time
        EINT_Set_HW_Debounce_Ext(gCHRDET_EINT_NO, 10);
	#if defined(PMIC_CHR_USB_DET_EINT_LEVEL_ACTIVE_HIGH)
	EINT_Registration_Ext(gCHRDET_EINT_NO, KAL_TRUE, LEVEL_HIGH, CHRDET_HISR, KAL_FALSE);
	#endif //defined(PMIC_CHR_USB_DET_EINT_LEVEL_ACTIVE_HIGH)
	#if defined(PMIC_CHR_USB_DET_EINT_LEVEL_ACTIVE_LOW)
	EINT_Registration_Ext(gCHRDET_EINT_NO, KAL_TRUE, LEVEL_LOW, CHRDET_HISR, KAL_FALSE);
	#endif //defined(PMIC_CHR_USB_DET_EINT_LEVEL_ACTIVE_LOW)
	#endif // #if !defined (__CHARGER_USB_DETECT_WIHT_ONE_EINT__)

	#if defined (__CHARGER_USB_DETECT_WIHT_ONE_EINT__)
	#ifndef IC_MODULE_TEST // When module test, we don't want to perform cable detection (For saving module time)
	//EINT_SW_Debounce_Modify(chr_usb_detect.chr_usb_eint, 0); // Set de-bounce time
	EINT_Set_HW_Debounce_Ext(chr_usb_detect.chr_usb_eint, 100);
	#if defined(PMIC_CHR_USB_DET_EINT_LEVEL_ACTIVE_HIGH)
				EINT_Registration_Ext(chr_usb_detect.chr_usb_eint, KAL_TRUE, LEVEL_HIGH, CHR_USB_EINT_HISR, KAL_FALSE);
	#endif //defined(PMIC_CHR_USB_DET_EINT_LEVEL_ACTIVE_HIGH)
		
	#if defined(PMIC_CHR_USB_DET_EINT_LEVEL_ACTIVE_LOW)
				EINT_Registration_Ext(chr_usb_detect.chr_usb_eint, KAL_TRUE, LEVEL_LOW, CHR_USB_EINT_HISR, KAL_FALSE);
	#endif //defined(PMIC_CHR_USB_DET_EINT_LEVEL_ACTIVE_LOW)
...

	{
		...
		{
			EINT_UnMask((kal_uint8)chr_usb_detect.chr_usb_eint);
		}
	#endif	
	}
