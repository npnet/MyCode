...
DCL_UINT8 bd_buffer[2048];  //add by lqy
DCL_UINT16 bd_num=0; //add by lqy
...
DCL_STATUS UART_Handler(DCL_DEV dev,DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	//DCL_UINT16 bd_tempnum=0;
	//DCL_UINT8 *bd_tmpbuffer=NULL;
		...
		case SIO_CMD_GET_BYTES:
				{
					...
					//kal_prompt_trace(MOD_SOC," SIO_CMD_GET_BYTES  u2Length=%d",prCtrlGetBytes->u2Length);
				       prCtrlGetBytes->u2RetSize=pUart_CMD_FUNC[dev]->GetBytes(dev, prCtrlGetBytes->puBuffaddr, prCtrlGetBytes->u2Length, prCtrlGetBytes->pustatus, (module_type)(prCtrlGetBytes->u4OwenrId) );
                                    //kal_prompt_trace(MOD_SOC," SIO_CMD_GET_BYTES  u2RetSize=%d",prCtrlGetBytes->u2RetSize);
