...
/********************yanchunhai 20130413 begin*************************************/
/*
kal_bool is_ac_charger_in(void)
{
	CHR_DET_TYPE_ENUM chr_type;
	chr_type = bmt_get_chr_type();

       kal_prompt_trace(MOD_SOC,"is_ac_charger_in chr_type=%d",chr_type);	
	   
         if((PW_AC_CHR == chr_type) ||(PW_AC_NON_STD_CHR == chr_type))  //  PW_USB_CHR
         {
              return KAL_TRUE;
         }
         else
...
kal_bool is_ac_charger_in(void)
{
	CHARGER_IN_OUT_STATUS chr_type;

	
	chr_type = bmt_read_chr_status();

       TRACE_P_GPS("is_ac_charger_in chr_type=%d",chr_type);	
	   
         if(CHARGER_PLUG_IN == chr_type) //  PW_USB_CHR
         {
              return KAL_TRUE;
         }
         else
...
//lqy
	m_voltage = BATPHYS->VCHARGER;
	m_curr = BATPHYS->ICHARGE;
}
//add by lqy
void bird_get_vbat(BATPHYStruct *BATPHYS)
{
	BATPHYS->VBAT = BMT_VOL_RESULT[BMT_ADC_VBAT];
	drv_trace1(TRACE_INFO, BMT_VBAT_TRC, BATPHYS->VBAT);
}

kal_uint32 bird_get_charge_channel_voltage(void)
{
      return m_voltage;
}

kal_uint32 bird_get_charge_channel_curr(void)
{
      return m_curr;
}

void bird_set_charge_channel_voltage(void)
{
      m_voltage = 0;
}

void bird_set_charge_channel_curr(void)
{
       m_curr = 0;
}
...
void bmt_CtrlCharge(kal_uint8 ON)
{
	kal_bool chr_enable;
	kal_bool ChrCtrl;
	static kal_uint8 pre_on=KAL_TRUE;

    drv_trace1(TRACE_GROUP_1, BMT_CTRL_CHARGE_TRC, ON);

	TRACE_P_GPS("bmt_ctrlcharge utility  %d,bat_state=%d",ON,BMT.bat_state);    //  3
...
//add by yanchunhai
	 if(ON)
    {
           if(ON != pre_on)
           {
		   TRACE_P_GPS("bmt_ctrlcharge  exitsleep");
	 	bmt_enable_sleepmode(KAL_FALSE);

              
           }
         //   TRACE_P_GPS("bmt_ctrlcharge utility on %d,%d",ON,pre_on);
    ...
    else
    {
         ...
         if(ON != pre_on)
         {
	      ...
	      TRACE_P_GPS("bmt_ctrlcharge  sleep");
          
         }
           //  TRACE_P_GPS("bmt_ctrlcharge  utility off %d,%d",ON,pre_on);
    }
                 pre_on = ON;
}
