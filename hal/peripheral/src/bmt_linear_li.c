...
//extern  bmt_usb_state_enum bmt_usb_state;
...
#ifdef BIRD_EXTERNEL_POWER_UNSUPPORT      
{
  extern void rj_usb_charging(void);  
  rj_usb_charging();
}
#endif
#else
   /**yanchunhai add 20120817    begin  ************************/
         ...
         b_charge_usb_in = KAL_TRUE;
        TRACE_P_GPS("bmt_charge_start");    //bmt_get_chr_type();
      //  TRACE_P_GPS("bmt_charge_start LINEAR  bmt_usb_state=%d , %s  : %d",bmt_usb_state,__FILE__,__LINE__); 
...
#ifdef BIRD_EXTERNEL_POWER_UNSUPPORT      
{
  extern void rj_usb_uncharge(void);  
  rj_usb_uncharge();
}
#endif
#else
   ...
   /**yanchunhai add 20120817    begin  ************************/
       ...
       TRACE_P_GPS("bmt_charge_end linear  ,%s  : %d",__FILE__,__LINE__); 
