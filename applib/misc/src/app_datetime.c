...
#include "vdat_systime.h"
    ...
    #include "string.h"
#if !defined(EXTERNAL_MMI) && !defined(EMPTY_MMI)
...
#if defined(PLUTO_MMI) && defined(__MMI_WORLD_CLOCK__)
...
/*****************************************************************************
 * FUNCTION
 *  applib_dt_utc_to_rtc_with_default_tz
 * DESCRIPTION
 *  compute utc time to rtc time with default time zone
 * PARAMETERS
 *  tz      [IN]        Time zone
 *  utc     [IN]        Utc time
 * RETURNS
 *  result of time
 *****************************************************************************/
void applib_dt_utc_to_rtc_with_default_tz(applib_time_struct *utc, applib_time_struct *result)
{
#if !defined(EXTERNAL_MMI) && !defined(EMPTY_MMI)
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    float tz = GetTimeZone(PhnsetGetHomeCity());

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    applib_dt_utc_to_rtc(tz, utc, result);
#else /* EXTERNAL_MMI */
 ...
 *  result of time
 *****************************************************************************/
void applib_dt_rtc_to_utc_with_default_tz(applib_time_struct *rtc, applib_time_struct *result)
{
#if !defined(EXTERNAL_MMI) && !defined(EMPTY_MMI)
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    applib_dt_utc_to_rtc(tz, rtc, result);
#else /* EXTERNAL_MMI */
    memset(result, 0, sizeof(applib_time_struct));
#endif /* EXTERNAL_MMI */
}
        ...
        applib_time_struct *Result)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    applib_time_struct decrement_local;
    applib_time_struct *decrement_local_ptr = &decrement_local;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    memset(&decrement_local, 0x00, sizeof(applib_time_struct));
    memcpy(&decrement_local, decrement, sizeof(applib_time_struct));
    
    if (decrement_local_ptr->nSec != 0)
    {
        if (timeToDecrement->nSec >= decrement_local_ptr->nSec)
        {
            Result->nSec = timeToDecrement->nSec - decrement_local_ptr->nSec;
        }
        else
        {
            Result->nSec = DT_MIN_PER_HR + timeToDecrement->nSec - decrement_local_ptr->nSec;
            decrement_local_ptr->nMin++;
        }
    }
    else
    {
    ...
    if (decrement_local_ptr->nMin != 0)
    {
        if (timeToDecrement->nMin >= decrement_local_ptr->nMin)
        {
            Result->nMin = timeToDecrement->nMin - decrement_local_ptr->nMin;
        }
        else
        {
            Result->nMin = DT_MIN_PER_HR + timeToDecrement->nMin - decrement_local_ptr->nMin;
            decrement_local_ptr->nHour++;
        }
    }
    else
    {
    ...
    if (decrement_local_ptr->nHour != 0)
    {
        if (timeToDecrement->nHour >= decrement_local_ptr->nHour)
        {
            Result->nHour = timeToDecrement->nHour - decrement_local_ptr->nHour;
        }
        else
        {
            Result->nHour = DT_HRS_PRE_DAY + timeToDecrement->nHour - decrement_local_ptr->nHour;
            decrement_local_ptr->nDay++;
        }
    }
    else
    {
    ...
    if (decrement_local_ptr->nDay != 0)
    {
        if (timeToDecrement->nDay > decrement_local_ptr->nDay)
        {
            Result->nDay = timeToDecrement->nDay - decrement_local_ptr->nDay;
        }
        else
        {
            decrement_local_ptr->nMonth++;
            if (timeToDecrement->nMonth != 1)
            {
                Result->nDay = applib_dt_last_day_of_mon(
                                (kal_uint8) (timeToDecrement->nMonth - 1),
                                timeToDecrement->nYear) + timeToDecrement->nDay - decrement_local_ptr->nDay;
            }
            else
            {
                Result->nDay = 31 + timeToDecrement->nDay - decrement_local_ptr->nDay;
            }
        }
    }
    else
    ...
    if (decrement_local_ptr->nMonth != 0)
    {
        if (timeToDecrement->nMonth > decrement_local_ptr->nMonth)
        {
            Result->nMonth = timeToDecrement->nMonth - decrement_local_ptr->nMonth;
        }
        else
        {
            Result->nMonth = DT_MONTH_PER_YEAR + timeToDecrement->nMonth - decrement_local_ptr->nMonth;
            decrement_local_ptr->nYear++;
        }
    }
    else
    {
    ...
    Result->nYear = timeToDecrement->nYear - decrement_local_ptr->nYear;
 ...
 * DESCRIPTION
 *  Check if date/time is valid, the range of year is decided by application.
 * PARAMETERS 
 * RETURNS
 *****************************************************************************/
kal_bool applib_dt_check_valid_by_app(kal_uint16 min_year, kal_uint16 max_year, applib_time_struct *t)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    /* error handling */
    if (t->nYear > max_year || t->nYear < min_year)
    {
        return KAL_FALSE;
    }

    if (t->nMonth > 12 || t->nMonth == 0)
    {
        return KAL_FALSE;
    }
    ...
    if (t->nMonth == 2)
    {
        if (t->nDay > (g_dt_day_per_mon[t->nMonth - 1] + applib_dt_is_leap_year(t->nYear)) || t->nDay == 0)
        {
            return KAL_FALSE;
        }
    }
    ...
    {
        if (t->nDay > g_dt_day_per_mon[t->nMonth - 1] || t->nDay == 0)
        {
            return KAL_FALSE;
        }
    }
...
kal_bool applib_dt_is_dst_enabled(void)
{
    ...
    return( (kal_bool)mmi_wc_get_home_dst());
#else
 ...
 *  result of time
 *****************************************************************************/
DCL_RTC_DATA_T applib_dt_conv_utc_to_local(float tz, DCL_RTC_DATA_T *utc_time)
{
#if !defined(EXTERNAL_MMI) && !defined(EMPTY_MMI)
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    DCL_RTC_DATA_T outcome;
...
#else /* EXTERNAL_MMI */
	DCL_RTC_DATA_T outcome;
    memset(&outcome, 0, sizeof(DCL_RTC_DATA_T));
 ...
 *****************************************************************************/
kal_uint8 applib_dt_sec_2_mytime(kal_uint32 utc_sec, applib_time_struct *result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
kal_uint8 applib_dt_utc_sec_2_mytime(kal_uint32 utc_sec, applib_time_struct *result, kal_bool daylightSaving)
{
 ...
 kal_uint8 applib_dt_utc_sec_2_mytime_ext(kal_int64 utc_sec, applib_time_struct *result, kal_bool daylightSaving)
 {
     ...
     else
    {
		 ...
		 {
			result->nHour = (kal_uint8)(DT_HRS_PRE_DAY - sec / DT_SEC_PER_HOUR);
			if(result->nHour >= DT_HRS_PRE_DAY)
			{
				result->nHour = 0;
				utc_sec += DT_SEC_PER_DAY;
			}
		 }
		 else
		 {
			result->nHour = (kal_uint8)(DT_HRS_PRE_DAY - sec / DT_SEC_PER_HOUR - 1);
			/* min */
			sec %= DT_SEC_PER_HOUR;
			if (sec % DT_SEC_PER_MIN == 0)
			{
				result->nMin = (kal_uint8)(DT_MIN_PER_HR - sec / DT_SEC_PER_MIN);
				if(result->nMin >= DT_MIN_PER_HR)
				{					
					result->nMin = 0;
					utc_sec += DT_MIN_PER_HR;
				}
			}
			else
			{
				result->nMin = (kal_uint8)(DT_MIN_PER_HR - sec / DT_SEC_PER_MIN - 1);
				/* sec */
				result->nSec = (kal_uint8)(DT_SEC_PER_MIN - sec % DT_SEC_PER_MIN);
			}
		 }
...
#if defined (PLUTO_MMI)
//extern void AlmIndicationCheck(void *mytime);
extern void srv_reminder_expiry_check(void *mytime);
#endif
...
#if defined (PLUTO_MMI)
	srv_reminder_expiry_check(&MyTime);
#endif
...
void applib_dt_init_time(void)
{
    ...
    MyTime.nMonth = (kal_uint8) systime.wMonth;
    MyTime.nDay = (kal_uint8) systime.wDay;
    MyTime.nHour = (kal_uint8) systime.wHour;
    MyTime.nMin = (kal_uint8) systime.wMinute;
    MyTime.nSec = (kal_uint8) systime.wSecond;
    MyTime.DayIndex = (kal_uint8) applib_dt_dow(MyTime.nYear, MyTime.nMonth, MyTime.nDay);
...
#else /* EXTERNAL_MMI */
    memset(t, 0, sizeof(applib_time_struct));
#endif
}
...
void applib_dt_get_rtc_time(applib_time_struct *t)
{
    ...
    /*----------------------------------------------------------------*/
    g_rtc_handler = DclRTC_Open(DCL_RTC, FLAGS_NONE);
    DclRTC_Control(g_rtc_handler, RTC_CMD_GET_TIME, (DCL_CTRL_DATA_T *)&rtc_time);

    t->nYear = rtc_time.u1Year + YEARFORMATE;
    t->nMonth = rtc_time.u1Mon;
    t->nDay = rtc_time.u1Day;
    t->nHour = rtc_time.u1Hour;
    t->nMin = rtc_time.u1Min;
    t->nSec = rtc_time.u1Sec;
    t->DayIndex = applib_dt_dow(t->nYear, t->nMonth, t->nDay);
#else /* __MTK_TARGET__ */ 
 ...
 *  kal_uint8
 *****************************************************************************/
kal_uint8 bird_applib_dt_get_rtc_time(applib_time_struct *t)
{
#ifdef __MTK_TARGET__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    ...
    RTC_CTRL_GET_TIME_T rtc_time;
    DCL_STATUS dcl_ret = STATUS_FAIL;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_rtc_handler = DclRTC_Open(DCL_RTC, FLAGS_NONE);
    dcl_ret = DclRTC_Control(g_rtc_handler, RTC_CMD_GET_TIME, (DCL_CTRL_DATA_T *)&rtc_time);
...
kal_uint8 applib_dt_get_week_number(applib_time_struct *date)
{
	...
	day_on_nextjan = applib_dt_dow(date->nYear + 1, 1, 1);

	if (365 + applib_dt_is_leap_year(date->nYear) - no_of_days < day_on_nextjan)
	{
		week_num = 1;
	}
	else
