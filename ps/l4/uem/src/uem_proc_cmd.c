...
void l4cuem_call_status_req_ind(kal_uint8 ind_type)
{
    ...
    switch (ind_type)
    {
        case UEM_CALL_SETUP:
            ...
            //.HAL BMT_CallState(KAL_TRUE);
            l4cuem_bmt_set_callstate(KAL_TRUE);
        ...
        case UEM_CALL_DISCONNECT:
            ...
            //.HAL BMT_CallState(KAL_FALSE);
            l4cuem_bmt_set_callstate(KAL_FALSE);
 ...
 *  void
 *****************************************************************************/
void l4cuem_gpio_control_led_req(kal_uint8 led, kal_uint8 cmd, kal_uint8 on_time, kal_uint8 off_time)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_trace(TRACE_FUNC, FUNC_UEM_GPIO_CONTROL_LED, led, cmd, on_time, off_time);
    ...
    {
        gpio_led_control(led, cmd, on_time, off_time);
    }
}
...
kal_bool l4cuem_get_hw_level_req(l4cuem_set_hw_level_req_struct *hw_level)
{
    ...
    if (hw_level)
    {
        ...
        for( i = 0; i< PWM_MAX_BACKLIGHT_LEVEL; i++ )
        {
            ...
            hw_level->bl_PWM_freq_Level_Info[i] = PWM1_Level_Info[i][0];
            hw_level->bl_PWM_duty_Level_Info[i] = PWM1_Level_Info[i][1];
#elif defined(LED_MAINLCD_USE_PWM_2)            
            hw_level->bl_PWM_freq_Level_Info[i] = PWM2_Level_Info[i][0];
            hw_level->bl_PWM_duty_Level_Info[i] = PWM2_Level_Info[i][1];
#elif defined(LED_MAINLCD_USE_PWM_3)            
            hw_level->bl_PWM_freq_Level_Info[i] = PWM3_Level_Info[i][0];
            hw_level->bl_PWM_duty_Level_Info[i] = PWM3_Level_Info[i][1];
