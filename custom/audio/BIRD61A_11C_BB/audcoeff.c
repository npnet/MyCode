...
const unsigned char ToneViaSCO = 1; // BT SCO connect: Warning Tone play via 1: BT, 0: loudspeaker
...
const unsigned short CUSTOM_RG_DEPOP_CURSEL = 2;
...
const unsigned short SpkAmpGain = 3; //0: -6db, 1: -3db, 2: 6db, 3: 12db
#if defined(__VIBRATION_SPEAKER_SUPPORT__)
   const unsigned char Spk2In1 = 1; // 1: enable 2-in-1, 0: disable 2-in-1
#else
   const unsigned char Spk2In1 = 0; // 1: enable 2-in-1, 0: disable 2-in-1
#endif
const unsigned char extPA_underHP = 0;
const unsigned char mic0_type_sel = 0; //main mic  2:dcc(ecm), 1: dcc(mems), 0:acc
const unsigned char mic1_type_sel = 0; //ear mic   2:dcc(ecm), 1: dcc(mems), 0:acc
/*
