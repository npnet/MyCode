...
#endif /* __ENS__ */ 
#if defined(__SIM_CUSTOM_SAT_REFRESH__)
static kal_bool g_pd_ignore[MAX_SIM_NUM];
static kal_bool g_refresh_ignore[MAX_SIM_NUM];
static kal_bool g_tr_custom_enabled[MAX_SIM_NUM];
static kal_bool g_tr_custom_info_enabled[MAX_SIM_NUM];
static kal_uint8 g_tr_custom_result[MAX_SIM_NUM];    // SAT_CMD_PERFORMED_SUCCESSFULLY
static kal_uint8 g_tr_custom_info[MAX_SIM_NUM];      // SAT_NO_SPECIFIC_CAUSE


/*****************************************************************************
 * FUNCTION
 *  custom_sat_refresh_set
 * DESCRIPTION
 *  This function is used to customize the sat refresh behavior.
 * PARAMETERS
 *      simInterface                [IN]
 *      pd_ignore                   [IN]
 *      refresh_ignore              [IN]
 *      tr_custom_enabled           [IN]
 *      tr_custom_result            [IN]
 *      tr_custom_info_enabled      [IN]
 *      tr_custom_info              [IN]
 * RETURNS
 *      N/A
 * NOTE
 *       simInterface:              0-based sim index
 *       pd_ignore:                 used to ignore the sat profile download when sim initialization triggered by sat refresh. 
 *       refresh_ignore:            used to ignore the sat refresh. 
 *       tr_custom_enabled:         used to enabled custom TR result for sat refresh. 
 *       tr_custom_result:          the custom TR result for sat refresh.  (the result value shall be followed by 102.223, Table 6.1)
 *       tr_custom_info_enabled:    used to enabled custom addition info in TR  for sat refresh. 
 *       tr_custom_info:            the custom addition info in TR  for sat refresh..  (the result value shall be followed by 102.223, Table 6.1)
 *****************************************************************************/
void custom_sat_refresh_set(kal_uint8   simInterface,
                            kal_bool    pd_ignore,
                            kal_bool    refresh_ignore,
                            kal_bool    tr_custom_enabled,
                            kal_uint8   tr_custom_result,
                            kal_bool    tr_custom_info_enabled,
                            kal_uint8   tr_custom_info)
{
    /* Check Sim Interface Range*/
    if(simInterface > MAX_SIM_NUM)
    {
        return;
    }
    
    g_pd_ignore[simInterface] = pd_ignore;
    g_refresh_ignore[simInterface] = refresh_ignore;

    /* Customized TR result / addtional info */
    g_tr_custom_enabled[simInterface] = tr_custom_enabled;
    g_tr_custom_result[simInterface] = tr_custom_result;
    g_tr_custom_info_enabled[simInterface] = tr_custom_info_enabled;
    g_tr_custom_info[simInterface] = tr_custom_info;
}

kal_bool custom_sat_refresh_pd_ignore_get(kal_uint8 simInterface)
{
    /* Check Sim Interface Range*/
    if(simInterface > MAX_SIM_NUM)
    {
    ...
    return g_pd_ignore[simInterface];
}

kal_bool custom_sat_refresh_ignore_get(kal_uint8 simInterface)
{
    /* Check Sim Interface Range*/
    if(simInterface > MAX_SIM_NUM)
    {
    ...
    return g_refresh_ignore[simInterface];
}

kal_bool custom_sat_refresh_tr_enabled_get(kal_uint8 simInterface)
{
    /* Check Sim Interface Range*/
    if(simInterface > MAX_SIM_NUM)
    {
    ...
    return g_tr_custom_enabled[simInterface];
}

kal_uint8 custom_sat_refresh_tr_result_get(kal_uint8 simInterface)
{
    /* Check Sim Interface Range*/
    if(simInterface > MAX_SIM_NUM)
    {
        return SAT_CMD_PERFORMED_SUCCESSFULLY;
    }

    return g_tr_custom_result[simInterface];
}

kal_bool custom_sat_refresh_tr_info_enabled_get(kal_uint8 simInterface)
{
    /* Check Sim Interface Range*/
    if(simInterface > MAX_SIM_NUM)
    {
    ...
    return g_tr_custom_info_enabled[simInterface];
}

kal_uint8 custom_sat_refresh_tr_info_get(kal_uint8 simInterface)
{
    /* Check Sim Interface Range*/
    if(simInterface > MAX_SIM_NUM)
    {
        return SAT_NO_SPECIFIC_CAUSE;
    }

    return g_tr_custom_info[simInterface];
}
#endif    /* __SIM_CUSTOM_SAT_REFRESH__ */
...
/*****************************************************************************
 * FUNCTION
 *  custom_sat_enable_comprehension_device_identities
 * DESCRIPTION
 *  As per 51.014, comprehension flag for 'Device Identities' is not necessary for 'Terminal Response'.
 *  For all comprehension-tlv objects with MIN = N, terminal should set the CR flag to 'not required'
 *  This custom function bypass this requirement.
 * PARAMETERS
 *  none
 * RETURNS
 *  KAL_FALSE or KAL_TRUE
 *****************************************************************************/
kal_bool custom_sat_enable_comprehension_device_identities(void)
{
    return KAL_FALSE;
}

#if defined(__SAT_CUSTOM_BYPASS_REFRESH_RESET__)
/*****************************************************************************
 * FUNCTION
 *  sat_custom_disable_refresh_reset
 * DESCRIPTION
 *  bypass SAT REFRESH - reset type
 * PARAMETERS
 * N/A
 * RETURNS
 * 
 ...
 *  
 *****************************************************************************/
kal_bool sat_custom_bypass_refresh_reset(void)
{
    return KAL_TRUE;
}
#endif /* __SAT_CUSTOM_BYPASS_REFRESH_RESET__ */
