...
#include "drvflash_ut.h"  //LocateSector_ext

#include "custom_nvram_int_config.h"  /* To get NVRAM_CUSTOM_CFG_MAX_RECORD_SECTOR_NUM for MSTABLE_ENTRY_NUM */


//Hard code memory layout setting for smart phone modem load instead of generate by emigen
#if defined(__SMART_PHONE_PLATFORM__) && !defined(__SMART_PHONE_MODEM__)
// 3 MB for MO/MT's load
#define REGION_INFO_LAYOUT {0x10000, 48},
#define TOTAL_BLOCKS          48
#define BANK_INFO_LAYOUT    { 0x300000, 1 },
...
kal_bool INT_QueryIsROMSpace(kal_uint32 addr)
{
   ...
   /* Nand project always return false */
   #if !defined(_NAND_FLASH_BOOTING_)
   // The complex statement here is to fix build warning of: pointless comparison of unsigned interger with zero in bootloader 
   if(( ((NOR_FLASH_BASE_ADDR)?(addr>= NOR_FLASH_BASE_ADDR):(KAL_TRUE)) && addr<(NOR_FLASH_BASE_ADDR+NOR_FLASH_SIZE)) || (addr>=0x90000000 && addr<(0x90000000+NOR_FLASH_SIZE)))
       return KAL_TRUE;
   else
   #endif
       return KAL_FALSE;
