/* EasyCASE V6.5 27/01/2011 11:08:55 */
/* EasyCASE O
If=vertical
LevelNumbers=no
LineNumbers=no
...
const U8  latch_time_tab[8] = {0x0c,0x0d,0x0e,0x01,0x02,0x03,0x04,0x05};     //   (0x8c  12.5ms)    (0x8d  25ms)   (0x8e  50ms)   (0x81  250ms)  (0x82  500ms)  (0x83  1s)  (0x84 2s)  (0x85 4s)                    (0x07 latch eint)
int bma222_init(bma222_t *bma222)
   ...
   p_bma222->bus_write = acc_sensor_write_bytes;
  p_bma222->bus_read = acc_sensor_read_bytes;  

   // p_bma222->bma222_WR_FUNC_PTR = acc_sensor_write_bytes;
   //p_bma222->bma222_BUS_READ_FUNC = acc_sensor_read_bytes;  
...
/* EasyCASE F */
/*******************************************************************************
 * Description: *//**\brief  This API reads data from the temp register
 *
 *
 *
 *
 ...
 *
 ******************************************************************************/
int bma222_get_temp(unsigned char *temp)
   {
   int comres = C_Zero_U8X ;
   unsigned char data;
   if (p_bma222 == C_Zero_U8X)
      ...
      {
      comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_TEMP_RD_REG, &data, C_One_U8X);
      *temp = data;
      }
   return comres;
   }
...
int bma222_reset_interrupt_xyz(void)
   {
   int comres=C_Zero_U8X;
   unsigned char data;
   if (p_bma222==C_Zero_U8X)
      ...
      {
      comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_INT_RESET_LATCHED__REG, &data, C_Three_U8X );
      data = bma222_SET_BITSLICE(data, bma222_INT_RESET_LATCHED, C_Three_U8X );
      comres = p_bma222->bma222_BUS_WRITE_FUNC(p_bma222->dev_addr, bma222_INT_RESET_LATCHED__REG, &data, C_Three_U8X);
      }
   return comres;
   }
/* EasyCASE ) */
/* EasyCASE ( 100682
   bma222_get_interruptstatus */
/* EasyCASE F */
/*******************************************************************************
 * Description: *//**\brief This API gets the interrupt status register
 *                       0x09 to 0x0C register will be read( 4 bytes)
 *
 *
 *
 ...
 *
 ******************************************************************************/
int bma222_get_interruptstatus(unsigned char *intstatus )
   {
   int comres = C_Zero_U8X ;
   if (p_bma222 == C_Zero_U8X)
      {
      ...
      {
      comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_STATUS1_REG, intstatus, C_Four_U8X );
      }
   return comres;
   }
/* EasyCASE ) */
   ...
   if (p_bma222==C_Zero_U8X)
      ...
      switch (int1sel)
         ...
         case C_Six_U8X:
          ...
          case C_Seven_U8X:   //ych 
            comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_EN_INT1_PAD_NO_MOTION_SLOPE__REG, &data, C_One_U8X );
            data = bma222_SET_BITSLICE(data, bma222_EN_INT1_PAD_NO_MOTION_SLOPE, state );
            comres = p_bma222->bma222_BUS_WRITE_FUNC(p_bma222->dev_addr, bma222_EN_INT1_PAD_NO_MOTION_SLOPE__REG, &data, C_One_U8X);
            break;
         default:  
            break;
         }
...
int bma222_set_slope_no_motion_duration(unsigned char duration)
   {
   int comres=C_Zero_U8X;
   unsigned char data;
   if (p_bma222==C_Zero_U8X)
      ...
      {
      comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_SLOPE_NO_MOTION_DUR__REG, &data, C_One_U8X );
      data = bma222_SET_BITSLICE(data, bma222_SLOPE_NO_MOTION_DUR, duration );
      comres = p_bma222->bma222_BUS_WRITE_FUNC(p_bma222->dev_addr, bma222_SLOPE_NO_MOTION_DUR__REG, &data, C_One_U8X);
      }
   return comres;
   }
 ...
 *
 ******************************************************************************/
int bma222_set_slope_no_motion_threshold(unsigned char threshold)
   {
   int comres=C_Zero_U8X;
   unsigned char data;
   if (p_bma222==C_Zero_U8X)
      ...
      {
      comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_SLOPE_NO_MOTION_THRES__REG, &data, C_One_U8X );
      data = bma222_SET_BITSLICE(data, bma222_SLOPE_NO_MOTION_THRES, threshold );
      comres = p_bma222->bma222_BUS_WRITE_FUNC(p_bma222->dev_addr, bma222_SLOPE_NO_MOTION_THRES__REG, &data, C_One_U8X);
      }
   return comres;
   }
/* EasyCASE ) */
 ...
 *
 ******************************************************************************/
int bma222_set_i2c_wdt_en(void)
   {
   int comres=C_Zero_U8X;
   unsigned char data;
   unsigned char update;
   ...
   if (p_bma222==C_Zero_U8X)
      ...
      {
      comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_EN_I2C_WATCHDOG__REG, &data, C_One_U8X );
      data = bma222_SET_BITSLICE(data, bma222_EN_I2C_WATCHDOG, update );
      comres = p_bma222->bma222_BUS_WRITE_FUNC(p_bma222->dev_addr, bma222_EN_I2C_WATCHDOG__REG, &data, C_One_U8X);
      }
   return comres;
   }
/* EasyCASE ) */
/* EasyCASE ( 100678
   bma222_get_i2c_wdt_en_status */
/* EasyCASE F */
/*******************************************************************************
 ...
 *
 ******************************************************************************/
int bma222_get_i2c_wdt_en_status(unsigned char *status )
   {
   int comres = C_Zero_U8X ;
   unsigned char data;
   if (p_bma222 == C_Zero_U8X)
      ...
      {
      comres = p_bma222->bma222_BUS_READ_FUNC(p_bma222->dev_addr, bma222_SERIAL_CTRL_REG, &data, C_One_U8X );
      data = bma222_GET_BITSLICE(data, bma222_EN_I2C_WATCHDOG);
      *status = data;
      }
   return comres;
   }
 ...
 *               0111            latched
 *               1000            non-latched
 *               1001            temporary, 500 峴
 *               1010            temporary, 500 峴
 *               1011            temporary, 1 ms
...
int bma222_get_update_image_status(unsigned char *imagestatus )
  ...
  /*  add by ranyingchun */
#ifdef BIRD_SWZ
extern U32 bird_get_hitlvalue();
void bma222_high_g_interrupt_init(void)
	{


		U32 threshold;
		threshold = bird_get_hitlvalue();
		bma222_set_mode(bma222_MODE_NORMAL);

   	  	bma222_set_range(2);     // 2g       0 -- 2g  1--4g  2--8g 3--16g
    		bma222_set_bandwidth(bma222_BW_62_50HZ);   //  bma222_BW_125HZ
		
	 
		bma222_set_int_set(0,0);   // ox20            active low 
		bma222_set_int_set(1,1);   //    int1   0-- pull_up;   1--open drain               


                     
		                                       //       0x24
		bma222_set_Int_Mode(0x8e);   //0x21     latch       (0x8e  50ms)                     (0x07 latch eint)

		bma222_set_high_g_duration(15);        // 0x25        (15+1)*2 ms             

		bma222_set_high_g_threshold((unsigned char)(threshold*1000.0/31.25));   // 0x26            8g 31.25mg*0x60=3g            31.25mg(8g)      62.5mg(16g)(0x70,7g)


		bma222_set_high_hy(1); 


		bma222_set_int1_pad_sel(1);        // high g map to int1
		bma222_set_enable_high_g_interrupt(0);	 //  0x17
		bma222_set_enable_high_g_interrupt(1);	 //  0x17
	
    		
	}
void bma222_high_g_interrupt_mode_on(void)
	{
		unsigned char thresh_data;
		unsigned char high_g_interupt_off=0x0;

		bma222_write_reg(0x17, &high_g_interupt_off, 1);  

		
	   	bma222_set_range(3);     // 2g       0 -- 2g  1--4g  2--8g 3--16g
    		bma222_set_bandwidth(bma222_BW_62_50HZ);   //  bma222_BW_125HZ
...
void bma222_move_quiet_mode(void)
{
	unsigned char high_g_interupt_off=0x0;
	 bma222_set_range(0);
	bma222_write_reg(0x17, &high_g_interupt_off, 1); 
...
**返回1设置成功，-1设置失败
*/
int bma222_set_high_g_interrupt_threshold(float threshold)
	{
		unsigned char range,thresh_data;
		unsigned char high_g_interupt_status;
		unsigned char high_g_interupt_off=0x0;
		if (threshold>16 || threshold<0)
			return -1;
		if (threshold>0 && threshold<1)
			threshold = 1;
		thresh_data = (unsigned char)(threshold*1000.0/31.25);
		bma222_read_reg(0x17, &high_g_interupt_status, 1);;
		bma222_write_reg(0x17, &high_g_interupt_off, 1);  
		bma222_set_high_g_threshold(thresh_data);  
		bma222_write_reg(0x17, &high_g_interupt_status, 1);  //  0x17		
		return 1;
	}
#endif
#ifdef MS_EINT_SUPPORT
extern void acc_sensor_interrput_configuration_bma222e();
extern U8 bird_get_vibration_sensitivity_value();
extern U32 bird_get_hitlvalue();
extern U8 bird_get_hitldelaytime();
extern U8 bird_get_hitl_latch_time();
void bma222_interrupt_init(void)
	{
		unsigned char no_mot_data=0x0f;
		U32 threshold;
		U8 delayms;
		U8 latch_time_index;
		U8 latch_time;
		...
		threshold = bird_get_hitlvalue();
		delayms = bird_get_hitldelaytime();
		latch_time_index = bird_get_hitl_latch_time();
		...
		if((latch_time_index>=1)&&(latch_time_index<=8))
			latch_time = latch_time_tab[latch_time_index-1];
		else
			latch_time = latch_time_tab[4];

		latch_time = latch_time |0x80;  
		                                       //       0x24
		bma222_set_Int_Mode(latch_time);   //0x21     latch   (0x8b  1ms)       (0x8c  12.5ms)    (0x8d  25ms)   (0x8e  50ms)   (0x81  250ms)  (0x82  500ms)  (0x83  1s)  (0x84 2s)  (0x85 4s)                    (0x07 latch eint)
		//bma222_set_Int_Enable(unsigned char InterruptType , unsigned char value );
	                                     //                   0x19
/*   high_g interrupt  */
		bma222_set_high_g_duration((delayms-1)/2);        // 0x25        (15+1)*2 ms             
		bma222_set_high_g_threshold((unsigned char)(threshold*1000.0/31.25));   // 0x26            8g 31.25mg*0x60=3g            31.25mg(8g)      62.5mg(16g)(0x70,7g)
	//	bma222_set_high_g_threshold(0xE0);   // 0x26            8g 31.25mg*0x60=3g            31.25mg(8g)      62.5mg(16g)(0x70,7g)
		bma222_set_high_hy(1); 

		bma222_set_int_set(2,0);   // ox20            active low 
		bma222_set_int_set(3,1);   //    int1   0-- pull_up;   1--open drain    
 		bma222_set_int2_pad_sel(1);                                              //                   0x1b
		//bma222_set_int1_pad_sel(1);        // high g map to int1
		bma222_set_enable_high_g_interrupt(0);	 //  0x17
		bma222_set_enable_high_g_interrupt(1);	 //  0x17
...
/* any motion */
/*
  		bma222_set_slope_duration(0);          // 1     //  0x27    slop_dur<1:0>   3+1=4 point

		//bma222_set_slope_threshold(2);               // 0x28    3        15.63mg(8g)     31.25mg(16g)
		bma222_set_slope_threshold((bird_get_vibration_sensitivity_value() +1)/4+ 1);               // 0x28    3.9mg(2g)        15.63mg(8g)     31.25mg(16g)
		
		bma222_set_int1_pad_sel(2);
		bma222_set_enable_slope_interrupt(3);                  // 0x16       3 -- > enable slope XYZ
*/
/*  no motion */	  
/*
		bma222_set_slope_no_motion_duration(9);    // 0x27    9+1=10s
		bma222_set_slope_no_motion_threshold(1);    // 0x29     3.9mg(2g)   15.63mg(8g)     31.25mg(16g)
		bma222_set_int1_pad_sel(7);                                   // int1
		bma222_write_reg(0x18, &no_mot_data, 1);         // 0x18
*/
    		acc_sensor_interrput_configuration_bma222e();	
	}
//extern b_motion;
void bma222_high_g_interrupt_mode(void)
	{
		unsigned char thresh_data;
		unsigned char high_g_interupt_off=0x0;
		unsigned char no_mot_interupt_off=0x0;
		unsigned char slope_interupt_off=0x0;
		U32 threshold;
		threshold = bird_get_hitlvalue();
		bma222_write_reg(0x17, &high_g_interupt_off, 1);  
		bma222_write_reg(0x16, &slope_interupt_off, 1);  
		bma222_write_reg(0x18, &no_mot_interupt_off, 1);         // 0x18
...
/*   high_g interrupt  */
		bma222_set_high_g_duration(15);        // 0x25        (15+1)*2 ms             
		bma222_set_high_g_threshold((unsigned char)(threshold*1000.0/62.5));   // 0x26            8g 31.25mg*0x60=3g            31.25mg(8g)      62.5mg(16g)(0x70,7g)
...
void bma222_slope_no_motion_interrupt_mode(void)
	{
		unsigned char no_mot_interupt_on=0x0f;
		unsigned char no_mot_interupt_off=0x0;
		unsigned char slope_interupt_off=0x0;
		unsigned char high_g_interupt_off=0x0;
		unsigned char slope_interupt_threshold;
		
		bma222_write_reg(0x17, &high_g_interupt_off, 1); 
		bma222_write_reg(0x16, &slope_interupt_off, 1);  
		bma222_write_reg(0x18, &no_mot_interupt_off, 1);         // 0x18
		
	   	bma222_set_range(0);     // 2g       0 -- 2g  1--4g  2--8g 3--16g
    		bma222_set_bandwidth(bma222_BW_62_50HZ);   //  bma222_BW_125HZ

/* any motion */
  		bma222_set_slope_duration(2);               //  0x27    slop_dur<1:0>   3+1=4 point

	//	bma222_read_reg(0x28, &slope_interupt_threshold, 1);
	//	if(slope_interupt_threshold == 2)
		bma222_set_slope_threshold(bird_get_vibration_sensitivity_value() + 2);               // 0x28    3.9mg(2g)        15.63mg(8g)     31.25mg(16g)
		kal_sleep_task(1);
		bma222_read_reg(0x28, &slope_interupt_threshold, 1);
		kal_prompt_trace(MOD_SOC,"bma222_slope_no_motion_interrupt_mode slope_interupt_threshold = %d ",slope_interupt_threshold);  

	//	else
	//		bma222_write_reg(0x28, &slope_interupt_threshold, 1);  	
...
/*  no motion */	  
		...
		bma222_set_slope_no_motion_threshold(1);    // 0x29     3.9mg(2g)   15.63mg(8g)     31.25mg(16g)
	//	bma222_set_int1_pad_sel(7);                                   // int1
		bma222_write_reg(0x18, &no_mot_interupt_on, 1);         // 0x18
	}
void bma222_no_motion_interupt_on(void)
	{
		unsigned char no_mot_interupt_on=0x0f;
		bma222_write_reg(0x18, &no_mot_interupt_on, 1);         // 0x18
	}
...
int bma222_set_high_g_interrupt_threshold(float threshold)
	{
		...
		else if (range == 0x0c)  //16g
			{
				thresh_data = (unsigned char)(threshold*1000.0/62.5);
			}
		else 
			return -1;
		bma222_read_reg(0x17, &high_g_interupt_status, 1);;
...
/*   
** 修改high g 中断延时触发时间2ms --512ms
**返回1设置成功，-1设置失败(1-2) 2ms     (3 - 4) 4ms
*/
int bma222_set_high_g_interrupt_delaytime(unsigned char delayms)
	{
		unsigned char high_g_interupt_status;
		unsigned char high_g_interupt_off=0x0;
		if((delayms>512)||(delayms<1))
			return -1;
		bma222_read_reg(0x17, &high_g_interupt_status, 1);;
		bma222_write_reg(0x17, &high_g_interupt_off, 1);  
		bma222_set_high_g_duration((delayms-1)/2);        // 0x25        (15+1)*2 ms         
...
/*   
** 修改high g 中latch时间  12.5ms -4s   (1  12.5ms)    (2  25ms)   (3  50ms)   (4  250ms)  (5  500ms)  (6  1s)  (7 2s)  (8  4s)  
**返回1设置成功，-1设置失败 
*/
int bma222_set_high_g_interrupt_latch_time(unsigned char latch_time_index)
	{
		unsigned char high_g_interupt_status;
		unsigned char high_g_interupt_off=0x0;
		U8 latch_time;
...
/*   
** 修改slope_no_motion 中断阈值澹(0~15)
*  0- 灵敏度最高，15灵敏度最高
**返回1设置成功，-1设置失败
*/
int bma222_set_slope_motion_threshold(unsigned char threshold)
	{

		unsigned char slope_interupt_off=0x0;
		unsigned char slope_interupt_status;
		
		if(threshold > 0 &&threshold <=15)
		{
		//	threshold = 16 -threshold;
			bma222_read_reg(0x16, &slope_interupt_status, 1);;
			bma222_write_reg(0x16, &slope_interupt_off, 1);  	
	/* any motion */
	  	//	bma222_set_slope_duration(2);               //  0x27    slop_dur<1:0>   3+1=4 point
			bma222_set_slope_threshold((threshold+1)/4+1);               // 0x28    3.9mg(2g)        15.63mg(8g)     31.25mg(16g)

			bma222_write_reg(0x16, &slope_interupt_status, 1); 
			//bma222_set_enable_slope_interrupt(3);                  // 0x16       3 -- > enable slope XYZ
			TRACE_P_3D("bma222_set_slope_motion_threshold  OK");
			return 1;
		}
		else
		{
			TRACE_P_3D("bma222_set_slope_motion_threshold error");
			return -1;
		}
